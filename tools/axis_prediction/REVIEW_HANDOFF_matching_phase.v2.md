...
★**커버리지 ↔ 신설·갭 상관** — M1 재태깅 우선순위의 정량 근거
```
단원            overlay 커버리지    신설    갭
닮음 M2_SIMPY      87/87 = 100%       0     0(구조적 — 예측층 유형이 0개)
수연산 M2_NE       71/211 = 34%       0     22항목(원시 31)·판정불가 개념 4
M2_GEOM            27/140 = 19%       8     5
M1 기하                     0%        —     전 개념 판정불가(대조 자체가 불가)
```
⇒ **overlay 0%인 단원은 처방을 써도 예측·관측 대조가 원리상 불가능하다.** M1은 재태깅이 선행되어야 한다.
