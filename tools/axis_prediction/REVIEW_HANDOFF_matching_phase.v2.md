...
- **설계 질의 (b) 미정**: 관측 데이터 **기관 통합 여부** — 설계 시 사용자 질의. ★D1 구조를 가르므로 스키마 확정 전 필요. **후보안 함께 제시**: "지금 전부 사용자 데이터"이므로 **통합 상태로 두되 분리 가능한 구조(org_id 컬럼·인덱스)만 확보**하는 방식도 가능 — 이 안 포함해 질의.
...
## 문서 계보 (★검수 2026-08-13 요청)
- **전체 인계 = `HANDOFF_검수_20260811.md`**(§번호 체계 · 역할 · URL · 인프라). ⚠**아직 리포 미보유** — 검수 보유본. 사용자 전달 → `tools/axis_prediction/`에 커밋 예정(TAG_DICTIONARY_v2 사례: 검수 채팅에만 있으면 유실).
- **현 국면 = 이 문서(v2)**. `HANDOFF_검수_20260811.md`의 §9 URL 등 전체 인계 내용은 여기 비어 있음(중복 금지, 상위 문서 참조).
- **★새 세션은 둘 다 읽을 것**: 전체 인계(역할·URL·인프라) + v2(현 국면·완료·백로그·다음 트랙).
- **다음 세션 시작 파일 = `HANDOFF_검수_20260811.md`**(리포 커밋 후) → 그 안에서 이 v2로 연결. (커밋 전까지는 v2가 진입점.)
