...
## ★★ 현재 재개 지점 (2026-08-18 갱신, 새 세션은 여기부터) ★★
> **Code탭 새 세션은 `HANDOFF_B_ARITH_PRESCRIPTION.v1.md` 로 바로 갈 것.** 현재 진행 중인 유일한 저작 스트림(수연산 58/414)의 진입점이며, 규격·도구·다음 할 일이 그 문서에 있다. 이 v2 문서는 **전체 국면·백로그·함정** 참조용.

### 2026-08-18 시점 요약
| 트랙 | 상태 |
|---|---|
| 매칭 파이프라인 | ✅ 종결 |
| admin 목록 UI | ✅ 종결 — **서버 페이지네이션까지**(워커 `2026.08.18-list-paging` 배포 확인). 총 등록 1,076건에서 1,000 상한 도달 → offset/limit·서버 필터·전체기준 집계로 해소. 정렬 안정성 결함(배치 동일 `created_at`) `id` 타이브레이커로 수정 |
| M2_GEOM 처방 140 | ✅ 완결 |
| **닮음(M2_SIMPY) 처방 87** | ⚠ **정식 빌드 미완**(2026-08-20 갱신) — 부분 카탈로그(87/177) 기준 87종 · 문항 34% 배정 재검토 필요. 저작 87/87 → part04 append(2023→2110) → 매니페스트 12523→12610 → 게이트 12→11단원 · 검증 7항목 PASS는 **그 부분 카탈로그 위에서** 이뤄진 것. 결과 = `B_similarity_merge_result.v1.md` · 사유 = `HANDOFF_B_CATALOG_TRUNCATION.v1.md` |
| **수연산 처방 414** | 🔵 **진행 중 58/414** — 진입점 `HANDOFF_B_ARITH_PRESCRIPTION.v1.md` |
| M1 기하 재태깅 192 | ⏸ 사용자 GPT 작업 대기 |
| 백로그 | ⑤ 비용 실측 **종결**(6.2%라 착수 가치 낮음, 옵션1 미착수) / **⑥ 관측→태그 절차 설계만 남음**(100종 이후) |

### 이 국면의 주 산출 (최종 제출에 쓸 것)
★**커버리지 ↔ 판정불가율** — M1 재태깅 우선순위의 정량 근거 (**2026-08-18 근거 정정본**)
> 🔴 **2026-08-20 갱신 — 닮음 행은 끝점에서 제외됐다(검수 판정 10차 §5).** 분모 87이 문항에서 조립된 부분 카탈로그(정식 177)이므로 100%는 성취가 아니라 구성상 필연이다. 각주가 아니라 **제외**이며, 아래 표에서 닮음 행을 근거로 인용하지 말 것. 원의 성질 100%도 같은 이유로 제외 대상. **대체 끝점 = 절단되지 않은 단원**(도형의 성질 140/140 · 그 외 36단원), 선정 미착수.

```
단원            overlay 커버리지   신설   판정불가율        갭(분모 병기 필수)
[제외] 닮음 M2_SIMPY  87/87 = 100%    0    0%              n/a (분모=조립본. 끝점 제외)
수연산 M2_NE(100)  71/211 = 34%      0    30% (11개념 30종) 35항목·원시45 / 분모 87 = 51.7%
M2_GEOM            27/140 = 19%      8    40% (재측정 16/40) 원시10 / 분모 79 = 12.7% (재측정치)
M1 기하                     0%       —    100% (전 개념)    측정 불가
```
⇒ **커버리지가 낮아도 처방은 쓸 수 있으나(신설 0), 그 처방이 맞는지 확인할 수 없다.** M1은 재태깅이 선행되어야 한다.

★★**검수 논리 정정(2026-08-18) — 이전 서술을 인용하지 말 것.**
- **폐기**: "커버리지가 낮으면 신설이 늘어난다"(8/14~ 검수 서술). **실측으로 반증** — 신설률은 커버리지에 단조적이지 않다(100%→0 · 34%→0 · 19%→**8**). tier-2가 자기 overlay 부재를 대신 받기 때문이며, 수연산 배치6은 overlay 10%에서도 신설 0이었다.
- **정정**: 근거는 **판정불가율**이다(100%→0% · 34%→30% · 0%→100%, 거의 단조적).
- 신설률 = 태그를 **붙일 수 있는가**(저작 가능성) / 판정불가율 = 붙인 태그가 **맞는지 확인할 수 있는가**(검증 가능성). 커버리지는 후자를 직접 결정한다. 두 지표를 섞지 말 것.
- **실물 예시(보존)**: 배치6 등식의 변형 17종(`M2_NE_PT195`~`PT211`) — 태그는 다 붙었고 검증 13항목도 통과했으나 개념 관측 0이라 검증 수단이 없다. M1 전 단원이 이 상태다.
- 정본 = `_meta.m1_rationale_correction` · 상세 = `HANDOFF_B_ARITH_PRESCRIPTION.v1.md` §4 · 수치 = `B_arith_100types_interim_report.v1.md`.

★**갭 수치는 분모 없이 인용 금지**(검수 지시). 분모 = 판정가능 개념(관측≥1 AND 예측층≥1)의 예측층 유형에 배치한 코드 슬롯 수. `collect_gaps.ps1`의 `[DENOMINATORS]` 블록이 자동 출력한다. **GAP RATE는 tier 구성의 함수이지 예측 품질 지표가 아니다.**
★**M2_GEOM 값은 재측정치** — 드래프트 기록(집계 5·편재제외 1)과 재측정(6·5)이 다르다(당시 절차가 수작업 혼합). 병합 완료본이라 **덮어쓰지 않았다**. 인용 시 이 단서 병기.

### (옛 재개블록 — 2026-08-15, 히스토리)
...
- **`git add`는 경로 명시. 디렉터리 통째 금지**(2026-08-18 실사고 — `git add tools/axis_prediction/`로 무관 파일 20개·+17,901줄 동반 커밋. 결과 무해로 **현상 유지 판정**, 되돌리지 않음).
- **diff 귀인**: 육안 재입력본 금지, 원문 JSON/코드포인트/도구 diff만.
- **★★"억제 건수 ≠ 카탈로그 종수" — 2회 발생**(M2_GEOM "272 vs 332" · 수연산 "739 vs 1053", 검수 정정 2026-08-16). `no_template_for_unit` 억제 건수·진단 로그 집계는 **그 진단에 등장한 유형 수**이지 단원 카탈로그 종수가 아니다. 저작 스코프는 **반드시 `data/problem_types/*.problem_types.v1.json` 실집계**로 잡을 것. 실측: 수연산 6단원 카탈로그 1053(억제 보고값 739). ★수치 출처를 문서에 함께 적을 것(어느 파일을 세었는지).
- **★검증 기준은 검증 대상과 독립일 것**(3회 발생) — PS 스크립트 zero-pad 버그로 "0-cp"·regex "M2 digit strip" 오탐. 스크립트 오류를 데이터 문제로 오인 말 것(재계산으로 확인).
- **★★그룹 간 비교는 교란 변수를 통제한 뒤에 할 것. 통제하지 않은 비교를 효과로 읽지 말 것**(2026-08-19 신설, 검수 지시).
  - **위 항목과 같은 계열이되 다른 축이다.** 위는 *무엇을 기준으로 재는가*(기준의 독립성), 이 항목은 *비교 대상의 조건이 통제되었는가*(비교의 타당성)이다.
  - **실사고**: M3_REAL 배치2에서 `not_shared` tier-2 76.4% vs `shared` 36.4%를 근거로 **기제② 가설 "반증"이 판정**됐다. 그러나 두 그룹의 **overlay 보유율이 37% vs 100%**로 달랐다 — tier-1의 정의가 "자기 overlay 태그"이므로 보유율이 tier 비중을 거의 그대로 결정한다. 즉 **구조 효과로 읽은 것이 보유율 효과일 수 있었다.**
  - **발견 경로**: 배치3에서 **구조를 공유하는(shared) 그룹이 tier-2 72.5%**를 기록해(보유율 35.3%), 배치2 shared(36.4%, 보유율 100%)와 정반대 위치에 놓인 것. 구조 등급이 아니라 보유율 순으로 6개 그룹이 정렬됐다.
  - **조치**: 반증 판정을 **"교란 미통제 상태의 관측"으로 격하**하고(`_meta.tier_breakdown.finding_batch2_check2_review_conclusion`), 확인 항목 2를 **보유율을 맞춘 그룹 쌍**으로 재설계했다(배치4 = shared 41.7% vs not_shared 41.7%).
  - ★**검수가 스스로 내린 판정을 Code탭 관측으로 정정한 사례**다. 판정 주체가 누구든 교란 통제는 같은 기준으로 적용한다.
  - ★**통제 편성이 불가능하면 미결로 종결한다**(검수 지시). 억지 결론보다 미결이 낫다.
- **★★생성기는 산출물의 "바이트"가 아니라 "동작"을 검증해야 한다**(2026-08-18 신설, 검수 지시).
  - **위 항목과 다른 실패다.** 위 항목은 *무엇을 기준으로 재는가*(기준의 독립성), 이 항목은 *무엇을 재는가*(형식 vs 동작)의 문제다. **둘이 짝을 이뤄야 도구 3종 시험이 완결된다.**
  - **실사고**: `new_tool.ps1`이 한 줄짜리 `.ps1`을 생성했는데(PS `,`가 `+`보다 결합력이 강해 배열이 뭉개짐), 1행이 `#`로 시작해 **스크립트 전체가 주석**이 됐다. 그런데 생성기는 **비ASCII 바이트 0을 확인하고 `[verified]`를 출력**했다 — 형식 검사만 했기 때문이다. 생성물은 실행되고 exit 0이며 출력만 없었다.
  - **원칙**: 산출물이 실행 가능한 것(스크립트·설정·쿼리)이면 **실제로 실행해 기대 출력이 나오는지**까지 확인하고, 실패하면 산출물을 폐기할 것.
  - **현행 4항목 검증**(검수 승인): ①비ASCII 0 ②줄 수 ③가드 블록 존재 ④**실제 실행 출력 문자열**. ★**④가 핵심이고 나머지 3개는 형식 검사**다 — ①~③만으로는 위 사고를 잡지 못한다.
- **★★도구 3종 시험 = 상시 항목**(검수 지시 2026-08-18). 도구를 만들거나 고치면 ①**정상 통과**(기존 입력에서 결과 불변=회귀) ②**실패**(잘못된 입력에 명시적 실패) ③**오판별**(구조가 다른 입력에 무의미한 출력을 내지 않는가)를 **모두** 시험한다.
  - ★**"도구가 무의미한 출력을 낼 때 그것을 알아채는가"는 ①②와 별개 능력이다.** 실사고(2026-08-18): `collect_gaps.ps1`에 M2_GEOM을 물렸더니 `concept_map`을 평면 구조로 오인해 **오류 없이 정상 종료**하며 "판정가능 개념 0 / 판정불가 5"를 출력했다(`shared_skeleton_fields`·`note` 같은 메타 키를 개념으로 셈). 실제 구조는 `concept_map.concepts`의 중첩 `{concept_id:{name,types[]}}`. 그대로 갔으면 **최종 제출 비교표에 잘못된 분모**가 실렸다.
  - 같은 계열 전례: `edit_json_meta.ps1` `$FFFD`/`$fffd` 대소문자 충돌 — 그때는 **실패 케이스만 시험**해 오탐을 못 봤다.
  - 조치 원칙: 도구가 입력을 **해석하지 못한 부분은 조용히 건너뛰지 말고 경고로 출력**할 것(집계 분모 과소계상 방지).
...
- **★★PS 한글 "리터럴 인코딩" 오염 — 2026-08-16 실사고(261건)**. 위 BOM 항목과 **다른 형태**라 별도 명문화.
  - **증상**: BOM 없는 `.ps1` 안에 한글 문자열을 **직접 리터럴로 적으면**(`$course = '중학수학 2'`) PS5.1이 파일을 ANSI로 읽어 **스크립트가 실행되기 전에 이미 깨진 문자열**이 된다. 스크립트는 오류 없이 정상 종료하고, **깨진 값이 데이터에 그대로 기록**된다. JSON 파싱도 통과한다(문법은 정상이므로).
  - **실사고**: 닮음 87 병합 시 `course` 리터럴 1개가 **261건 오염**(87 × `course`·`taxonomy_levels.course`·`visible_path[0]`). 병합 대상(87)보다 오염 범위가 컸다.
  - **발견 경로**: 타 단원 1883 해시 대조가 아니라 **표본 렌더 출력의 육안 확인**. 검증 절차가 없었으면 배포까지 갔다.
  - **회피법(3단)**: ① `.ps1`은 **ASCII 전용** — 한글은 단 한 글자도 넣지 말 것(주석 포함). ② 필요한 한글 값은 **데이터 파일에서 읽어 쓸 것**(예: 같은 파일의 정상 엔트리에서 `course` 값을 읽어 사용). ③ 스크립트에 넘길 한글 파라미터는 **UTF-8 JSON 패치 파일**로 전달.
  - **★★도구로 강제됨(검수 지시 2026-08-18)** — 위 규칙이 문서화돼 있는데도 한 세션에서 **2회 재발**(둘 다 신규 스크립트 작성 시)해, 검사를 **도구에 내장**했다. 모든 `.ps1`은 `param()` 직후 **ASCII 자기검사 블록**을 갖고, 자기 파일에 비ASCII 바이트가 1개라도 있으면 **실행을 거부**한다. 신규 도구는 `new_tool.ps1` 스캐폴더로 만들면 블록이 포함된 채 생성된다.
    - **★사각지대**: 파일 경로 없이(내용을 파이프로) 실행하면 `$MyInvocation.MyCommand.Path`가 null이라 검사가 건너뛰어진다. **반드시 `powershell -File <경로>`로 실행할 것.**
    - **설치 즉시 적발**: `edit_json_meta.ps1`에 기존 비ASCII 3바이트(주석의 `★`)가 있어 차단됨 → ASCII로 교체. 핵심 도구에 이미 위반이 있었는데 그동안 발견되지 않았다.
    - ★근거: 이 프로젝트에서 **사람의 주의력에 의존한 규칙은 전부 재발**했고(push 누락 3회·배치 ID 4회·한글 리터럴 2회), **도구로 강제한 규칙은 재발하지 않았다**(분모 자동 동반·`_meta` 편집기).
- **★PowerShell `,`가 `+`보다 결합력이 강하다 — 2026-08-18 실사고**. 배열 리터럴에서 `@( 'a' + $x, 'b' )`는 `'a' + ('b'...)`로 묶여 **조용히 중첩 배열/단일 문자열로 뭉개진다**. 실제로 스캐폴더가 **한 줄짜리 .ps1**을 생성했고, 1행이 `#`로 시작해 **스크립트 전체가 주석**이 됐다 — 실행은 되고 exit 0에 출력만 없었다(전형적 "조용한 실패"). 바이트 단위 ASCII 검사로는 못 잡는다.
  - **회피법**: 배열 리터럴에서 `+`를 쓰는 원소는 **반드시 괄호로 감쌀 것** `('a' + $x),`.
  - **★검증 교훈**: 생성기는 산출물의 **바이트가 아니라 동작을 검증**해야 한다. 현재 `new_tool.ps1`은 생성 후 ①비ASCII 0 ②줄 수 ③가드 블록 존재 ④**실제 실행 결과 문자열**을 확인하고, 하나라도 실패하면 생성물을 삭제한다.
  - **비ASCII 문자 전반**에 해당 — U+FFFD 같은 특수문자도 리터럴 금지, `[char]0xFFFD`로 쓸 것.
- **★대량 수정 후 U+FFFD 스캔 = 상시 검증 항목**(검수 지시 2026-08-16). 데이터 파일을 스크립트로 수정했으면 **반드시** 대체문자(U+FFFD) 수를 세고 0을 확인할 것. 인코딩 손상은 문법 오류를 내지 않으므로 파싱 통과만으로는 못 잡는다.
- **★★PS 비교 연산자도 대소문자 미구분 — 2026-08-21 실사고(검수 판정 22차 §1 등재 지시)**. 위 변수명 항목과 **같은 뿌리, 다른 표면**이다. `-contains` · `-eq` · `-match` · `-replace` 는 전부 **기본이 대소문자 무시**다. 대소문자로 의미가 갈리는 값을 비교하면 조용히 오탐이 난다.
  - **실사고**: `CHECK 9`(배열 멤버명 오염 검사)가 `$PS_MEMBERS -contains $el` 로 `'Count'`·`'Length'` 를 찾았는데, `response_formats` 의 **정상값** `"count"`(1건)·`"length"`(41건)와 미적분 `error_tags` 의 `"length"`(3건)가 매칭됐다. **오탐 45건**이 나왔고, 그대로 보고했으면 **멀쩡한 단원 2곳을 오염으로 판정**할 뻔했다.
  - **회피법**: 값 비교는 `-ccontains` · `-ceq` · `-cmatch` 를 쓸 것. 특히 **식별자·타입명·열거값**처럼 대소문자가 의미를 갖는 것은 예외 없이.
  - ★**검사 도구를 새로 만들면 "정답이 알려진 입력"으로 먼저 돌려 볼 것.** 이번엔 그렇게 해서 첫 실행에서 잡혔다. 검사 도구가 스스로 틀리는 유형은 발견이 가장 늦다.
- **★PS 변수명은 대소문자 미구분 — 2026-08-18 실사고**. `$FFFD`(문자)와 `$fffd`(개수)를 다른 변수로 알고 쓰면 **뒤 대입이 앞을 덮는다**. 실제로 edit_json_meta.ps1의 사후 게이트가 `[regex]::Escape(0)` → 숫자 `"0"` 개수(86)를 세어 **정상 편집을 계속 차단**했다(오탐). 검증 도구가 스스로 틀리는 유형이라 발견이 늦다 — ★도구를 만들면 **정상 통과 케이스와 실패 케이스를 둘 다** 시험할 것(이번엔 실패 케이스만 시험해 오탐을 못 봤다).
- **★_meta 구조 편집은 텍스트 치환 금지 — 2회 발생**(8/15 M2_GEOM `waver_pairs` 중괄호 소실 · 8/16 M2_SIMPY 이스케이프 안 된 큰따옴표). 우연이 아니므로 **도구로 강제**한다.
  - 수단 = **`tools/axis_prediction/edit_json_meta.ps1`**(ASCII 전용). `_meta` 구간만 중괄호 매칭으로 떼어 편집하고, **prescriptions 텍스트는 바이트 보존**. 편집 후 ① 전체 JSON 파싱 ② U+FFFD 스캔 ③ prescriptions 무변경을 **모두 통과해야 기록**하며, 실패 시 자동 복원한다. 한글은 `-PatchFile`(UTF-8 JSON)로만 전달.
  - ★전체 재직렬화(`ConvertTo-Json`)로 고치려는 유혹을 경계할 것 — 단일원소배열 언랩 버그로 `dropped:["x"]` 같은 필드가 스칼라로 바뀐다.
