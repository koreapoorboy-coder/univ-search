...
## ⚠ 공통규칙 포함 경위 + 오탐 provenance (검수 지적 반영)
- **10단원 전부 공통규칙(C-) 포함됨** — 이름-5는 baker가, PT-5는 러너 Get-Rules가 common+pack을 씀. (앞 라운드 "공통 보류"였는데 들어감.)
- **되돌리지 않고**(검수 지시: 출처 남기기) 각 item에 **`falsepos_rules` 필드** 추가 = 오탐 대장 4건(C-01·06·09·10) 발화 표시. meta에 `falsepos_fired` 카운트.
- **오탐 발화 대량**: QE C-01:68·C-10:31 · LE C-01:41 · probability C-10:31. **C-01→E1·C-10→E3 과잉공급.**
- ⚠ 검수 가설 정정: LF 61→82%가 C-01 탓이라 했으나 **LF엔 C-01 미발화**(C-06:7·C-10:1뿐). +21%는 정상 공통규칙(C-02~).
- 공통규칙 오탐 정밀화 = 45단원 스윕+AELF 재계산 → **A라인 안건**(`B_redzone_Aline_안건.md` §4).

## 연결 총계
- **연결 완료 10단원**(PT 5 + 이름 5): 🟢7·🟡3. 문항 축부여 ~2,281. (파일 = B-connect-v3, falsepos_rules 포함)
- **A라인 안건 4단원**: 856문항 유형 부재 → `B_redzone_Aline_안건.md`.
- 엔진 미연결(데이터만, §9). 태그(observed) 층 별개.
