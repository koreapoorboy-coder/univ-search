...
- **사용자 관리** = secret 1개 설정(대시보드) + 페이지 키입력 1회.
- ★**검수 확정: `/profile`(읽기)도 동일 키 요구.** 읽기를 열면 학생코드만 알면 남의 이력 노출 — 코드가 SC-STUDY-001 순번이라 추측 쉬움. 내부도구라도 학생데이터라 읽기도 막음. ⇒ **/record·/profile 둘 다 `X-Write-Key` 검증**(불일치 401).
...
- **제안**: **localStorage 우선 쓰기(항상 성공)** + `/record` POST. 실패 시 레코드 id를 **pending 큐**(localStorage `scstudy_sync_pending`)에 넣음.
  ...
  - ★**검수 추가**: (a) pending 레코드에 **first_failed_at** 기록 → **7일 이상 미동기면 경고색**(교사가 방치 인지). (b) **재시도 실패 사유 기록**(`last_error`: 401 인증실패 vs 네트워크 vs 5xx) → 사용자가 뭘 고칠지 앎(키 재입력? 서버?). profile에 사유 표시.
...
4. **최소 기간·사용량 (★검수 조정: "또는"→"둘 다")**: 이중쓰기 활성 후 **≥14일 그리고 실사용 진단 ≥10회**(둘 다 충족). 14일 지나도 진단 2건뿐이면 표본 없이 전환하는 것 방지 — 시간·사용량 동시.
...
## ✅ 검수 확정 (2026-08-10)
- 스키마·인덱스·created_at 승인.
- 인증 공유키+CORS 승인 + **`/profile`도 동일 키**(읽기 보호, 순번코드 추측 방지).
- 실패 = pending 재시도 큐 승인 + **7일 경과 경고색·실패사유(401/네트워크) 기록**.
- 전환기준 1·2·3 승인, **4번 "또는"→"둘 다"**(≥14일 AND 진단≥10회). 수동플립·가역·플립후 localStorage 백업 승인.
- **착수 순서**: 0단계(사이트 갱신+circle 배포검증) 선행 → 이후 코딩. D1 생성/바인딩/SQL/secret은 **사용자 첫 경험** → 착수 시점에 Worker 배포 안내 수준으로 화면 단계별 안내.
