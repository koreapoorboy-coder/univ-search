...
> **🆕 재태깅→관측축 파이프라인 진행 중** = `HANDOFF_B_RETAGGING.v1.md`(4단원 완료·다음 닮음). 관측 태그 정본 `TAG_DICTIONARY_v1.md`. 이 마스터는 (A)/(B) 큰 그림.
...
## 📋 A라인 안건 — `B_redzone_Aline_안건.md`
1. 🔴 856문항 유형추가+규칙신설 → **⛔ 보류(소스 부재 확정: 문항 본문 리포에 없음).**
2. D3 드리프트 7규칙 → **✅ 완료**(3 제거·4 D3→C4, "5개 C4"는 실측 3개 정정, 45단원 재검증 통과·AELF 353/96/76/20 불변·D3 17→10). ⚠ 연결파일 `B_connect_m3_statistics.v1.json` 재생성 필요.
3. 오탐 213부착 재판정 → **⛔ 보류(소스 부재 확정).** C-01은 리포가 "오탐 아님"이라 명시(등재≠오탐확정). provenance 표시 유지.
