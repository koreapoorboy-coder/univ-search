...
> 큰 그림 `tools/HANDOFF.md`·`PACK_HANDOFF.md`(A라인). 축 원문 `axis_definitions_v10_orig.md`(§11만).
...
- **남은 것 전부 외부 입력 대기**(§10·v11 원문 + A라인 회신). Code탭이 더 할 수 있는 건 거의 없음.
...
4. **관측층 정본** = `B_tag_axis_map.v1.json`(86태그 §11 레이블·오류절). 보류 5(A2/D2·B1/C1·②·축없음).
5. **17축 원문 §11**(v10판) = `axis_definitions_v10_orig.md`. E1=검산생략·A3=조건정리안함·D3=단순연산실수 등.

## ⛔ 막힌 것 + 왜
- **A2/D2·B1/C1 경계 미해소** → 보류 태그 3(146). §11은 명칭표라 안 갈림. **§10·v11 필요.**
...
- **§10·v11 원문** → **리포에 없음(전수 탐색 확인, 검수측 보유).** 단 §10 팩주도율은 axis_rules v44 팩 노트 9곳에 흩어져 부분 재구성 가능.

## ❌ 이 세션에서 뒤집힌 것 + 왜 (옛 문서 판단 폐기)
| 옛 판단 | 실제 | 왜 틀렸나 |
|---|---|---|
| 복원 조작정의(팩 why 기반) | 원문 §11로 교체 | A3·B3·C3·C4·**D1↔D3** 6축 어긋남. 해법절/오류절 혼동 |
| LI A3 = 팩 미예측 발견 | 철회→보류 | 오류절 재분류로 딜레마 소멸. 여섯 글자 레이블로 성급 판정 |
| 🔴 = 무거운 재분류 | 기계적 키 조인 | **필드 오류**(세부이름 매칭)+**팩 오류**(EX_LE→SE_EQ). 실제 PT-다리 75% |
| E1 = 유령축 의심 | 참 축 확정 | 원문 §11 E1=검산 + 관측 70% 확인 |
...
3. 오탐 213부착 정밀화(스윕+AELF) — 대장 v10판(10건), **v11(13건+형태③) 확보 후 재스윕**
4. §10·v11 확보 → A2/D2 경계 + 오탐 3건 추가
...
정본(현재)
  ...
  axis_definitions_v10_orig.md 17축 원문 §11
