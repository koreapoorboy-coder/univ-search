...
| **수연산 처방 414종** | 🔵 **진행 중 — 144/414 저작 완료** ← 현재 작업 |
...
**현재 144종**은 전부 `M2_NUMBER_EXPRESSION`. 이 단원 잔여 **67종**, 그 다음 `M3_REAL_NUMBER_CALC` 203종.
...
| 파일 | 역할 |
|---|---|
| `tools/axis_prediction/m2_ne_prescriptions.draft.v1.json` | **저작 정본**(_meta + prescriptions 144종) |
...
**gaps 누계**: 집계 44항목(원시 54건), 전부 B계열. **판정불가 개념 13개(35종 = 저작분의 24.3%)**.
...
| 시점 | 판정불가 | 비율 |
|---|---|---|
| 100종(배치6 직후) | 30종 | **30.0%** |
| 121종(배치7) | 35종 | 28.9% |
| **144종(배치8)** | 35종 | **24.3%** |

최종 제출의 판정불가율이 **배치 구성 편향을 반영하면 근거가 흔들리므로** 매 배치 이 비율을 확인하고 다음 배치 구성으로 조절한다.
★**잔여 67종에 overlay 0 범주가 몰려 있어 후반부에 다시 오른다.** 최종값은 **211종 완료 시점**으로 내고, 편향 관리 내역을 병기할 것.
...
|---|---|---|---|---|---|
| 수연산 M2_NE **144종**(현재 도구 실측) | 34% | 31 / 44 (갭가능 28) | 115 | 54 | **47.0%** |
...
★**판정가능(31) ≠ 갭가능(28)**: 관측 유형만 있고 예측층 유형이 없는 개념이 3개 있다(전건 관측). 판정은 가능하지만 판정할 대상이 없어 분모에 기여하지 않는다. **최종 제출 시 두 수를 함께 낼 것**(검수 지시).
...
| 지표 | 신설률 | **판정불가율** |
| 실측 | 100%→0 · 34%→0 · 19%→**8** (단조적이지 않음, 반증됨) | 100%→0% · 34%→**24.3%** · 0%→**100%** (거의 단조적) |

※ 수연산 판정불가율은 저작 진행에 따라 변한다(100종 30% · 121종 28.9% · **144종 24.3%**). 최종 제출에는 **211종 완료 시점 값**을 쓰고, 배치 구성 편향 관리 내역을 병기한다.
...
```
# 검증 (Expected = 누적 종수)
powershell -File tools\axis_prediction\validate_ne_prescriptions.ps1 -Expected 144
...
### (1) 수연산 배치9 — 20~24종
`m2_number_expression` 잔여 **67종**에서 **범주 단위로** 다음 묶음을 고른다. 후보(잔여 종수·overlay, 큰 범주 순):
