...
| **수연산 처방 414종** | 🔵 **진행 중 — 100/414 저작 완료** ← 현재 작업 · ★100종 중간 보고 지점 |
...
**현재 100종**은 전부 `M2_NUMBER_EXPRESSION`. 이 단원 잔여 **111종**, 그 다음 `M3_REAL_NUMBER_CALC` 203종.
...
| 파일 | 역할 |
|---|---|
| `tools/axis_prediction/m2_ne_prescriptions.draft.v1.json` | **저작 정본**(_meta + prescriptions 100종) |
...
**gaps 누계**: 집계 35항목(원시 45건), 전부 B계열. **판정불가 개념 11개(30종 = 저작분의 30%)**.
...
|---|---|---|---|---|---|
| 수연산 M2_NE 100종(현재 도구 실측) | 34% | 17 / 28 | 87 | 45 | **51.7%** |
...
```
# 검증 (Expected = 누적 종수)
powershell -File tools\axis_prediction\validate_ne_prescriptions.ps1 -Expected 100
...
### (1) 수연산 배치7 — 20~24종
`m2_number_expression` 잔여 **111종**에서 **범주 단위로** 다음 묶음을 고른다. 후보(잔여 종수·overlay, 큰 범주 순):
```
식의 값                        5종/ov2   소수점 아래 n번째 자리의 숫자 구하기  4종/ov2
순환소수를 분수로 나타내기(3)      4종/ov2   도형에서의 단항식의 계산(2) 입체도형   4종/ov1
B/(A×x)가 유한소수가 되도록 …    4종/ov2   지수법칙(5) 몫의 거듭제곱           3종/ov0
지수법칙(6) 종합                3종/ov2   지수법칙의 응용(2) 다른 문자로(1)   3종/ov2
지수법칙의 응용(4) 일의 자리수     3종/ov2   도형에서의 다항식의 계산(2) 입체도형  3종/ov2
```
★잔여 범주가 대부분 2~4종이라 배치7부터는 **6~8범주를 묶어야 20종**이 된다. 계열이 흩어지므로 배치 서사(finding)는 계열별로 나눠 기록할 것.
★**후보를 고를 때 overlay 보유 범주를 의도적으로 섞을 것.** 배치6에서 overlay 0 범주만 17종을 몰아 넣은 결과 판정불가가 13종→30종으로 뛰었다. 판정불가 비율이 계속 오르면 갭 수집 자체가 무의미해진다.
...
### (2) 100종 이후 — 백로그 ⑥ 설계 ← **지금 착수 대상**
`관측→태그 개선 절차` 설계. 5절 필수: 트리거 / 입력 / 판정 / 반영 / 호환.
★전제로 쓸 것: **"커버리지가 얇은 단원은 관측 기반 개선이 불가능하다"**(판정불가 **100종 중 30종 = 30%** 실측).
