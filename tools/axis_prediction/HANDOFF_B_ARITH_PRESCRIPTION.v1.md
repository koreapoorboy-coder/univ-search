...
| **수연산 처방 414종** | 🔵 **진행 중 — 166/414 저작 완료** ← 현재 작업 |
...
**현재 166종**은 전부 `M2_NUMBER_EXPRESSION`. 이 단원 잔여 **45종**, 그 다음 `M3_REAL_NUMBER_CALC` 203종.
...
| 파일 | 역할 |
|---|---|
| `tools/axis_prediction/m2_ne_prescriptions.draft.v1.json` | **저작 정본**(_meta + prescriptions 166종) |
...
**gaps 누계**: 집계 50항목(원시 62건), 전부 B계열. **판정불가 개념 17개(44종 = 저작분의 26.5%)**.
...
현재 26.5%는 이론값보다 낮고, 잔여 45종 중 **29종이 판정불가 개념 소속**이라 **34.6%로 수렴하며 오른다. 상승은 품질 저하가 아니라 편성의 산술적 귀결이다.**
...
|---|---|---|---|---|---|
| 수연산 M2_NE **166종**(현재 도구 실측) | 34% | 35 / 52 (갭가능 32) | 129 | 62 | **48.1%** |
...
★**판정가능(35) ≠ 갭가능(32)**: 관측 유형만 있고 예측층 유형이 없는 개념이 3개 있다(전건 관측). 판정은 가능하지만 판정할 대상이 없어 분모에 기여하지 않는다. **최종 제출 시 두 수를 함께 낼 것**(검수 지시).
...
```
# 검증 (Expected = 누적 종수)
powershell -File tools\axis_prediction\validate_ne_prescriptions.ps1 -Expected 166
...
### (1) 수연산 배치10 — 잔여 45종 마무리(2배치)
`m2_number_expression` 잔여 **45종**에서 **범주 단위로** 다음 묶음을 고른다. 후보(잔여 종수·overlay, 큰 범주 순):
