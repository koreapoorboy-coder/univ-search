...
- **⚠ 맵 위생(경계 재검토 중 발견)**: `M3CP.v1`(27)·`M3CP_circle6new`(28)은 **둘 다 `M3CP.v2`(55)의 완전 부분집합·축 불일치 0** → 정규 맵 = **12개**(v2 단독). 반영 축·풀 321은 무영향(union·축 일치). 단 tier **naive 집계가 중복 계수**됨. **정규 tier(dedup): confident 174·boundary 147**(앞 보고 223/154는 중복분). 경계쌍(정규): **C1↔C3=22**(C1>C3·13+C3>C1·9)·**C2↔C3=22**(12+10)·C4↔C1=11(9+2)·C4↔C3=9·C1↔C2=7·C4↔D1=6. **C계 상호경계가 압도** → 판례화 표적.
- **🔎 경계 재검토 국면 진행중(다음=이 작업)**: 검수 방식=경계쌍별 dossier(태그 axis/alt/note+대표문항 rationale) 전달→검수 판정→기준 도출→반대방향 자동정리 확인. **1단계 C1↔C3 dossier 생성**=`tools/axis_prediction/_boundary_review_C1_C3.md`(C1>C3 13 + C3>C1 9). 저증거 6종(1회등장·rationale공란) 표시. 순서: C1↔C3 → C2↔C3(22) → C4↔C1+C4↔C3 → C4↔D1+C1↔C2 → 잔여.
...
- **확정 판례/규칙**(§확정규율·§7 참조): 그래프↔식 3분(순수형태변환=C4/해석적연결=C3/원자개념=C2)·**§7 경계판례 4종**: ①**D2 vs D3**(대입자리=D2/계수계산=D3) ②**B1 vs C2**(문제에주어짐안씀=B1/알고꺼내야하는데못꺼냄=C2) ③D3/C2(공식알되계수틀림) ④**D2 vs C2**(★확률 등록: 이미 정해진 도구 중 고름=D2 예:law_selection / 상황을 어떻게 이해했느냐 갈림=C2 예:addition·multiplication_principle_selection. Q112 D2+C3 공존이 층위구분 실증)·converse류=C3(cyclicity B3 미승계)·**E1=문항구조 기준**(역방향 문항 유무, 단원 아님)·**기준명시**(관측 union vs 축매핑풀).
