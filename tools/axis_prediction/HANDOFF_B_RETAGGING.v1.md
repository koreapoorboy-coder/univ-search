...
> **🎯 최종목표 확정(검수, 2026-08-10)**: 프로젝트 최종목표=**태그 세분화**(정밀 진단). 현 321종=PDF 예측층. 목표=학생답안 관측층. 되먹임 고리 필수. **순서 확정**: ①Fix-A(풀이원문 저장)←완료 ②실사용 시작 ③item_bank 선택형 출제(조건부승인·후순위) ④문항등록 통로 ⑤D1 대시보드·세분화 화면. **★2번을 1번보다 먼저 하지 말 것**(지금 쌓는 데이터는 원문없으면 세분화 불가였음→Fix-A로 해소).
> **✅ Fix-A 완료(커밋 `574f7f19`)**: 조사=Worker가 시험지 읽고 태그만 방출·풀이전사 폐기 확정. 수정=Worker 2스키마(TYPE_ASSIGN·AI_EXTRACTION) 문항별에 `student_work_text·student_answer·tag_rationale`(optional, Anthropic structured output이 optional 허용→컴파일실패0·오답에서만 비용) + stage-2 프롬프트(WRONG/PARTIAL만 원문·정답·근거, 개인정보 미전사) + store `_normAttempt` 화이트리스트+길이캡(600/200/400) + D1 attempts=JSON라 마이그레이션 불요. 전달경로 spread 통과·엔진 회귀0 확인. **⏸ 재배포 후 검증**: 오답 진단→profile/D1에서 3필드 채워짐(정답·빈칸은 빈값=정상).
> **다음 실작업 후보**: (a)**비용 실측→절감판단**(`B_cost_measure_guide`: 재배포후 진단 몇회→`_runtime.usage`+Anthropic Console. effort/모델은 실측後만. **Fix-A 전사로 오답당 출력토큰 소폭↑ 실측 포함**) / (b)**item_bank 선택형 출제**(Fix-A 이후·조건부승인: 개념태그 links조인 커버리지·문항중복회피 확인) / (c)**고등 재태깅**(현황조사부터, 아래 🎯) / (d)중등 Tier2 큐레이션 / (e)전환기준 충족시 서버 플립. **파일 넘길 때 검수용은 반드시 Downloads 복사+SendUserFile**(메모리 `file-transfer-workflow`).
...
- **✅ (b) 확대 배선 완료·배포대기**(2026-08-09, `B_wiring_overlay_coverage_report`·`B_wiring_expansion_deploy`): **9단원 오버레이 생성**(레시피 reflection⋈links, join_fail=0, QF재생성=기존일치). **정정: "19단원"=워크시트 착시, 실제 11 distinct 단원**(QF완료·circle보류→깨끗이 9). index.v1.json 10엔트리에 `fine_error_tags_overlay` 등록(polynomial=M3_POLYNOMIAL_FACTORING·QF=기존qf_). **Worker index-구동화**(하드코딩맵 제거→index서 읽기, VERSION `2026.08.09-fine-overlay-index-driven`) — **이후 단원추가=데이터만, 재배포 0회**. 검증=`overlay_tester.html`(배포본·index구동·전단원 드롭다운·similarity/geometry 프리셋). **⏸ 사용자 배포 1회(wrangler)+사이트갱신1회 후 양극단 검증**(similarity100%·geometry19%). 커버리지 19~100%(저커버=fine 적게 나와도 정상, 검수기록). **circle+statistics=PT파일 부재 별건**(`ISSUE_missing_problem_types_files`, git이력전무=애초미생성, 조용히 유형진단 격하, 전수스캔=2건뿐). **fine층 상한=중등 11단원**(고등=재태깅 선행).
- **✅ (b) 배포본 확대 검증 통과**(2026-08-09): 배포된 GitHub Pages에서 overlay_tester로 similarity(fine4종)·geometry(fine3종) 확인. index 구동 실동작. `right_triangle_altitude_geometric_mean_failure`(경계재검토 C1→C3 이동)까지 실사용 반영 관찰. **9단원 확대 완료 확정.**
- **⚠ 배선 함의(fine≠rules, 2026-08-10 측정)**: fine 태그 어휘와 `diagnosis_rules` 트리거 어휘가 **겹치지 않음(측정 0.0%)**. ⇒ **fine 태그로는 규칙이 발화하지 않는다. 지금 열린 건 축(observed_axes) 경로뿐.** "fine 넣었으니 규칙도 돌 것"은 오해 — fine으로 규칙을 돌리려면 **어휘를 잇는 별도 작업** 필요. 근거 `B_measure_pt_error_tags_inflation`·`ISSUE_pt_error_tags_attribution`(종결).
- **✅ circle/statistics 복구 완료**(`ISSUE_missing_problem_types_files`): 처음 "stub"이라 오판(사용자가 type_name 실재 지적→정정). 실제는 **type_variant_bank에 M3_CIRC_PT###47/M3_STAT_PT###23 + 실 한글 type_name 완비**(problem_types 조립만 안 됨). **조립 복구**(`B_wiring_assemble_pt.ps1`): problem_types 생성(type_name_source=variant_bank·id=links 완전일치·concept_ids 비움) + circle 오버레이 재생성 + edges/rules/remediation 최소빈파일6(엔진 _loadUnits 4파일요구) + index fine_error_tags_overlay·build_status제거. 엔진실측·배포본 실측 통과(missing_type_count=0·circle fine→축·실 type_name). **조용한실패 종료**(전수스캔=이2건뿐). 남은것=concept정의·큐레이션 진단명(별도트랙). 가시화(build_status·Worker type_load_failures)는 dormant로 남음.
- **✅ B 이관(A→D1) 코딩 완료**(`B_wiring_kvd1_migration_design`·`B_wiring_d1_deploy_guide`, 검수확정): Worker `/api/axis-store/record·profile`(둘 다 X-Write-Key 검증·D1 AXIS_DB, VERSION `2026.08.10-axis-store-d1`)·D1 스키마SQL(JSON컬럼·amv별도·인덱스3)·store 이중쓰기(localStorage소스+서버POST·실패시 pending큐 first_failed_at/last_error·retry·7일aged)·profile 서버설정/동기화/서버우선조회. mock 실측 통과. 진단페이지는 shared localStorage로 자동 이중쓰기. 전환기준=레코드≥20일치·최근10회 pending0·profile 3명·**≥14일 AND 진단≥10회**(수동플립·가역).
- **✅ circle/statistics concept Tier1 빌드 완료**(2026-08-10, 방식B 반자동: Code탭 초안→검수 감수→빌드. `B_concept_draft_circle/statistics`·`B_build_*_concepts.ps1`): 개념명은 **links의 concept_id↔PT type_name 조인으로 유도**(PDF/검수 불요, source="pt_derived"). **circle 31 + statistics 23 concept** → math_concepts.v1.json(4092→4146) + 각 problem_types의 `pt.concept_ids` 채움(47/47, 23/23). 엔진실측 bare_id 0·실 한글 개념명·통합반영(circle C025-027·C029-030 동일명 id유지). Tier2(description·diagnostic_signals·remediation_keywords)는 진단 불요라 후순위.
- **⚠ future_connection_ids 기록**: statistics **C023·024·026** = 같은 단원(M3_STAT) 뒤쪽·심화 개념인데 set03 문항이 앞으로 연결만 걸어둔 것(primary 아님). circle 미참조 gap과 본질 동일. **통계 다른 워크시트 set 도입 시 primary 평가될 수 있음 → 재검토 대상.**
- **📍 중등 완결 상태(2026-08-10)**: 재태깅 11단원=유형·**축**·개념 / circle=유형·축(오버레이)·개념 / statistics=유형·개념(재태깅 없어 **축 없음=정상**). 조용한실패 단원 0(전수). **중등 canonical 층 완결.** 남은 중등 잔여=Tier2 큐레이션(진단 description 등, 선택).
- **✅ 검증문항 구조·API비용 조사 + usage로깅·review-hardening 코딩**(2026-08-10, `B_investigation_verification_and_cost`·`B_cost_measure_guide`): **검증 10문항=AI생성**(files:[], 진단 개념/단원 근거) — 문제집 추가는 검증풀 무의미(풀 없음). 품질레버=concept Tier2/재태깅. **API비용 실측: 모델opus-4-8·effort high·캐싱없음·analyze=N+2호출·usage미기록**이었음 → **usage 로깅 추가**(callClaudeJson 호출별 토큰→`_runtime.usage.per_call`+Worker Logs`[usage]`) + **review-hardening(structured 우선/폴백, 회귀불가)** 번들. VERSION `2026.08.10-usage-and-review-hardening`. effort/모델 변경은 **실측 후만**(진단품질이 목적).
- **다음 세션 즉시 할 일**: ①②③④·3·4·라이브·확대·circle복구·B이관코딩·팽창재측정·concept빌드·**usage로깅·review-hardening** 완료·push. **⏸ 사용자 Worker 재배포 1회**(누적: axis-store+GET+**usage로깅+review-hardening** 전부, VERSION `2026.08.10-usage-and-review-hardening`) → 진단 몇 회 → `_runtime.usage`·Anthropic Console(`console.anthropic.com>Usage`)로 **비용 실측**(가이드 `B_cost_measure_guide`). **미완/대기**: (a)**비용 실측→절감판단**(effort medium 시범 대조/review생략/캐싱/모델, 실측 후) / (b)**고등 재태깅**(다음 국면, 착수조건 아래) / (c)중등 Tier2 큐레이션(선택) / (d)전환기준 충족 시 서버 플립.
- **📋 중등 완성 [C] 실사용 전 필요(갱신)**: **비용 구조 확정**(실측→effort/캐싱 판단) 추가. ~~검증문항용 문항 추가~~ 제거(생성형이라 무효 확인).
- **🎯 고등 재태깅 착수 조건 제안(2026-08-10)**: 중등 완결됐으니 다음 국면=고등. **성격이 완전히 다름**(5,161문항·연결층 edges≈0). ⇒ 바로 재태깅 말고 **① 현황 조사 먼저**(circle/stat 때처럼): 고등 각 단원에 어느 층이 있나 — type_variant_bank? links? problem_types? concepts? edges? **② 회생 vs 신규 판정**(중등처럼 조립만 하면 되는지, 아예 없는지) → **③ 연결층(edges/prereq) 부재 대응 방침**(고등은 0이라 개념 선행/보강 진단이 안 됨 — 유형·축·개념까지만 가능한지, edges를 별도 구축할지) → **④ 규모 분할**(5,161문항 단원별 순차, 중등처럼 1단원씩 검수 왕복). **먼저 검수와 ①현황조사부터 합의 후 착수** 권고.
- 검증서버=scratchpad `serve.ps1`(8731·ASCII). ↓ 아래는 이전 국면 이력(참고).
