...
- **다음 할 일**: ① 확보 PDF 순차 반영 — 다항식(5)·도형성질(6)(856) + 이차방정식(5)·실수(6)·확률(5)(주제형). ② 예측 error_tags = M2_SIMPY 예측층 정비 시 재검토(별도). ③ D2 후속(타단원 대조)·관측층 병합(§미결) = 남은 PDF 반영 후.
...
- **단원별 편중 = 진단축 변별력 실증** (검수확정 사례 누적): ①수와식 D편중 vs 원 C편중. ②**일차함수 B2·B3 등장(매개변수 범위·경우) vs 이차함수 C3 지배(그래프↔식 연결)** — 같은 함수 계열인데 오류 성격이 갈림(2번째 변별 사례). 닮음 C4·C1편중, 삼각비 광폭. → 축 체계가 단원 특성을 변별함이 반복 실증.
