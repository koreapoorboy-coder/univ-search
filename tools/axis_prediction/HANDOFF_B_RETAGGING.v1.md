...
- **관측 반영 19단원 완료**: 수와식5·원12+6·삼각비6·닮음3+8·삼각비무번호(set01)+10(set11)·함수5+8·이차함수3+9·다항식5+13·도형성질6·이차방정식5+9·실수(6)set07·**확률(5)set06**... **856 트랜치 종료**(닮음300+이차함수300+다항식293+도형성질150).
...
- **🔴 남은 것 = 주제형 1 (완비 유닛, 856/등재 불요, 반영만)**:
  | 단원 | code | set→item_id | source_file |
  |---|---|---|---|
  | 확률(10) | m2_probability | set11→`M2_PROB_150_S11` | `260711_경우의 수와 확률(10).pdf` |
  - ⚠ **확률(10) 기대치(검수)**: 확률(5)와 **같은 단원 → 재사용률 90%+ 예상**(닮음8·삼각비 100% 선례처럼 사전 포화 가능). 신규 소수. **중등 재태깅 마지막 파일**.
