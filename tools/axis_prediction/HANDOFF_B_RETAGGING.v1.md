...
- **다음 세션 즉시 할 일**: ①②③④·3·4·라이브·**(b)확대 배선** 완료·push. **⏸ 사용자 배포1회+사이트갱신 → overlay_tester로 similarity·geometry 검증.** 이후 후보: **(d) 5단계 팽창 재측정**(②로 완화되는지·`ISSUE_pt_error_tags_attribution`) / **(e) B(KV/D1) 이관 설계**(누적저장소 A→B) / **(f) review-hardening**(`ISSUE_analyze_review_json_structural`) / circle·statistics PT복구(별건) / 고등 재태깅. 검증서버=scratchpad `serve.ps1`(8731·ASCII). ↓ 아래는 이전 국면 이력(참고).
