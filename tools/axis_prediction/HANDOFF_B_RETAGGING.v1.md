...
- **✅ circle/statistics = stub 단원 확정 + 가시화 처리**(`ISSUE_missing_problem_types_files`): (A)복구 착수 중 발견 — PT뿐 아니라 **개념(M3_CIRC_C/M3_STAT_C)도 DB에 0**·규칙도 없음 = links만 자동생성된 미구축 stub. (A-thin)은 "조용한실패→껍데기진단"이라 기각. **(가시화만) 채택·구현**: index.v1.json `build_status:"unbuilt"`(데이터구동) → index/hybrid 범위선택 **진단 前 경고**("준비 중 단원: 원의 성질,통계", 실측통과) + Worker `_staged.type_load_failures` 명시(VERSION `2026.08.09-unbuilt-detect`, 다음배포와 함께). 정식빌드는 재태깅/큐레이션 버킷(circle은 오버레이 있어 빌드시 즉시 fine편입).
- **다음 세션 즉시 할 일**: ①②③④·3·4·라이브·확대·가시화 완료·push. **⏸ 사용자: (1) 9단원+가시화+unbuilt-detect 배포1회+사이트갱신**(가시화는 데이터, Worker는 unbuilt-detect 포함) → **(2) 다음 실작업=B 이관**(`B_wiring_kvd1_migration_assessment`, **D1 권장**·이중쓰기 롤백·재배포1회, **검수 판단 대기**: D1확정?·JSON컬럼vs정규화?·이중쓰기OK?). 이후: (d)팽창재측정 / (f)review-hardening / circle·stat 정식빌드+고등 재태깅(큐레이션 버킷). 검증서버=scratchpad `serve.ps1`(8731·ASCII). ↓ 아래는 이전 국면 이력(참고).
