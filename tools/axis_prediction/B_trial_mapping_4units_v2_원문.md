# 오답형 4단원 86태그 → 17축 재매핑 v2 (축 레이블 기준 · 출처 "§11" 미확인)

> ⚠ 파일명의 "원문"은 축 정의 **내용**을 가리킴(확보). 출처 절 표기 "§11"은 미확인(원본 인계문 리포 밖).
기준: 축 레이블(내용 확보, 출처 절 "§11" 미확인) · **오류절**(해법절 아님) · 문항 stages·first_action_hint(태그 뜻의 둘째 발).
표기: `구축(v1 복원)` → **신축(원문)**. 미해소 경계(A2/D2·B1/C1)·② 항목은 **보류**(축 정의 출처 미확인·경계 판례 부재), 억지 배정 금지.
...
### 보류 (억지 배정 안 함 — 축 정의 출처 미확인·경계 판례 부재)
- **A2/D2 경계(★승계② ≈146):** mean_formula_application(42)·squared_deviation_or_denominator(65)·representative_value_selection(50, C3/D2) — 공식 "암기 vs 선택·대입"이 축 레이블로 안 갈림.
