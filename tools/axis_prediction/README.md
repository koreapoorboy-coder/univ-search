...
| `axis_rules.v4~v1.json` | 이전 버전. 기록용 보관 — 스크립트는 **v6** 를 읽는다 (v5는 검수 채팅 내부 번호, 리포엔 없음) |
...
`axis_rules.v6.json` 은 스크립트와 같은 폴더에 둔다. 출력 시트: `predicted_axes`
...
- **다음 팩(규칙 v7)**: pack_gap 3위 ATF 326(삼각함수 — 일반각·호도법/그래프/사인·코사인법칙). 그다음 CM2GE 276·M1D 231.
