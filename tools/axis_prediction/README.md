...
| 파일 | 내용 |
|---|---|
| `axis_rules.v7.json` | 규칙 199개 (공통 15 C-* + 단원팩 184 D-*: EL_AELF 9·PS 10·GE 11·CA2_M2D 23·MM_CMM 15·PF_M1S1 16·EI_CMEI 38·SQ_ASQ 30·TF_ATF 32) |
| `axis_rules.v4~v1.json` | 이전 버전. 기록용 보관 — 스크립트는 **v7** 을 읽는다 (v5는 검수 채팅 내부 번호, 리포엔 없음) |
...
`axis_rules.v7.json` 은 스크립트와 같은 폴더에 둔다. 출력 시트: `predicted_axes`
...
- **다음 팩(규칙 v8)**: pack_gap 상위 CM2GE 276(도형의 방정식, 이름단독 매칭 약함)·M1D 231·CM2SP 230·FN 216.
