...
| `axis_rules.v7~v1.json` | 이전 버전. 기록용 보관 — 스크립트는 **v8** 을 읽는다 (v5는 검수 채팅 내부 번호, 리포엔 없음) |
...
텍스트 삽입으로 기존 6팩을 바이트 보존하고, 삽입 후 common+각 팩을 v(n)과 대조 검증한다.
...
`axis_rules.v8.json` 은 스크립트와 같은 폴더에 둔다. 출력 시트: `predicted_axes`
...
- **다음 팩(규칙 v9)**: pack_gap 상위 M1D 231·CM2SP 230·FN 216·BG 193·M2I 181.
