...
1. **다음 팩 큐**: **연립방정식(중2, M2SL 143)** 우선 — pack_gap 크기보다 **「활용문제 공용팩」 판단**이 먼저(검수 채팅 권고, §8-2 아래). 이후 QE 140 → RC 137 → PF_M3S1 132 → … (QF·LE 완료). **재료 뽑을 때 코드(그룹ID) 열 복원 + 형식 고정**(검수 채팅 지적: 이번 LE tsv가 이전 GS/TR/QF와 열·인용·구분자가 달라 §8-1 스크립트가 매번 손봐야 함).
...
```
검수 채팅이 **연립방정식(M2SL) 팩 블록**을 주면 §1 절차로 v24 반영. **핵심 판단**: D-LE-63·77(거리·속력·시간·농도)과 같은 why가 또 나오면 §4 「활용 공용팩」을 §3 공용키로 신설할지 결정(신규팩 or 공용키는 블록의 `applies_to`를 그대로 따름). **먼저 M2SL 재료 tsv를 코드열 복원·형식 고정해 뽑아 발송**(§1 step 8).
