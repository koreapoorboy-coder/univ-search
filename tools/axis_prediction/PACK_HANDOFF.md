...
> 규칙 **v22** · 스크립트 v5(포팅 v22) · 팩 **21개 / 26단원-행 커버**. HEAD `8879628a`.
> 큰 그림은 `tools/HANDOFF.md`, 서브시스템 상세는 `tools/axis_prediction/README.md`(v3~v22 라운드 이력 전부).
> **순서 확정(사용자 지시)**: (A) 팩 생산 라인 §0~5 를 **먼저 끝낸 뒤** → (B) 채점·판정 로직 §6-B 로 넘어간다.
> 지금 할 일 = **(A) 계속. 다음은 LE 문자와식 165**(순수 신규, pack_gap 최대). 검수 채팅이 LE 팩 블록 주면 v23로 반영.
> **QF 완료(v21 QD_QF 61, LF_FN 58% 기각)** + **v22 D-FN-16 오탐 수정**(§3 예외, QF 검수에서 발견). LE는 이름이 짧을 가능성 높아 tsv에 묶음 이름 온전 여부 확인 필요(§7-3).
...
## 5. 현재 팩 로스터 (21개, v22 · 규칙 754=공통15+단원팩739)
...
전체(45단원, 46 unit-row): 미매칭 **1104** · pack_gap **2641** · 팩보유 **26/46**.

**지배축 표(누적, (B) 채점로직용)**: CM2FG C4 45 · BG C2 52 · M2I D2 72 · CP C3 75 · GS D2 54 · **TR 없음**(균등) · **QF 없음**(C4 44.9%이나 팩주도 37.1%=첫 유형변별). CM2FG·TR·QF 균등단원 = 정확도검증 대조군. 팩마다 갱신.

## 6. 열린 항목 (우선순위)

1. **다음 팩 큐(pack_gap 순, 팩 없는 20단원)**: **LE 문자와식 165**(순수 신규, 재료 발송 필요) → M2SL 143 → QE 140 → RC 137 → PF_M3S1 132 → … (QF 완료)
...
(CM2FG C4 45%·BG C2 52%·M2I D2 72%·CP C3 75%·GS D2 54%·TR/QF 지배축 없음, 팩마다 갱신 중). QF는 C4 44.9%가 지배축은 아니나 **팩주도(공통기여 3.4%)라 첫 유형변별 단원** — 대조군 균등단원(CM2FG·TR·QF)의 정확도검증 가치. 채점 초안의 축별 판정 `confirmed/exposed`는
...
tools\mathflat_builder\Run-AxisPrediction.ps1 -Only QF     # 205·100%·100%·gap0·이름단독95 (v21 최신팩)
tools\mathflat_builder\Run-AxisPrediction.ps1 -Only FN     # 267·100%·100%·gap0·미매칭1 (v22 D-FN-16 수정 불변)
```
검수 채팅이 **LE 문자와식 팩 블록**을 주면 §1 절차로 v23 반영. LE는 재사용 여지 거의 없어 순수 신규 예상 — 블록의 `applies_to`(`["LE"]`)를 그대로 따를 것. **먼저 LE pack_gap tsv 재료를 뽑아 발송**해야 검수 채팅이 설계 착수(§1 step 8).
