...
> 규칙 **v23** · 스크립트 v5(포팅 v23) · 팩 **22개 / 27단원-행 커버**. HEAD `2d8c69ea`.
> 큰 그림은 `tools/HANDOFF.md`, 서브시스템 상세는 `tools/axis_prediction/README.md`(v3~v23 라운드 이력 전부).
> **순서 확정(사용자 지시)**: (A) 팩 생산 라인 §0~5 를 **먼저 끝낸 뒤** → (B) 채점·판정 로직 §6-B 로 넘어간다.
> 지금 할 일 = **(A) 계속. 다음은 연립방정식(중2, M2SL)** — pack_gap 크기보다 **「활용문제 공용팩」 판단**이 먼저(검수 채팅 권고). D-LE-63·77과 같은 why가 또 나오면 §3 공용키 결정. 블록 주면 v24로 반영.
> **LE 완료(v23 EX_LE 91, LF_FN 25.2% 기각, D1 첫 공급원)**. **재료 tsv 형식 고정 + 코드(그룹ID) 열 복원 필요**(§1 step 8, 검수 채팅 지적).
...
- **⭐ 활용문제 공용팩 후보(신규 유형)**: D-LE-63(거리·속력·시간)·D-LE-77(농도)의 why가 **FN에서도 일치**. 지금까지 재사용 후보는 전부 *교과내용* 공유였으나 이건 유일하게 ***문제상황*** 공유. 이번엔 신규팩이 100% 덮어 안 넓혔으나(각 단원 맥락으로 다시 씀), **연립방정식(중2)에서 세 번째로 같은 why 확인되면 「활용 공용팩」을 §3 공용키로 신설 검토**. 다음 단원을 M2SL로 잡는 이유.

## 5. 현재 팩 로스터 (22개, v23 · 규칙 845=공통15+단원팩830)
...
전체(45단원, 46 unit-row): 미매칭 **1023** · pack_gap **2476** · 팩보유 **27/46**.

**지배축 표(누적, (B) 채점로직용)**: CM2FG C4 45 · BG C2 52 · M2I D2 72 · CP C3 75 · GS D2 54 · **TR/QF/LE 지배축 없음**(QF C4 44.9% 팩주도 · LE C1 41.5%·D1 28% 팩단독). 균등단원 4개(CM2FG·TR·QF·LE) = 정확도검증 대조군. **D1 첫 공급원=LE**(공통규칙 D1 부여 0). 팩마다 갱신.

## 6. 열린 항목 (우선순위)

1. **다음 팩 큐**: **연립방정식(중2, prefix `EQ` = `m2_linear_equation`, pack_gap 124)** 우선 — pack_gap 크기보다 **「활용문제 공용팩」 판단**이 먼저(검수 채팅 권고, §4 참조). 재료 발송 완료(`EQ_pack_gap.tsv`/`연립방정식_pack_gap.tsv`, 명세 §17-9 형식). 이후 QE(이차방정식 중3, C-11 90%) → RC → PF_M3S1 → … (QF·LE 완료).
   - **⚠ prefix 매핑(검수 채팅 라벨 ≠ unit_code)**: 검수 채팅의 "M2SL/QE" 등은 내부 라벨. 실제 실행기 prefix는 **unit_code**다. 연립방정식=**EQ**, 이차방정식=**QE**(추정), 함수(일차)=FN, 방정식은 EQ 하나에 일차·연립·활용 중영역이 다 들어있음(EQ C-11 63% 과다는 §6-3 그 EQ).
   - **재료는 이제 `Export-PackGapMaterial.ps1 -Only <unit_code>`로 고정형식 방출**(명세 §17-9: 그룹ID·코드·중영역·묶음·이름·적용규칙·분류 / 무공백 쉼표 / BOM 없음 / 무인용). 라운드마다 형식 흔들리던 문제 해결(검수 채팅 지적).
...
5. **기존 팩 짧은 한자어 토큰 오탐 점검**: ~~`D-FN-16 그래프의 평행`(⊂그래프의 평행이동)~~ **v22 수정 완료**(§3 예외, `그래프의 평행(?!이동)`). 남은 것: `D-GE-08 중심`(⊂무게중심)·`D-BG-36 대각`(⊂대각선) — 앞의 두 건은 팩 미개봉이라 무해했으나 GEV/GES/GEC·BG **자체** 단원에서 발화하면 D-FN-16처럼 살아있는 오탐. 각 단원 자체 실행으로 점검(GS엔 무영향). **정본 Python D-FN-16 도 검수 채팅이 `(?!이동)` 로 미러해야 FN 교차검증 일치.**
...
(CM2FG C4 45%·BG C2 52%·M2I D2 72%·CP C3 75%·GS D2 54%·TR/QF/LE 지배축 없음, 팩마다 갱신 중). QF는 C4 44.9%가 팩주도라 첫 유형변별 단원, **LE는 D1 28%가 전부 팩단독(공통 D1 부여 0)=§10-1 D1 첫 공급원**(대표함정: 괄호앞 −부분적용·분모 최소공배수 정수항 미적용·이항 부호유지). 균등단원 4개(CM2FG·TR·QF·LE)=정확도검증 대조군. 채점 초안의 축별 판정 `confirmed/exposed`는
...
tools\mathflat_builder\Run-AxisPrediction.ps1 -Only LE     # 246·100%·100%·gap0·이름단독89 (v23 최신팩)
tools\mathflat_builder\Run-AxisPrediction.ps1 -Only FN     # 267·100%·100%·gap0·미매칭1 (v22 D-FN-16 수정 불변)
```
검수 채팅이 **연립방정식(prefix EQ) 팩 블록**을 주면 §1 절차로 v24 반영(재료 발송 완료). **핵심 판단**: D-LE-63·77(거리·속력·시간·농도)과 같은 why가 또 나오면 §4 「활용 공용팩」을 §3 공용키로 신설할지 결정(신규팩 or 공용키는 블록의 `applies_to`를 그대로 따름). 재료 재생성은 `Export-PackGapMaterial.ps1 -Only EQ`.
