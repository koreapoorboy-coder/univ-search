...
> 규칙 **v24** · 스크립트 v5(포팅 v24) · 팩 **23개 / 45단원-행 중 28 커버**. HEAD `c978f8a4`.
...
전체(**45단원 = 45 unit-row**, 실측 확정): 미매칭 **991** · pack_gap **2352** · 팩보유 **28/45**.
...
1. **다음 팩 큐**: **이차방정식(중3, prefix `QE` 확정 = `m3_quadratic_equation`, pack_gap 140)** — **활용 공용팩 4번째 확인 지점**(§4). 재료 발송 완료(`이차방정식_pack_gap.tsv`, §17-9). 이후 RC → PF_M3S1 → … (QF·LE·EQ 완료).
   - **QE↔QF 충돌 없음(양방향 확인)**: unit_code **QE(이차방정식 M3S1) ≠ QF(이차함수 M3S1)**. 팩키 QD_QF는 QF 전용, QE는 신규 팩키. QE는 어느 applies_to에도 없음.
   - **AP_USE 4차 확인 재료 2종 발송**: ① QE 활용(이차방정식의 활용 47행) ② **D-CMEI-29 CMEI 자체 발화 58건**(`D-CMEI-29_CMEI_발화목록.tsv`) — 이차방정식·이차함수·삼차사차·연립·부등식 활용 전반, 상황(도형넓이·원가정가·속력·농도·과부족) 풍부. 이 둘로 AP_USE 상황목록을 이 라운드에서 닫을 수 있음.
   - **prefix 매핑**: 검수 채팅 라벨(M2SL/QE 등)≠unit_code. 확정: 연립방정식=EQ, 이차방정식=QE, 이차함수=QF, 함수(일차)=FN. 재료는 `Export-PackGapMaterial.ps1 -Only <unit_code>`(§17-9 고정형식, commit 82c85c0b).
   - **규칙 전달 = 델타 방식(검수 채팅 합의)**: 다음 라운드부터 axis_rules 전체 대신 **새/수정 팩 블록만** 전달 → 검수 채팅이 작업본에 합쳐 체크섬 대조. 전송계층 문제·전사 사각지대 회피.
...
3. **공통 60% 상한(팩 규칙)**: QE C-11 90%·**EQ C-11 63.5%·C-13 51.3%(실측 확정)** 등(묶음 이름이 주제어). AELF·CMM에서 억제 규칙 0이라 미구현 — 도입 시 QE/EQ부터. EQ 실측: C-11 억제→E1 70.5%→B3 60.9%, C-11+C-13 억제→C1 37.8% 실질축 노출(둘 다 미매칭 0 유지). **단 C-13(`연립`)은 EQ 전용 토큰이라 C-11과 따로 다룰 것**. **⚠ 새 증거(§16-E 연동)**: 공통규칙이 주제어를 물면 오염이 축 분포에서 멈추지 않고 **메타 지표(이름단독)까지 번진다** — EQ C-11·C-13이 이름 57행을 물어 설계 품질 지표를 부풀렸음. 억제 규칙을 「축 분포만 보정」으로 잡으면 이 층을 놓침.
...
8. **이름단독 정의 = 팩 D-규칙만(정본 확정·러너 수정 완료 v24)**: §16-E 「측정 도구도 틀린다」 사례. 러너 line 89 D-* 한정, AELF 자체검증 92→60. **남은 것: 커밋된 과거 값 재계산 범위 판정**(검수 채팅). delta 실측(`이름단독_두정의_delta.csv`, 28팩 중 27개 차이≠0): 큰 것부터 EL 63→7 · GES 92→37 · ATF 86→35 · CMEI 81→31 · ASQ 74→31 · M2D 82→43 · CP 93→55 · **EQ 78→42 · LE 89→75 · AELF 92→60 · TR 91→62 · QF 95→81 · FN 99→86**. 인계문/README의 이름단독 값은 전부 왼쪽(공통+팩). **정본 Python 도 이름단독을 팩D-만으로 바꿔야 AELF 60 교차검증 일치**(현재 Python이 어느 정의인지 검수 채팅 확인 필요).
...
tools\mathflat_builder\Run-AxisPrediction.ps1 -Only AELF   # 353·96·76·20·이름단독60·요약1 (앵커, 이름단독 팩D-만 v24)
tools\mathflat_builder\Run-AxisPrediction.ps1 -Only EQ     # 156·100%·100%·gap0·이름단독42·C-11 63% (v24 최신팩)
tools\mathflat_builder\Run-AxisPrediction.ps1 -Only FN     # 267·100%·100%·gap0·미매칭1 (v22 D-FN-16 수정 불변)
```
검수 채팅이 **이차방정식(prefix QE) 블록**을 주면 §1 절차로 v25 반영(델타 방식: 블록만 받아 작업본 v24에 합침). **핵심 판단**: 활용 공용팩 **4번째 확인**(왕복·농도·도형 등, QE 활용 47행 + D-CMEI-29 58건 참조) 나오면 §4대로 `AP_USE` 신설 + 3팩(LF_FN·EX_LE·SE_EQ) 동시 §3수정을 한 묶음으로 결정. **미해결: 이름단독 과거값 재계산 범위(§6-8)·정본 Python 이름단독 정의 확인**.
