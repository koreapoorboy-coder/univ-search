...
- `axis_rules.v19.json`: 규칙 630개(공통 15 + 단원팩 615: …·CR_CP 49·SM_GS 74). IN1_M1I 는 M1I+M2I 공용, CA2_M2D 는 M2D+M1D 공용.
  v1~v18 은 기록용 보관(v5는 검수 채팅 내부번호). 스크립트는 v19 를 읽음. **커밋됨.**
...
- **지배축 표(누적)**: CM2FG C4 45% · BG C2 52% · M2I D2 72% · CP C3 75% · GS D2 54%. (B) 채점로직에서 절대건수/P3로 판정.
- **신규 과제**: 짧은 한자어 토큰 오탐(D-GE-08 중심⊂무게중심·D-BG-36 대각⊂대각선) 기존 팩 자체발화 점검. C-15 GS 51% 60%상한 감시.
- **대기**: 다음 팩 pack_gap 순 잔여(LE·QF·TR 등). TR 삼각비는 SM_GS 재사용 후보(닮음↔삼각비). PSS/PSC 세분화, 공통 60% 상한 열림.
