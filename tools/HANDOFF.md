...
- `axis_rules.v4.json`: 규칙 99개(공통 15 C-* + 단원팩 84 D-*: EL_AELF 9·PS 10·GE 11·CA2_M2D 23·MM_CMM 15·PF_M1S1 16).
  v1~v3 은 기록용 보관. 스크립트는 v4 를 읽음. **커밋됨.**
...
- **완료(v4)**: 소인수분해 팩 PF_M1S1 16개. 재실행 PF_M1S1 149·적중100%·팩89%·gap11%·과다없음(예측 완전 일치). PF 팩키 분리 정상.
- **대기**: 검수 채팅이 CMEI(499, 중영역 12개 — 이차부등식111·복소수82·연립일차부등식61…) 중영역 단위로 팩 초안 → 규칙 v5.
  그다음 ASQ 350·ATF 326. PSS/PSC 세분화, 공통 60% 상한 판단은 여전히 열림.
