...
- `axis_rules.v18.json`: 규칙 556개(공통 15 + 단원팩 541: …·FG_CM2FG 60·CR_CP 49). IN1_M1I 는 M1I+M2I 공용, CA2_M2D 는 M2D+M1D 공용.
  v1~v17 은 기록용 보관(v5는 검수 채팅 내부번호). 스크립트는 v18 을 읽음. **커밋됨.**
...
- **지배축 표(누적, (B) 채점로직에 반영)**: CM2FG C4 45% · BG C2 52% · M2I D2 72% · CP C3 75%. 지배축은 `confirmed/exposed` rate로 과소진단됨 → 절대건수/증명문항(P3)로 판정. 팩마다 지배축 회신에 남겨 45단원 표 완성.
- **대기**: 다음 팩 pack_gap 순 잔여(GS 도형닮음·LE 문자와식·QF 이차함수 등, 재사용후보 BF_BG·LF_FN·GE2·CR_CP). PSS/PSC 세분화, 공통 60% 상한 열림.
