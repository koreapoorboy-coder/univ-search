...
- `axis_rules.v20.json`: 규칙 693개(공통 15 + 단원팩 678: …·SM_GS 74·TB_TR 63). IN1_M1I 는 M1I+M2I 공용, CA2_M2D 는 M2D+M1D 공용.
  v1~v19 는 기록용 보관(v5는 검수 채팅 내부번호). 스크립트는 v20 을 읽음. **커밋됨.**
...
- **완료(v16·v17)**: BF_BG·IN1_M1I note 정정(재사용 커버율 오보 수정) + FG_CM2FG 60(함수와그래프). CM2FG 284/적중284/팩100%/gap0/미매칭0. 축분포 최고 균등(C4 45%).
  전체 미매칭 →1269·pack_gap →3284. 팩 **22/46**.
- **⚠ 선측정 버그(정정됨)**: 재사용 커버율을 파이프라인 `?{$_.name}`으로 재면 거짓 0% — **직접순회 `foreach($r in $rows)`로만** 잴 것(README 미결에 상세). 커밋 결과·정본 실행기는 무관.
- **팩 재사용 2례**: CA2_M2D(M2D→M1D 51%, 공용확장) · IN1_M1I(M1I→M2I 69%, 공용키). why 검증: BG←GE 36%·M1I←CA2_M2D 35%는 걸리나 why 외지/중복이라 안 넓힘.
- **완료(v18)**: CR_CP 49(원의 성질). CP 212/적중212/팩100%/gap0/미매칭0. 재사용 진짜 0%(직접순회). 전체 미매칭 →1237·pack_gap →3104. 팩 23/46.
- **완료(v19)**: SM_GS 74(도형의 닮음·피타고라스). GS 239/적중239/팩100%/gap0/미매칭0. 전체 미매칭 →1177·pack_gap →2925. 팩 24/46.
- **완료(v20)**: TB_TR 63(삼각비). TR 152/적중152/팩100%/gap0/미매칭0/이름단독91%. TF_ATF 74%는 D-ATF-32 단일규칙 부풀림이라 기각(중3 삼각함수 미학습). 전체 미매칭 →1164·pack_gap →2786. 팩 25/46. C-15 66%(공통규칙 60%초과 첫 실측).
- **⚠ 선측정 거짓0% 근본원인 확정**: `$R` 살아있을 때 `foreach($r in ...)`가 `$R`을 덮음(대소문자무시). **`$r`·`$p` 금지, `$row`·`$rule`·`$pk`만**(README 상세). 커밋 결과·실행기 무관.
- **지배축 표(누적)**: CM2FG C4 45 · BG C2 52 · M2I D2 72 · CP C3 75 · GS D2 54 · **TR 없음(균등, CM2FG와 대조군)**. §8-2 「커버÷발화규칙수」 지표는 채택/기각 안 갈려 기각(M1D 16.3>GS←BF_BG 5.8). C-15 억제는 스크립트 몫·팩라인 후 (B)와 도입.
- **신규 과제**: 짧은 한자어 토큰 오탐(D-GE-08 중심⊂무게중심·D-BG-36 대각⊂대각선) 기존 팩 자체발화 점검. C-15 GS 51% 60%상한 감시.
- **대기**: 다음 팩 QF 이차함수(LF_FN·EI_CMEI 공용 여지) 권장→LE·M2SL. PSS/PSC 세분화, 공통 60% 상한 열림.
