...
- `axis_rules.v7.json`: 규칙 199개(공통 15 C-* + 단원팩 184 D-*: …·EI_CMEI 38·SQ_ASQ 30·TF_ATF 32).
  v1~v4 는 기록용 보관(v5는 검수 채팅 내부번호). 스크립트는 v7 을 읽음. **커밋됨.**
...
- **완료(v7)**: TF_ATF 32. ATF 385/적중384/팩99%/gap1%/과다없음(예측 일치, 최고 커버리지). 불변식 5개 유지.
  전체 미매칭 →1664·pack_gap →4969. 팩 보유 14/46 unit-row. 모두 채팅 본문 패치로 반영(파일전송 불안정).
- **대기**: 다음 팩 CM2GE(276, 도형의 방정식·이름단독 약함)→규칙 v8. 그다음 M1D 231·CM2SP 230·FN 216. PSS/PSC 세분화, 공통 60% 상한 열림.
