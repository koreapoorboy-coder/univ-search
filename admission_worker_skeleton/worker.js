  ...
  async fetch(request, env, ctx) {
    ...
    const seedMatch = matchSeed(input, seedPack);
        ...
        if (input.reportStage === STAGE.DRAFT) {
          try {
            ...
            bookChoices = matchBooks(bookList, {
              subject: input.subject, concept: reportConcept,
              keyword: input.selectedKeyword || input.keyword, axisTitle: reportAxis?.title,
        ...
        if (input.reportStage !== STAGE.DRAFT && source === 'openai') {
          try {
            // **이 보고서가 선 개념의 축**을 쓴다. 같은 과목이어도 다른 단원의 축이 잡히기 때문이다 —
            // 커피 추출 보고서에 '화학량론 해석 축'이 잡혀 "몰비 계산, 반응식 계수 해석"을 하라고 나왔다.
            // (지구과학 보고서에 국어 축이 붙어 "매체 해석 메모"가 나왔던 것의 같은 과목 판이다.)
            const axis = reportAxis;
            const bookList = seedPack.bookMatchIndex?.books || [];
...
function buildPrompt(input, seedMatch, env) {
  ...
  const prompt = [
    ...
    // 운영 테스트 24(고2 효소 실험): 이론적 배경 한 문단을 미카엘리스-멘텐 식(Vmax, Km)에 썼다 — 실험 데이터로는 한 번도 쓰지 않는 식이다.
    '한 단계 위 수준은 **이번 데이터를 해석하는 깊이**로 보여 준다. 이번 실험·자료로 직접 계산하거나 확인하지 않는 대학 과정의 식과 기호(예: 미카엘리스-멘텐 식, Vmax, Km, 미분방정식)는 이론적 배경에 넣지 않는다. 필요하면 결론의 후속 탐구에서 이름과 방향만 한 번 말한다.',
    ...
    // 운영 검사 2026-09-22: 표지에는 「서사·극 갈래와 이야기 구성」이라고 찍혀 나가는데 본문은
    // 「문학·독서와 주체적 수용 단원에서 배운」이라고 썼다(통합사회 고령화 보고서에서도 같은 일).
    // 학생이 그대로 내면 표지와 본문이 서로 다른 말을 한다.
    ...(input.selectedConcept
      ? [`본문에서 단원 이름을 말할 때는 「${input.selectedConcept}」 하나만 쓴다. 다른 단원 이름을 끌어오지 않는다.`]
      : []),
    // 운영 검사 2026-09-22(국어 서평·영어 기사 비평): 글로 쓰는 과제인데 보고서가 「코더 2인이 20% 표본을
    // 중복 코딩해 일치도를 확인한다」, 「본 코딩 뒤 1주 지나 재코딩한다」 같은 연구자용 절차를 시켰다.
    // 고등학생이 혼자 못 하는 일을 시키면, 학생은 그 절에서 멈추거나 안 한 일을 했다고 적게 된다.
    '탐구 절차는 **학생이 혼자, 학교와 집에서, 며칠 안에 할 수 있는 크기**로 쓴다. 다른 사람을 연구 인력으로 동원하거나(평정자 2인, 코더 간 일치도, 동료 교차 검증), 몇 주에 걸친 반복 일정(1주 뒤 재코딩), 연구자용 표집 설계(무작위 층화 표집, 사전 등록)를 절차로 요구하지 않는다.',
    '자료가 적거나 한 번만 재서 생기는 문제는 절차를 늘려 해결하지 말고 「한계」 절에 그대로 적는다.',
    ...
    ...(input.readWork
      ? [`학생이 읽은 작품은 「${input.readWork}」이다. 보고서는 이 작품을 다루며, 다른 작품으로 바꾸거나 다른 작품을 나란히 놓지 않는다.`,
         ...
         '작품을 부를 때는 학생이 적은 제목을 고쳐 부르거나 원제·영문 제목으로 바꾸지 않는다.',
         // 2026-09-25 테스트: 「동백꽃 (김유정)에서 서로 호감이 있으면서도…」처럼 괄호를 문장마다 끌고 다녔다.
         '학생이 지은이를 괄호로 함께 적었으면 본문에서는 **제목만** 쓴다. 지은이는 처음 한 번만 밝히고, 그 뒤로는 괄호를 문장 안에 끌고 다니지 않는다.']
      : []),
    ...
    ...(textbookUnitOf(input.textbookCitation)
      ? [`- 교과서 단원을 이름으로 가리킬 때는 「${textbookUnitOf(input.textbookCitation)}」 단원으로 쓴다(참고 자료에 이 단원이 적힌다). 다른 단원 이름을 붙이지 않는다.`,
        // 운영 테스트 24: 「고등학교 생명과학과 통합과학의 「물질대사와 에너지」 단원」 — 통합과학에는 그런 단원이 없다.
        `- 이 단원은 ${input.subject || '이 과목'}의 단원이다. 다른 과목에도 같은 단원이 있다고 쓰지 않는다(예: "생명과학과 통합과학의 ~ 단원"). 다른 과목과의 연결은 단원 이름 없이 개념으로만 말한다.`]
      : []),
