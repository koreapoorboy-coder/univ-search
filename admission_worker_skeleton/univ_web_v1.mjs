...
export function pickUnivWeb(list, text, { limit = 3, skip = '', strict = false, subject = '' } = {}) {
  // 참고 자료(strict)는 과제 낱말 둘이 제목에 함께 있어야 한다(univ_research_v1 의 need).
  return pickForTask(Array.isArray(list) ? list : [], contentWords(text, subject), limit, { skip, strict, need: strict ? 2 : 1 });
}
