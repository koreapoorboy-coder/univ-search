...
const TITLE_EXAMPLES = {
  [COLLECTION.MEASUREMENT]: [
    ...
    '  좋음: "물 온도에 따른 효소 세제의 얼룩 제거 효과 비교"',
    '  좋음: "빗면의 기울기에 따른 수레의 가속도 측정과 역학적 에너지 분석"',
  ],
  [COLLECTION.SURVEY]: [
...
export function titleRules(kind = COLLECTION.MEASUREMENT, stage = STAGE.COMPLETE) {
  ...
  return [
    ...
    '- 한 구절 안에 두 가지가 보여야 한다: ① 무엇을 — 구체적인 대상과 교과 개념(예: 효소 세제의 얼룩 제거, 수레의 가속도, 고령화 속도) ② 어떻게 — 측정·분석·비교·평가·설계처럼 탐구한 방식이나 관점(예: ~에 따른, ~를 이용한, ~로 본).',
    '- 교과 개념어를 하나 이상 넣는다. 그래야 주제로 읽힌다.',
    '- 길이는 공백 포함 15~40자, 하나의 명사구다. 끝을 ~측정, ~분석, ~비교, ~탐구, ~평가, ~설계로 맺는다.',
