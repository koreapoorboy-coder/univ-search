...
const UNIT_SYMBOLS = {
  ...
  // 「도 섭씨」도 같은 말이다 — 운영 검사 2026-09-22(지구시스템과학 기온): 표 머리글이
  // 「연평균 기온 5년 평균 (도 섭씨)」로 나왔다. 몰 매리터·와트 매 제곱미터와 같은 갈래다.
  도섭씨: '°C', 섭씨온도: '°C', 켈빈: 'K', 켈빈온도: 'K',
  // 운영 검사 2026-09-23(데이터 과학 미세먼지): 표 머리글이 「초미세먼지 월평균 농도
  // (마이크로그램 매 세제곱미터)」로 나왔다. 「마이크로그램」이 표에 없어서 「매」 규칙이 안 걸렸다.
  // 농도·미세한 양은 학교 자료에 흔하다.
  마이크로그램: 'μg', 나노그램: 'ng', 마이크로미터: 'μm', 나노미터: 'nm', 마이크로리터: 'μL',
  피피엠: 'ppm', 피피비: 'ppb', 킬로파스칼: 'kPa', 헥토파스칼: 'hPa', 밀리몰: 'mmol',
};
...
// 책 **한 권이 과제의 대상**인가. 「서평」·「독후감」·「도서를 한 권 골라 읽고」처럼 책 자체를
// 다루라는 과제다. 「참고 도서를 활용해도 좋다」처럼 곁들이는 말과는 다르다.
// 화면(decision_flow_v1 의 BOOK_REQUIRED_RE)도 같은 말을 본다 — 화면은 책을 고르게 하고,
// 여기서는 보고서가 그 책을 다루게 한다.
const BOOK_IS_SUBJECT = /서평|독후|독서\s*감상|책을\s*읽고|도서를\s*읽고|도서를?\s*(한\s*권|선정|골라)|책을?\s*(한\s*권|선정|골라)/;
export function bookIsSubject(input) {
  return BOOK_IS_SUBJECT.test([input?.taskDescription, input?.taskName].filter(Boolean).join(' '));
}

export function bookRules(book, asSubject = false) {
  if (!book) return [];
  // 책이 과제의 대상이면 규칙이 뒤집힌다 — 아래의 「책을 주제로 만들지 않는다」는 과학 보고서가
  // 책을 근거로 한 줄 인용할 때의 규칙이다. 서평에 그 규칙을 쓰면 책이 사라진다
  // (운영 검사 2026-09-22: 서평 과제에서 책 대신 신문 칼럼을 분석한 보고서가 두 번 나왔다).
  if (asSubject) {
    return [
      `- 이 과제의 대상은 책 「${book.title}」${bookAuthor(book) ? ` (${bookAuthor(book)})` : ''} 한 권이다. 보고서 전체가 이 책을 다룬다.`,
      '- 신문 칼럼·기사·통계 같은 **다른 소재로 바꾸지 않는다.** 책 대신 다른 글을 분석하면 학생은 과제를 안 한 것이 된다.',
      '- 아래 [참고 도서]에 적힌 것과 학생이 적은 것만 근거로 쓴다. 줄거리·인용·장면·인물을 지어내지 않는다.',
...
export function stagePromptLines(stage, input) {
  ...
  if (stage === STAGE.DRAFT) {
    return [
      ...
      // 설계서 26장을 읽히니 「자료가 없는데 연구 질문에 답을 냈다」가 두 장에서 [높음] 으로 걸렸다(2026-09-25).
      '- **설계서는 연구 질문에 답하지 않는다.** 아직 잰 값이 없다. "연구 질문에 대한 답은 다음과 같다"처럼 쓰지 말고, 답은 최종 보고서에서 자료를 보고 쓴다. 여기서는 무엇을 어떻게 재서 답하겠다는 계획과, 어떤 결과가 나오면 어떻게 읽겠다는 예상까지만 쓴다.',
      ...
      ...((input.recentCombinations || []).length
        ...
        : []),
      '- referenceInputs는 결과와 견줄 **기준값**을 학생이 적을 칸이다(식초 병에 표시된 산도, 이론값, 공식 기록값처럼 실험 밖에서 옮겨 적는 숫자). 안내문이 그런 값과 비교하라고 할 때만 label과 unit으로 1~3개 넣고, 아니면 빈 배열이다. 표의 조건으로 넣지 않는다. **conditions에 이미 들어 있는 값(조건 이름에 적힌 거리·농도·질량)은 referenceInputs로 또 만들지 않는다.** 안내문이 견주라고 한 값(도면에 적힌 높이, 병에 표시된 값, 이론값)이 있으면 반드시 use="비교" 칸으로 만든다. 칸마다 use를 정한다 — 식에 넣는 값(눈높이·실온·비열·구간 길이)은 "계산", 결과와 견주어 오차를 볼 값(표시된 산도, 이론값, 공식 기록)은 "비교"다. **탐구 방법이나 계산에 필요한 값(실온, 처음 값, 기준선, 표시값)은 반드시 여기에 칸을 만든다** — 표에도 없고 이 칸에도 없는 값은 학생이 적을 곳이 없어 그 계산과 문장이 통째로 사라진다.',
      ...
      '- 안내문이 표의 칸을 정해 주었으면(네 칸에 인원수를 적는다, 조건 다섯 개를 잰다) **그대로 그 칸만** 만든다. 안내문에 없는 변인(통학 시간, 성별, 학년)을 새로 끼워 넣어 표를 늘리지 않는다 — 학생이 모으지 않은 자료를 요구하게 된다.',
      '- 한 대상에서 **두 가지 값**을 읽어야 하면(별의 겉보기 등급과 절대 등급, 관측소의 P파와 S파 도달 시각, 반응 전과 후의 질량) conditions를 「대상 · 값 이름」으로 나눠 두 줄로 만든다. 그래야 한 칸에 값 하나라는 규칙을 지키면서 두 값을 모두 적을 수 있다. 한 값을 적을 곳이 없으면 학생은 계산을 할 수 없다.',
