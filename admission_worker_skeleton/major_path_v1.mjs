...
export function axisPromptLines(axes, path) {
  ...
  return [
    '',
    '[후속 탐구 — 결론 마지막 문단에서 이 탐구가 어디까지 뻗는가]',
    ...(why ? [`- ${why} 그러니 전공 이야기로 끌고 가지 않는다.`] : []),
    '- 결론의 마지막 문단(후속 탐구)은 학생이 이번 탐구에서 쓴 교과 역량이 어느 방향으로 더 깊어지는지를 쓰는 자리다. 진로를 맞히는 자리가 아니다.',
    ...(top.length
      ? [
...
export function curriculumPathPromptLines(path) {
  ...
  return [
    '',
    `[후속 탐구 — ${path.major}에서 실제로 이어지는 과목, 결론 마지막 문단에서]`,
    ...
    '- 이 문단은 "그 학과에 가고 싶다"는 말을 쓰는 자리가 아니라, 이번에 쓴 교과 역량이 그 과목에서 어떻게 이어지는지를 쓰는 자리다.',
