...
//
// 엔진 전수 검사(2026-09-18)에서 나온 조각도 뺀다. 이것들이 서울대 글을 고를 때 '과제 낱말'로 쓰여
// 「적으로」가 「유기적으로 움직이는 입자」 기사에 걸렸다.
//   · 꾸밈말       창의적인 · 종합적인 · 정량적으로
//   · 동사 조각    예측할 · 나타낼 · 있으며 · 참여했 · 활용하였   ('역할'은 명사다)
//   · 조사 조각    으로 · 에서 · 책에서 · 내에서            (한 글자 + 조사 — '사과'·'나이'는 그대로)
const FRAGMENT = (word) => /적(인|으로)$/.test(word)
  || (/할$/.test(word) && word.length >= 3 && word !== '역할')
  || /(했|하였|되었|였|있으며|낼)$/.test(word)
  || /^[가-힣]?(에서|으로)$/.test(word);
export const isVerb = (word) => VERB_TAIL.test(String(word || '')) || /^[가-힣]{2,}하$/.test(String(word || ''))
  || FRAGMENT(String(word || ''));
...
// 과목 이름도 뺀다. 「생명과학」이 남으면 「농업**생명과학**대학 … 반추위 메탄」 기사에 걸린다
// (운영 테스트 2026-09-18, 방형구 과제). 글자만 같고 뜻은 과제와 아무 상관이 없다.
export function contentWords(text, subject = '') {
  return paperQuery(text, '', { subject }).words.join(' ');
}
