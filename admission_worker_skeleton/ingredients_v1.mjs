// 참고 연구 재료 — 보고서는 **조합**이다. 몸통은 교과 개념 + 학생 데이터이고, 실제 연구(논문·대학 연구 소개,
// 나중에 시사)는 학생의 설명을 받쳐 주는 **배경 지식**으로 녹아든다. 출처는 **참고 문헌에만** 적는다.
//
// 사용자 결정(2026-09-18, 세 번째):
//   · 본문에 "○○대학교 ○○학부 연구팀은…", "(저자, 연도)"처럼 이름을 드러내지 않는다 — 그것만 보이고 학생의 데이터와
//     탐구가 가려진다. 참고한 곳은 참고 문헌으로 들어간다
//   · 「교과 심화와 확장」 같은 따로 된 절도 없다 — 학교 보고서 양식에는 없어 학생이 쓸 수 없다. 이론적 배경의 개념 설명이나
//     결론의 마지막 문단(후속 탐구)에 자연스럽게 녹인다
//   · 대학 연구를 넣는 까닭은 그대로다: 대학이 연구하는 방향과 이어진 탐구 — 다만 이름이 아니라 **내용이 이어지게**
// 재료는 설계서(주제 잡기)에는 안 가고, 보고서를 쓰는 단계(최종·한 번에 끝나는 보고서)에만 간다.
//
// 흐름:
//   1. 이 보고서 단원에 이어진 논문(GPT 가 단원 꼬리표를 붙여 둔 것)과 대학 연구 소개를 **후보**로 고른다
//   2. AI에게 제목·키워드만 보낸다. AI는 1~2개만 배경 지식으로 쓰고, 관계없는 후보는 버린다(비교 시험에서 잘 버렸다)
//   3. AI가 **실제로 쓴 번호만**(usedIngredients) 참고 문헌이 된다
//
// **조합이 다양해야 한다.** 가장 가까운 둘은 늘 넣고 나머지는 무작위 — 같은 과제라도 학생마다 넓히는 방향이 달라진다.
...
export function ingredientPromptLines(ingredients) {
  ...
  return [
    ...
    '- 재료는 학생의 설명을 받쳐 주는 **배경 지식**으로 녹인다. 쓸 자리는 둘이다: 이론적 배경에서 교과 개념이 실제로 어디에 쓰이는지 한두 문장(예: 산화·환원 → 이차전지 양극재, 전자·원자의 충돌 → 반도체 공정), 결론 마지막 문단(후속 탐구)에서 이번 결과가 어느 방향으로 더 깊어질 수 있는지 한두 문장. 따로 절을 만들지 않는다.',
    '- 본문에 대학 이름·학부·연구팀·저자 이름·연도를 쓰지 않는다. "○○대학교 연구팀은", "(저자, 연도)", "관련 연구를 찾아보니 ~한 연구가 있었다"처럼 연구를 앞세우는 문장을 쓰지 않는다. 연구가 보이면 학생의 데이터와 탐구가 가려진다. 출처는 참고 문헌에 자동으로 들어간다.',
    '- 재료는 **제목으로 알 수 있는 주제까지만** 쓴다. 그 연구의 결과·수치·결론, 읽고 알게 된 내용처럼 쓰지 않는다. 일반적인 사실("~ 분야에서는 ~를 다룬다") 수준으로 쓴다.',
    '- recordDraft ③(참고한 자료)에는 재료로 넓힌 방향을 이름 없이 한 문장으로 쓴다. 예: "관련 연구 자료를 찾아 군집 조사 방법이 결과 해석에 주는 영향으로 탐구를 확장함".',
    // 운영 테스트(2026-09-18): 세특 초안에 「~라는 점을 확인함」이 나왔다 — 학생은 제목만 보았는데 읽은 것처럼 보인다.
    '- 재료에 대해서는 "확인함", "확인했다", "알게 됨", "알게 되었다", "밝혀졌다"처럼 연구 내용을 읽고 안 것처럼 쓰지 않는다. 학생은 제목만 보았다. "관련 연구 자료를 찾아봄", "~ 방향으로 탐구를 확장함", "~로 이어질 수 있음을 생각함"처럼 쓴다. 본문과 recordDraft 모두 같다.',
    // 운영 테스트 21(물리 단진자): 느낀 점에 「관련 연구 자료를 찾아보며 …」가 나왔다 — 학생이 쓰지 않은 행동이다.
    '- 느낀 점에는 재료를 쓰지 않는다. 느낀 점은 학생 자신의 글이다.',
    // 같은 테스트: 진자 실험 이론적 배경에 드론 고도 제어(PID) 논문을 끌어와 한 문단을 썼다 — 억지 연결이다.
    '- 재료가 이번 실험·조사의 교과 개념과 **직접** 닿을 때만 쓴다. 한두 단계를 건너야 이어지는 재료(예: 진자 주기 실험에 드론 고도 제어)는 쓰지 않고 usedIngredients에서 뺀다. 알맞은 재료가 하나도 없으면 usedIngredients는 빈 배열이다 — 억지로 쓰는 것보다 안 쓰는 것이 낫다.',
  ...
  return {
    ...
    papers: items.map((one) => ({
      ...
      guide: one.kind === 'paper'
        ? 'KCI(kci.go.kr)에서 제목으로 찾아 초록을 읽어 보세요. 발표나 면접에서 "더 찾아본 것이 있어?"라고 물으면 말할 거리가 돼요.'
