...
## 2026-04-28 업데이트 — 교과 개념·추천 키워드·후속 연계축 1차 잠금

### 0. 이번 업데이트 결론
2026-04-28 기준으로 현재 업로드된 교과에 대한 **3번 교과 개념·키워드 선택 + 4번 교과 개념 기반 후속 연계축 선택 로직은 1차 완료**로 잠금 처리한다.

이번 잠금은 아래 흐름이 실제 화면 캡처 검수를 통과했다는 의미다.

1. 과목 선택
2. 학과 선택
3. 학과별 교과 개념 추천 3개 정렬
4. 교과 개념 선택
5. 오른쪽 추천 키워드 노출
6. 추천 키워드 선택
7. 선택 키워드에 따라 4번 후속 연계축 1순위와 보조축 분화
8. 학과에 따라 4번 라벨·연결 과목·설명·활동 예시 보정

### 1. 이번 업데이트에서 잠금된 추가 교과
이번 세션에서 신규 또는 후속으로 검수·잠금한 교과는 아래다.
...
기존에 사용자가 이미 검수한 교과는 기존 완료 상태를 유지한다.
...
### 2. 전체 잠금 원칙
앞으로 5번 도서, 6번 보고서 방식, 7번 관점, 8번 보고서 라인, 최종 MINI 생성은 반드시 아래 payload를 상속해야 한다.

```json
{
  "selectedSubject": "과목명",
  "selectedMajor": "실제 등록 학과명",
  "selectedConcept": "3번에서 선택한 교과 개념",
  "selectedKeyword": "3번 오른쪽에서 선택한 추천 키워드",
  "selectedFollowupAxis": "4번의 1순위 또는 사용자가 선택한 후속 연계축",
  "axisLabel": "직접 연계 강함 / 역량 브리지 / 일반 탐구",
  "linkedSubjects": ["연결 과목", "후속 과목"],
  "activityExample": "4번 카드의 활동 예시",
  "longitudinalPath": "다음 학년·후속 탐구 확장 방향"
}
```

이 payload를 누락하면 엔진이 요구하는 형태의 보고서가 나오지 않는다.

### 3. 세포와 물질대사 정책 기준 반영
세포와 물질대사에는 아래 정책 기준을 데이터에 반영했다. 이 정책은 이후 모든 교과와 5번 이후 단계에도 준용한다.
...
- 간호학과는 물질대사·건강·항상성 방향으로 정상화
- 약학과는 효소·수용체·약물 작용 방향으로 정상화
...
#### 역학과 에너지
- 기계공학과/자동차공학과/항공우주공학과는 시공간과 운동 우선
- 건축공학과/토목공학과는 탄성파와 소리 우선
- 컴퓨터공학과는 운동 데이터·시뮬레이션 연결 방향 유지
...
#### 물질과 에너지
- 환경공학과 + 기체 조성 → 대기 조성·오염 분석 축
- 에너지공학과 + 압력 → 기체 상태·열역학 해석 축
- 기계공학과 + 압력 → 기체 상태·열역학 해석 축
- 약학과 + 수소 결합 → 약물 물성·용해도 해석 축
- 식품공학과 + 끓는점 → 식품 물성·가공 응용 축

#### 세포와 물질대사
- 세포막 → 세포 시스템·물질 이동 해석 축
- 능동 수송 → 막수송·농도 기울기 해석 축
- 막단백질 → 세포 신호·수용체 응용 축
- 모델링 → 세포 데이터·투과성 실험 축

#### 지구시스템과학
- 대기·해양 순환과 기후 + 자료/데이터 키워드 → 기후 데이터 예측·시각화 축
- 대기·해양 순환과 기후 + 단열 변화/대기 안정도/편서풍/밀도류 → 대기·해양 자료 해석 축
- 판 구조와 지구 내부 + 지진/화산/지진파/내진/지반 → 재난·지구물리 응용 축
- 지구 탄생과 시스템 진화 + 태양계 성운/원시 지구/행성 형성 → 행성·환경 진화 해석 축

### 5. 실제 등록 학과 기준 사용 규칙 추가
이후 검수와 안내는 실제 등록 학과명만 사용한다. 예를 들어 `지구과학과`, `재난안전공학과`, `지질학과`, `에너지자원공학과`처럼 현재 엔진에 없는 이름을 기준으로 검수하지 않는다.

확인 대상 학과는 반드시 `public/keyword-engine/seed/major-engine/major_catalog_198.json`을 기준으로 한다.

### 6. 다음 작업 우선순위
이제 3번·4번 로직은 잠금 상태로 보고, 다음 작업은 아래 순서로 진행한다.

1. 5번 도서 연결 로직
2. 6번 보고서 전개 방식 연결
3. 7번 보고서 관점 연결
4. 8번 보고서 라인 연결
5. 최종 MINI payload 생성

### 7. 다음 작업의 핵심 주의
다음 단계는 새 로직을 별도로 만드는 것이 아니라, 기존 3번·4번 결과를 상속해서 확장해야 한다.

특히 5번 도서 연결은 반드시 아래 데이터를 사용해야 한다.
...
이 조건이 빠지면 학생에게 나가는 보고서가 엔진의 의도와 다르게 일반 추천형으로 흐를 가능성이 높다.
