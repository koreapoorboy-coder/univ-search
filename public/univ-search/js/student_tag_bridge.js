...
// integrated engine용 taggedStudent를 만드는 브리지 스타터 코드
...
function pickStudentFromRecordDetail(data, studentId) {
  ...
  return arr.find(s => norm(s.student_id || s.id) === norm(studentId)) || null;
}
...
function collectRawRows(rawData, studentId) {
  const root = rawData?.[studentId] || rawData?.raw_record || {};
...
function inferTagsFromText(text) {
  ...
  addIfIncludes(text, ["AI", "인공지능", "프로그래밍", "코딩", "정보"], themeTags, "AI/SW");
  addIfIncludes(text, ["신소재", "소재", "재료"], themeTags, "신소재");
  addIfIncludes(text, ["윤리", "생명윤리", "토론", "찬반"], themeTags, "윤리");

  // method
  addIfIncludes(text, ["실험", "관찰", "배양", "측정"], methodTags, "실험");
  addIfIncludes(text, ["자료", "조사", "논문", "기사"], methodTags, "자료조사");
  addIfIncludes(text, ["데이터", "그래프", "회귀", "시각화"], methodTags, "데이터분석");
  addIfIncludes(text, ["모델링", "함수", "그래프를", "개형"], methodTags, "모델링");
  addIfIncludes(text, ["제작", "설계", "구현", "장치"], methodTags, "제작");
  ...
  addIfIncludes(text, ["AI", "인공지능", "프로그래밍", "코딩", "정보"], majorTags, "AI/SW");
...
function inferActivityLevel(subjectRows, rawRows) {
  ...
  if (["모델링", "회귀", "점근선", "미분", "촉매", "윤리", "구현"].some(k => joined.includes(k))) return "심화";
...
export async function buildTaggedStudent(studentId, options = {}) {
  ...
  if (!student) throw new Error(`student not found in record_detail.json: ${studentId}`);
