...
function roadmapByTitle(title = "", taggedStudent = {}) {
  ...
  if (title.includes("미세먼지") || title.includes("대기")) {
    return {
      direction: "환경 데이터 해석을 바탕으로 발생 조건을 설명하고 개선 방안까지 설계하는 연구형 탐구",
      start: "미세먼지는 어떤 조건에서 증가하는가를 핵심 질문으로 두고, 시간·계절·기온·풍속 같은 변수를 기준으로 문제를 정의한다.",
      expand: "공공 데이터 또는 직접 측정 자료를 활용해 조건별 미세먼지 변화를 비교하고, 그래프·표·추세선으로 패턴을 시각화한다.",
      deepen: "도출한 패턴을 바탕으로 간단한 예측 기준을 세우거나 센서 활용, 저감 장치, 운영 개선 같은 해결 방안을 기술적으로 설계한다.",
      complete: "결론은 단순 결과 나열이 아니라 어떤 변수에서 차이가 컸는지, 그 이유를 어떻게 해석했는지, 개선 방향이 무엇인지를 구조적으로 설명하도록 정리한다.",
      outputs: ["조건별 비교 분석 보고서", "그래프·표 기반 시각화 자료", "탐구 발표 자료 또는 설계 포스터"],
      record_points: ["변수 설정과 비교 기준 수립", "데이터 기반 패턴 해석", "해결 방안 설계와 한계 분석"]
    };
  }
