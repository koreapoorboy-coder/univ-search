// Upgraded recommendation engine (rules-aware)
(function (global) {
  "use strict";
  ...
  function buildReason(subjectPlan, matches, studentRule) {
    ...
    if (studentRule?.main_track) reasons.push(`현재 학생의 주된 진로 축인 '${studentRule.main_track}'과 연결성이 높음`);
    if (studentRule?.sub_track) reasons.push(`보조 진로 축인 '${studentRule.sub_track}'과 함께 읽을 수 있음`);
    ...
    if (relatedMatches.length) {
      ...
      if (top.length) reasons.push(`교과서 단원 기준으로 ${top.join(", ")}와 자연스럽게 연결될 수 있음`);
    }

    if (subjectMatches(subject, "미적분")) {
      reasons.push("그래프, 변화율, 모델링 해석을 실제 데이터 이해와 연결하는 데 유리함");
    } else if (subjectMatches(subject, "물리2") || subjectMatches(subject, "물리Ⅱ") || subjectMatches(subject, "물리학Ⅱ")) {
      reasons.push("에너지 전환, 전류·전압, 전자기 개념을 시스템 관점으로 확장하기 좋음");
    } else if (subjectMatches(subject, "화학2") || subjectMatches(subject, "화학Ⅱ")) {
    ...
    } else if (subjectMatches(subject, "생명")) {
  ...
  function buildCoreRecommendation(subjectPlan) {
    ...
    if (subjectMatches(subject, "미적분")) return { top_activity_title: "데이터 변화와 시스템 해석을 수학적으로 구조화하는 중심 과목" };
    if (subjectMatches(subject, "물리2") || subjectMatches(subject, "물리Ⅱ") || subjectMatches(subject, "물리학Ⅱ")) return { top_activity_title: "에너지 흐름과 전기전자 시스템을 이해하는 핵심 과목" };
    if (subjectMatches(subject, "화학2") || subjectMatches(subject, "화학Ⅱ")) return { top_activity_title: "전지 반응과 물질 변화를 심화하는 핵심 과목" };
    if (subjectMatches(subject, "고급화학")) return { top_activity_title: "소재와 공정, 안정성을 더 깊게 다루는 심화 과목" };
    if (subjectMatches(subject, "생명")) return { top_activity_title: "생명 현상을 체계적으로 확장하는 과목" };
