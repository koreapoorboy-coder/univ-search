...
// ── 관문이 실제로 세기를 낮춘다 ───────────────────────────
{
  ...
  // 2026-09-24 2차: 어제는 「흔들림만으로는 설명되지 않는」으로 바꾸게 했는데, 그 말도 결국
  // 범위로 믿음성을 판단하는 것이어서 검수가 그대로 흠으로 잡았다(49장 중 26장). 이제 판단 자체를 막는다.
  ok(/흔들림보다 크다|흔들림보다 컸다/.test(out), `사실만 남겨야 한다 — ${out.slice(-70)}`);
  ok(!/설명되지 않|우연이 아니|신뢰할 수 있/.test(out), `판단하는 말이 남았다 — ${out.slice(-70)}`);
