...
// 돌려 보니 참고 도서 칸이 아예 안 나왔다. 까닭이 둘이었다.
{
  ...
  check(/stage === "experiment_draft" \? renderCollectionPanel\(stageResult, rawData\?\.bookChoices, rawData\?\.filledTable\) : renderBookPick\(rawData\?\.bookChoices\)/.test(bridge),
...
// 빈칸이었고, 축은 화학 과제인데 통합사회1·미적분1로 잡혔다. 그래서 책이 한 권도 안 나왔다.
{
  ...
  check(worker.includes("const chosen = fromTask ? { concept: fromTask, from: UNIT_SOURCE.TASK } : chooseUnit({"),
    "A3f 과제 글이 단원을 안 말하면 우리가 정한다");
  check(worker.includes("used: input.pastConcepts,"),
    "A3f 그 학생이 이미 쓴 단원은 빼고 고른다");
  check(worker.includes("input.unitSource = chosen.from;"),
    "A3f 어느 길로 정했는지 남긴다");
...
// A6: 워커와 화면이 실제로 이어져 있는가.
{
  ...
  check(bridge.includes("renderBookPick") && bridge.includes("renderCollectionPanel(stageResult, rawData?.bookChoices, rawData?.filledTable)"),
    "A6 화면이 그 후보를 받아 그린다");
