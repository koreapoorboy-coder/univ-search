...
(function(global){
  ...
  function getStateContext(){
    ...
    return {
      ...
      selectedBookSource: state.selectedBookSource || "",
  ...
  function getStableRenderContext(selectedBookValue){
    ...
    ctx.selectedBookTitle = stateCtx.selectedBookTitle || ctx.selectedBookTitle || "";
    return ctx;
  }
  ...
  function bookKey(book){
    const raw = book && (book.sourceId || book.bookId || book.book_id || book.managementNo);
    // v226: 일부 전체 DB 검색 도서는 별도 id가 비어 있어 data-value가 빈값으로 렌더링될 수 있었다.
    // 이 경우 클릭 후 selectedBook이 갱신되지 않아 이전 선택 도서(예: 국가론)가 요약에 남는 문제가 생긴다.
    // id가 없으면 제목을 안정 key로 사용해, 클릭한 카드와 요약 카드가 반드시 같은 책을 가리키게 한다.
    return val(raw || (book && book.title));
  }

  function findBook(bookId){
    const id = val(bookId);
    if (!id || !master || !Array.isArray(master.books)) return null;
    const nid = normalizeBookSearchQuery(id);
    return master.books.find(book => {
      const keys = [book.sourceId, book.bookId, book.book_id, String(book.managementNo || ""), book.title].map(val);
      if (keys.includes(id)) return true;
      return normalizeBookSearchQuery(book && book.title) === nid;
    }) || null;
  }
  ...
  function renderBookCard(book, active, index, sectionType){
    ...
    return `
      <button type="button" class="engine-book-card ${active ? "is-active" : ""} book-chip" data-kind="book" data-source="${esc(sectionType || "")}" data-value="${esc(key || book.title || "")}" data-title="${esc(book.title)}" data-book-search="${esc(getBookSearchText(book, sectionType))}">
  ...
  function buildBookWhyItems(book, sc, ctx, sectionType){
    ...
    if (sectionType === "search" || book.__globalSearchHit) {
      items.push(`${title}은(는) 학생이 직접 검색해 확인한 도서입니다. 추천 알고리즘이 전공·교과와 직접 일치한다고 확정한 책은 아닙니다.`);
      items.push(`보고서에 쓰려면 책의 실제 장·개념·사례 중 ${concept || "선택 개념"}${keyword ? " · " + keyword : ""}와 연결되는 부분을 학생이 직접 확인해야 합니다.`);
      items.push(`연결성이 충분하지 않다고 판단되면 “도서 선택 해제”를 눌러 도서 미사용 자료 중심형으로 진행하면 됩니다.`);
  ...
  function buildReadingFocusItems(book, sc, ctx, sectionType){
    const override = getBookDetailOverride(book);
    ...
    if (sectionType === "search" || book.__globalSearchHit) {
      items.push("책 제목만 보고 연결하지 말고, 목차·핵심 장·사례 중 탐구 주제와 직접 연결되는 부분이 있는지 먼저 확인합니다.");
      items.push("도서 내용이 현재 교과 개념과 약하게만 연결된다면 보고서의 핵심 근거가 아니라 보조 관점으로만 사용합니다.");
      return uniq(items).filter(Boolean).slice(0, 3);
    }
  ...
  function renderBookSummary(book, ctx, sectionType){
    ...
    const badge = sectionType === "direct" ? "직접 일치 도서" : (sectionType === "search" ? "학생 검색 선택 도서" : "확장 참고 도서");
    const aboutItems = buildStudentContentPoints(book);
    const whyItems = buildBookWhyItems(book, sc, ctx, sectionType);
    const focusItems = buildReadingFocusItems(book, sc, ctx, sectionType);
  ...
  // A-39 영어영문학과 하드락(v194)
  // 실제 데이터에 존재하는 학과명 "영어영문학과"만 대상으로 한다.
  // 공통국어1 화면에서 실제로 먼저 뜨는 문학 갈래 3개 중
  // 대표 확인은 "서사·극 갈래와 이야기 구성"으로 진행한다.
  // 4번 카드 제목/선택값을 우선 판별해 설명문 속 일반어가 다른 축으로 오인되는 것을 막는다.
  function getBookA39EnglishLiteratureVisiblePageText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (text) parts.push(text);
  ...
  function getBookA39EnglishLiteratureVisibleActiveTrackText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
    ...
    try {
      ...
      selectors.forEach(function(selector){
        document.querySelectorAll(selector).forEach(function(node){
          ...
          const titleNode = node.querySelector && node.querySelector(".engine-track-title, .track-title, .card-title, strong, h3, h4");
          push(titleNode ? titleNode.textContent : "");
          // v194: 설명문은 보조로만 읽는다. 제목/선택값이 먼저 축을 결정한다.
          const shortNode = node.querySelector && node.querySelector(".engine-track-short, .track-short, .card-short, .desc, p");
          push(shortNode ? shortNode.textContent : "");
        });
      });
  ...
  function isBookA39EnglishLiteratureContext(ctx){
    ctx = ctx || {};
    const pageText = getBookA39EnglishLiteratureVisiblePageText();
    const careerText = normalizeLockText([
      ctx.career, ctx.selectedMajor, ctx.department, ctx.major, ctx.majorName, ctx.majorSelectedName, pageText
    ].join(" "));
    const subjectConceptText = normalizeLockText([
    ...
    const isEnglishMajor = /영어영문학과/i.test(careerText);
    ...
    return !!(isEnglishMajor && isActualFlow);
  }

  function inferBookA39EnglishLiteratureAxis(ctx){
    ctx = ctx || {};
    const activeAxisText = normalizeLockText([
      getBookA39EnglishLiteratureVisibleActiveTrackText(),
      ctx.axisLabel,
      ctx.followupAxisId,
      ctx.linkTrack,
      ctx.axisDomain,
    ...
    const conceptText = normalizeLockText([
      ...
      ctx.keyword,
      ctx.selectedKeyword,
      getBookA39EnglishLiteratureVisiblePageText()
    ].join(" "));
    const fromText = function(text){
      text = normalizeLockText(text || "");
      if (!text) return "";
  ...
  function cloneBookForA39EnglishLiteratureLock(book, ctx, sectionType, axisId, rank){
    if (!book) return null;
    const lockedContext = buildLockedBookContextA26Humanities(book, ctx, sectionType, axisId, rank);
    const axisLabelMap = {
      narrative_structure: "서사 구조 분석 축",
    ...
    const axisLabel = axisLabelMap[axisId] || "영어영문학과 선택 축";
    return {
      ...book,
      matchType: sectionType,
      matchScore: 6060 - rank * 10,
      matchReasons: uniq(arr(book.matchReasons).concat([`A-39 영어영문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서 잠금`])),
      selectedBookContext: {
        ...lockedContext,
        recommendationReason: sectionType === "direct"
          ? `${book.title}은(는) ${axisLabel}에서 영미문학의 서사 구조, 인물 갈등, 문화적 맥락을 분석하는 직접 연결 도서입니다.`
          : `${book.title}은(는) ${axisLabel}에서 제국주의, 사회 비판, 번역·비교문화 관점으로 확장하는 참고 도서입니다.`,
        matchReasons: uniq(arr(lockedContext.matchReasons).concat([`영어영문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서`]))
      },
      bookA39EnglishLiteratureLock: true,
      bookA39EnglishLiteratureRank: rank,
      bookA39EnglishLiteratureAxisLock: axisId
    };
  }

  function applyBookA39EnglishLiteratureHardLock(result, ctx){
    if (!result || !isBookA39EnglishLiteratureContext(ctx)) return result;
    const axisId = inferBookA39EnglishLiteratureAxis(ctx);
    const directMap = {
      narrative_structure: ["빌러비드", "앵무새 죽이기", "시학"],
      storytelling_media: ["1984", "실낙원", "시학"],
      character_conflict: ["빌러비드", "어둠의 속", "앵무새 죽이기"],
      lyric_appreciation: ["실낙원", "그리스 비극 걸작선", "시학"],
      creative_expression: ["시학", "실낙원", "젊은 예술가의 초상"],
      emotion_media: ["1984", "오리엔탈리즘", "미디어의 이해"],
      reflective_writing: ["빌러비드", "어둠의 속", "젊은 예술가의 초상"]
    };
    const expansionMap = {
      narrative_structure: ["오만과 편견", "위대한 유산", "어둠의 속", "오리엔탈리즘", "1984"],
      storytelling_media: ["오리엔탈리즘", "제3의 물결", "미디어의 이해", "감시와 처벌", "문학과 예술의 사회사"],
      character_conflict: ["오만과 편견", "주홍글씨", "위대한 유산", "오리엔탈리즘", "문학과 예술의 사회사"],
      lyric_appreciation: ["문학과 예술의 사회사", "악의 꽃", "정지용전집", "백석 시 전집", "젊은 예술가의 초상"],
      creative_expression: ["문학과 예술의 사회사", "미디어의 이해", "디자인 인문학", "고도를 기다리며", "오리엔탈리즘"],
      emotion_media: ["문학과 예술의 사회사", "감시와 처벌", "제3의 물결", "일차원적 인간", "반지성주의"],
      reflective_writing: ["오만과 편견", "주홍글씨", "위대한 유산", "문학과 예술의 사회사", "오리엔탈리즘"]
    };
    const directTitles = arr(directMap[axisId] || directMap.narrative_structure);
    const expansionTitles = arr(expansionMap[axisId] || expansionMap.narrative_structure);
    const directBooks = directTitles.map((title, index) =>
      cloneBookForA39EnglishLiteratureLock(findBookForLock(title, result), ctx, "direct", axisId, index + 1)
    ).filter(Boolean).slice(0, 3);
    if (!directBooks.length) return result;
    const directIds = new Set(directBooks.map(book => bookKey(book)));
    const expansionBooks = expansionTitles.map((title, index) =>
      cloneBookForA39EnglishLiteratureLock(findBookForLock(title, result), ctx, "expansion", axisId, index + 1)
    ).filter(book => book && !directIds.has(bookKey(book))).slice(0, 5);
    return {
      ...result,
      directBooks,
      ...
      debug: {
        ...(result.debug || {}),
        bookA39EnglishLiteratureHardLock: axisId,
        bookA39EnglishLiteratureVersion: "v194",
        bookA39EnglishLiteratureDirectTitles: directBooks.map(book => book.title),
        bookA39EnglishLiteratureExpansionTitles: expansionBooks.map(book => book.title)
      }
    };
  }
  ...
  // A-41 중어중문학과 하드락(v196)
  // 실제 데이터에 존재하는 학과명 "중어중문학과"만 대상으로 한다.
  // 공통국어1 화면의 어문·문학 기본 3개 흐름을 기준으로 하며,
  // 4번 카드 제목/선택값을 우선 판별해 설명문 속 일반어가 다른 축으로 오인되는 것을 막는다.
  function getBookA41ChineseLiteratureVisiblePageText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (text) parts.push(text);
  ...
  function getBookA41ChineseLiteratureVisibleActiveTrackText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
  ...
  function isBookA41ChineseLiteratureContext(ctx){
    ctx = ctx || {};
    const pageText = getBookA41ChineseLiteratureVisiblePageText();
    const careerText = normalizeLockText([
      ctx.career, ctx.selectedMajor, ctx.department, ctx.major, ctx.majorName, ctx.majorSelectedName, pageText
    ].join(" "));
    const subjectConceptText = normalizeLockText([
    ...
    const isChineseMajor = /중어중문학과/i.test(careerText);
    const isActualFlow = /(공통국어1|공통국어Ⅰ|공통국어|국어|문학|서정\s*갈래와\s*시적\s*표현|서사\s*[·ㆍ-]?\s*극\s*갈래와\s*이야기\s*구성|교술\s*갈래와\s*성찰적\s*표현|문학\s*감상|서사\s*구조|스토리텔링|인물\s*[·ㆍ-]?\s*갈등|표현\s*[·ㆍ-]?\s*창작|정서\s*[·ㆍ-]?\s*매체)/i.test(subjectConceptText);
    return !!(isChineseMajor && isActualFlow);
  }

  function inferBookA41ChineseLiteratureAxis(ctx){
    ctx = ctx || {};
    const activeAxisText = normalizeLockText([
      getBookA41ChineseLiteratureVisibleActiveTrackText(),
      ctx.axisLabel,
      ctx.followupAxisId,
      ctx.linkTrack,
      ctx.axisDomain,
    ...
    const conceptText = normalizeLockText([
      ...
      ctx.keyword,
      ctx.selectedKeyword,
      getBookA41ChineseLiteratureVisiblePageText()
    ].join(" "));
    const fromText = function(text){
      text = normalizeLockText(text || "");
      if (!text) return "";
  ...
  function cloneBookForA41ChineseLiteratureLock(book, ctx, sectionType, axisId, rank){
    if (!book) return null;
    const lockedContext = buildLockedBookContextA26Humanities(book, ctx, sectionType, axisId, rank);
    const axisLabelMap = {
      narrative_structure: "서사 구조 분석 축",
    ...
    const axisLabel = axisLabelMap[axisId] || "중어중문학과 선택 축";
    return {
      ...book,
      matchType: sectionType,
      matchScore: 6020 - rank * 10,
      matchReasons: uniq(arr(book.matchReasons).concat([`A-41 중어중문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서 잠금`])),
      selectedBookContext: {
        ...lockedContext,
        recommendationReason: sectionType === "direct"
          ? `${book.title}은(는) ${axisLabel}에서 중국 고전·근현대 문학의 서사 구조, 사상, 인물 갈등을 분석하는 직접 연결 도서입니다.`
          : `${book.title}은(는) ${axisLabel}에서 동아시아 사상, 비교문화, 사회 변화 관점으로 확장하는 참고 도서입니다.`,
        matchReasons: uniq(arr(lockedContext.matchReasons).concat([`중어중문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서`]))
      },
      bookA41ChineseLiteratureLock: true,
      bookA41ChineseLiteratureRank: rank,
      bookA41ChineseLiteratureAxisLock: axisId
    };
  }

  function applyBookA41ChineseLiteratureHardLock(result, ctx){
    if (!result || !isBookA41ChineseLiteratureContext(ctx)) return result;
    const axisId = inferBookA41ChineseLiteratureAxis(ctx);
    const directMap = {
      narrative_structure: ["수호전", "루쉰 소설전집", "시학"],
      storytelling_media: ["수호전", "아라비안 나이트", "동방견문록"],
      character_conflict: ["루쉰 소설전집", "수호전", "맹자"],
      lyric_appreciation: ["시경", "논어", "시학"],
      creative_expression: ["시경", "루쉰 소설전집", "시학"],
      emotion_media: ["오리엔탈리즘", "동방견문록", "미디어의 이해"],
      reflective_writing: ["논어", "맹자", "인물로 읽는 장자"]
    };
    const expansionMap = {
      narrative_structure: ["논어", "맹자", "인물로 읽는 장자", "오리엔탈리즘", "문학과 예술의 사회사"],
      storytelling_media: ["오리엔탈리즘", "문학과 예술의 사회사", "미디어의 이해", "데카메론", "국화와 칼"],
      character_conflict: ["논어", "맹자", "인물로 읽는 장자", "문학과 예술의 사회사", "오리엔탈리즘"],
      lyric_appreciation: ["문학과 예술의 사회사", "정지용전집", "백석 시 전집", "젊은 예술가의 초상", "인물로 읽는 장자"],
      creative_expression: ["문학과 예술의 사회사", "디자인 인문학", "미디어의 이해", "젊은 예술가의 초상", "고도를 기다리며"],
      emotion_media: ["문학과 예술의 사회사", "국화와 칼", "감시와 처벌", "제3의 물결", "반지성주의"],
      reflective_writing: ["시경", "루쉰 소설전집", "문학과 예술의 사회사", "오리엔탈리즘", "국화와 칼"]
    };
    const directTitles = arr(directMap[axisId] || directMap.narrative_structure);
    const expansionTitles = arr(expansionMap[axisId] || expansionMap.narrative_structure);
    const directBooks = directTitles.map((title, index) =>
      cloneBookForA41ChineseLiteratureLock(findBookForLock(title, result), ctx, "direct", axisId, index + 1)
    ).filter(Boolean).slice(0, 3);
    if (!directBooks.length) return result;
    const directIds = new Set(directBooks.map(book => bookKey(book)));
    const expansionBooks = expansionTitles.map((title, index) =>
      cloneBookForA41ChineseLiteratureLock(findBookForLock(title, result), ctx, "expansion", axisId, index + 1)
    ).filter(book => book && !directIds.has(bookKey(book))).slice(0, 5);
    return {
      ...result,
      directBooks,
      ...
      debug: {
        ...(result.debug || {}),
        bookA41ChineseLiteratureHardLock: axisId,
        bookA41ChineseLiteratureVersion: "v196",
        bookA41ChineseLiteratureDirectTitles: directBooks.map(book => book.title),
        bookA41ChineseLiteratureExpansionTitles: expansionBooks.map(book => book.title)
      }
    };
  }


  // A-42 독어독문학과 하드락(v197)
  // 실제 데이터에 존재하는 학과명 "독어독문학과"만 대상으로 한다.
  // 공통국어1 화면의 어문·문학 기본 3개 흐름을 기준으로 하며,
  // 4번 카드 제목/선택값을 우선 판별해 설명문 속 일반어가 다른 축으로 오인되는 것을 막는다.
  function getBookA42GermanLiteratureVisiblePageText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (text) parts.push(text);
  ...
  function getBookA42GermanLiteratureVisibleActiveTrackText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
  ...
  function isBookA42GermanLiteratureContext(ctx){
    ctx = ctx || {};
    const pageText = getBookA42GermanLiteratureVisiblePageText();
    const careerText = normalizeLockText([
      ctx.career, ctx.selectedMajor, ctx.department, ctx.major, ctx.majorName, ctx.majorSelectedName, pageText
    ].join(" "));
    const subjectConceptText = normalizeLockText([
    ...
    const isGermanMajor = /독어독문학과/i.test(careerText);
    const isActualFlow = /(공통국어1|공통국어Ⅰ|공통국어|국어|문학|서정\s*갈래와\s*시적\s*표현|서사\s*[·ㆍ-]?\s*극\s*갈래와\s*이야기\s*구성|교술\s*갈래와\s*성찰적\s*표현|문학\s*감상|서사\s*구조|스토리텔링|인물\s*[·ㆍ-]?\s*갈등|표현\s*[·ㆍ-]?\s*창작|정서\s*[·ㆍ-]?\s*매체)/i.test(subjectConceptText);
    return !!(isGermanMajor && isActualFlow);
  }

  function inferBookA42GermanLiteratureAxis(ctx){
    ctx = ctx || {};
    const activeAxisText = normalizeLockText([
      getBookA42GermanLiteratureVisibleActiveTrackText(),
      ctx.axisLabel,
      ctx.followupAxisId,
      ctx.linkTrack,
      ctx.axisDomain,
    ...
    const conceptText = normalizeLockText([
      ...
      ctx.keyword,
      ctx.selectedKeyword,
      getBookA42GermanLiteratureVisiblePageText()
    ].join(" "));
    const fromText = function(text){
      text = normalizeLockText(text || "");
      if (!text) return "";
  ...
  function cloneBookForA41GermanLiteratureLock(book, ctx, sectionType, axisId, rank){
    if (!book) return null;
    const lockedContext = buildLockedBookContextA26Humanities(book, ctx, sectionType, axisId, rank);
    const axisLabelMap = {
      narrative_structure: "서사 구조 분석 축",
    ...
    const axisLabel = axisLabelMap[axisId] || "독어독문학과 선택 축";
    return {
      ...book,
      matchType: sectionType,
      matchScore: 6000 - rank * 10,
      matchReasons: uniq(arr(book.matchReasons).concat([`A-42 독어독문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서 잠금`])),
      selectedBookContext: {
        ...lockedContext,
        recommendationReason: sectionType === "direct"
          ? `${book.title}은(는) ${axisLabel}에서 독일어권 문학의 근대적 자아, 소외, 성장, 인물 갈등을 분석하는 직접 연결 도서입니다.`
          : `${book.title}은(는) ${axisLabel}에서 유럽 근현대 사상, 예술사, 실존·사회비판 관점으로 확장하는 참고 도서입니다.`,
        matchReasons: uniq(arr(lockedContext.matchReasons).concat([`독어독문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서`]))
      },
      bookA42GermanLiteratureLock: true,
      bookA42GermanLiteratureRank: rank,
      bookA42GermanLiteratureAxisLock: axisId
    };
  }

  function applyBookA42GermanLiteratureHardLock(result, ctx){
    if (!result || !isBookA42GermanLiteratureContext(ctx)) return result;
    const axisId = inferBookA42GermanLiteratureAxis(ctx);
    const directMap = {
      narrative_structure: ["변신", "데미안", "시학"],
      storytelling_media: ["고도를 기다리며", "변신", "데미안"],
      character_conflict: ["데미안", "변신", "마의 산"],
      lyric_appreciation: ["악의 꽃", "그리스 비극 걸작선", "시학"],
      creative_expression: ["시학", "젊은 예술가의 초상", "디자인 인문학"],
      emotion_media: ["미디어의 이해", "변신", "문학과 예술의 사회사"],
      reflective_writing: ["데미안", "마의 산", "변신"]
    };
    const expansionMap = {
      narrative_structure: ["마의 산", "고도를 기다리며", "문학과 예술의 사회사", "오리엔탈리즘", "이방인"],
      storytelling_media: ["문학과 예술의 사회사", "미디어의 이해", "1984", "감시와 처벌", "이방인"],
      character_conflict: ["고도를 기다리며", "이방인", "페스트", "문학과 예술의 사회사", "오리엔탈리즘"],
      lyric_appreciation: ["문학과 예술의 사회사", "정지용전집", "백석 시 전집", "젊은 예술가의 초상", "이방인"],
      creative_expression: ["문학과 예술의 사회사", "미디어의 이해", "고도를 기다리며", "변신", "데미안"],
      emotion_media: ["감시와 처벌", "1984", "오리엔탈리즘", "문학과 예술의 사회사", "반지성주의"],
      reflective_writing: ["고도를 기다리며", "이방인", "페스트", "문학과 예술의 사회사", "젊은 예술가의 초상"]
    };
    const directTitles = arr(directMap[axisId] || directMap.narrative_structure);
    const expansionTitles = arr(expansionMap[axisId] || expansionMap.narrative_structure);
    const directBooks = directTitles.map((title, index) =>
      cloneBookForA41GermanLiteratureLock(findBookForLock(title, result), ctx, "direct", axisId, index + 1)
    ).filter(Boolean).slice(0, 3);
    if (!directBooks.length) return result;
    const directIds = new Set(directBooks.map(book => bookKey(book)));
    const expansionBooks = expansionTitles.map((title, index) =>
      cloneBookForA41GermanLiteratureLock(findBookForLock(title, result), ctx, "expansion", axisId, index + 1)
    ).filter(book => book && !directIds.has(bookKey(book))).slice(0, 5);
    return {
      ...result,
      directBooks,
      ...
      debug: {
        ...(result.debug || {}),
        bookA42GermanLiteratureHardLock: axisId,
        bookA42GermanLiteratureVersion: "v197",
        bookA42GermanLiteratureDirectTitles: directBooks.map(book => book.title),
        bookA42GermanLiteratureExpansionTitles: expansionBooks.map(book => book.title)
      }
    };
  }


  // A-43 불어불문학과 하드락(v198)
  // 실제 데이터에 존재하는 학과명 "불어불문학과"만 대상으로 한다.
  // 공통국어1 화면의 어문·문학 기본 3개 흐름을 기준으로 하며,
  // 4번 카드 제목/선택값을 우선 판별해 설명문 속 일반어가 다른 축으로 오인되는 것을 막는다.
  function getBookA43FrenchLiteratureVisiblePageText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (text) parts.push(text);
  ...
  function getBookA43FrenchLiteratureVisibleActiveTrackText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
  ...
  function isBookA43FrenchLiteratureContext(ctx){
    ctx = ctx || {};
    const pageText = getBookA43FrenchLiteratureVisiblePageText();
    const careerText = normalizeLockText([
      ctx.career, ctx.selectedMajor, ctx.department, ctx.major, ctx.majorName, ctx.majorSelectedName, pageText
    ].join(" "));
    const subjectConceptText = normalizeLockText([
    ...
    const isFrenchMajor = /불어불문학과/i.test(careerText);
    const isActualFlow = /(공통국어1|공통국어Ⅰ|공통국어|국어|문학|서정\s*갈래와\s*시적\s*표현|서사\s*[·ㆍ-]?\s*극\s*갈래와\s*이야기\s*구성|교술\s*갈래와\s*성찰적\s*표현|문학\s*감상|서사\s*구조|스토리텔링|인물\s*[·ㆍ-]?\s*갈등|표현\s*[·ㆍ-]?\s*창작|정서\s*[·ㆍ-]?\s*매체)/i.test(subjectConceptText);
    return !!(isFrenchMajor && isActualFlow);
  }

  function inferBookA43FrenchLiteratureAxis(ctx){
    ctx = ctx || {};
    const activeAxisText = normalizeLockText([
      getBookA43FrenchLiteratureVisibleActiveTrackText(),
      ctx.axisLabel,
      ctx.followupAxisId,
      ctx.linkTrack,
      ctx.axisDomain,
    ...
    const conceptText = normalizeLockText([
      ...
      ctx.keyword,
      ctx.selectedKeyword,
      getBookA43FrenchLiteratureVisiblePageText()
    ].join(" "));
    const fromText = function(text){
      text = normalizeLockText(text || "");
      if (!text) return "";
  ...
  function cloneBookForA43FrenchLiteratureLock(book, ctx, sectionType, axisId, rank){
    if (!book) return null;
    const lockedContext = buildLockedBookContextA26Humanities(book, ctx, sectionType, axisId, rank);
    const axisLabelMap = {
      narrative_structure: "서사 구조 분석 축",
    ...
    const axisLabel = axisLabelMap[axisId] || "불어불문학과 선택 축";
    return {
      ...book,
      matchType: sectionType,
      matchScore: 6000 - rank * 10,
      matchReasons: uniq(arr(book.matchReasons).concat([`A-43 불어불문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서 잠금`])),
      selectedBookContext: {
        ...lockedContext,
        recommendationReason: sectionType === "direct"
          ? `${book.title}은(는) ${axisLabel}에서 프랑스어권 문학의 근대 도시, 인간 소외, 실존, 사회비판을 분석하는 직접 연결 도서입니다.`
          : `${book.title}은(는) ${axisLabel}에서 프랑스 문학·사상·문화와 비교문학 관점으로 확장하는 참고 도서입니다.`,
        matchReasons: uniq(arr(lockedContext.matchReasons).concat([`불어불문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서`]))
      },
      bookA43FrenchLiteratureLock: true,
      bookA43FrenchLiteratureRank: rank,
      bookA43FrenchLiteratureAxisLock: axisId
    };
  }

  function applyBookA43FrenchLiteratureHardLock(result, ctx){
    if (!result || !isBookA43FrenchLiteratureContext(ctx)) return result;
    const axisId = inferBookA43FrenchLiteratureAxis(ctx);
    const directMap = {
      narrative_structure: ["고리오 영감", "이방인", "시학"],
      storytelling_media: ["고도를 기다리며", "이방인", "시학"],
      character_conflict: ["페스트", "이방인", "적과 흑"],
      lyric_appreciation: ["악의 꽃", "시학", "문학과 예술의 사회사"],
      creative_expression: ["악의 꽃", "젊은 예술가의 초상", "시학"],
      emotion_media: ["미디어의 이해", "악의 꽃", "문학과 예술의 사회사"],
      reflective_writing: ["이방인", "페스트", "고리오 영감"]
    };
    const expansionMap = {
      narrative_structure: ["페스트", "적과 흑", "문학과 예술의 사회사", "사회계약론", "감시와 처벌"],
      storytelling_media: ["문학과 예술의 사회사", "미디어의 이해", "1984", "고리오 영감", "감시와 처벌"],
      character_conflict: ["고도를 기다리며", "고리오 영감", "사회계약론", "문학과 예술의 사회사", "감시와 처벌"],
      lyric_appreciation: ["젊은 예술가의 초상", "정지용전집", "백석 시 전집", "문학과 예술의 사회사", "이방인"],
      creative_expression: ["문학과 예술의 사회사", "미디어의 이해", "고도를 기다리며", "이방인", "페스트"],
      emotion_media: ["감시와 처벌", "1984", "반지성주의", "문학과 예술의 사회사", "사회계약론"],
      reflective_writing: ["적과 흑", "사회계약론", "문학과 예술의 사회사", "감시와 처벌", "고도를 기다리며"]
    };
    const directTitles = arr(directMap[axisId] || directMap.narrative_structure);
    const expansionTitles = arr(expansionMap[axisId] || expansionMap.narrative_structure);
    const directBooks = directTitles.map((title, index) =>
      cloneBookForA43FrenchLiteratureLock(findBookForLock(title, result), ctx, "direct", axisId, index + 1)
    ).filter(Boolean).slice(0, 3);
    if (!directBooks.length) return result;
    const directIds = new Set(directBooks.map(book => bookKey(book)));
    const expansionBooks = expansionTitles.map((title, index) =>
      cloneBookForA43FrenchLiteratureLock(findBookForLock(title, result), ctx, "expansion", axisId, index + 1)
    ).filter(book => book && !directIds.has(bookKey(book))).slice(0, 5);
    return {
      ...result,
      directBooks,
      ...
      debug: {
        ...(result.debug || {}),
        bookA43FrenchLiteratureHardLock: axisId,
        bookA43FrenchLiteratureVersion: "v198",
        bookA43FrenchLiteratureDirectTitles: directBooks.map(book => book.title),
        bookA43FrenchLiteratureExpansionTitles: expansionBooks.map(book => book.title)
      }
    };
  }


  // A-44 노어노문학과 하드락(v199)
  // 실제 데이터에 존재하는 학과명 "노어노문학과"만 대상으로 한다.
  // 공통국어1 화면의 어문·문학 기본 3개 흐름을 기준으로 하며,
  // 4번 카드 제목/선택값을 우선 판별해 설명문 속 일반어가 다른 축으로 오인되는 것을 막는다.
  function getBookA44RussianLiteratureVisiblePageText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (text) parts.push(text);
  ...
  function getBookA44RussianLiteratureVisibleActiveTrackText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
  ...
  function isBookA44RussianLiteratureContext(ctx){
    ctx = ctx || {};
    const pageText = getBookA44RussianLiteratureVisiblePageText();
    const careerText = normalizeLockText([
      ctx.career, ctx.selectedMajor, ctx.department, ctx.major, ctx.majorName, ctx.majorSelectedName, pageText
    ].join(" "));
    const subjectConceptText = normalizeLockText([
    ...
    const isRussianMajor = /노어노문학과/i.test(careerText);
    const isActualFlow = /(공통국어1|공통국어Ⅰ|공통국어|국어|문학|서정\s*갈래와\s*시적\s*표현|서사\s*[·ㆍ-]?\s*극\s*갈래와\s*이야기\s*구성|교술\s*갈래와\s*성찰적\s*표현|문학\s*감상|서사\s*구조|스토리텔링|인물\s*[·ㆍ-]?\s*갈등|표현\s*[·ㆍ-]?\s*창작|정서\s*[·ㆍ-]?\s*매체)/i.test(subjectConceptText);
    return !!(isRussianMajor && isActualFlow);
  }

  function inferBookA44RussianLiteratureAxis(ctx){
    ctx = ctx || {};
    const activeAxisText = normalizeLockText([
      getBookA44RussianLiteratureVisibleActiveTrackText(),
      ctx.axisLabel,
      ctx.followupAxisId,
      ctx.linkTrack,
      ctx.axisDomain,
    ...
    const conceptText = normalizeLockText([
      ...
      ctx.keyword,
      ctx.selectedKeyword,
      getBookA44RussianLiteratureVisiblePageText()
    ].join(" "));
    const fromText = function(text){
      text = normalizeLockText(text || "");
      if (!text) return "";
  ...
  function cloneBookForA44RussianLiteratureLock(book, ctx, sectionType, axisId, rank){
    if (!book) return null;
    const lockedContext = buildLockedBookContextA26Humanities(book, ctx, sectionType, axisId, rank);
    const axisLabelMap = {
      narrative_structure: "서사 구조 분석 축",
    ...
    const axisLabel = axisLabelMap[axisId] || "노어노문학과 선택 축";
    return {
      ...book,
      matchType: sectionType,
      matchScore: 6000 - rank * 10,
      matchReasons: uniq(arr(book.matchReasons).concat([`A-44 노어노문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서 잠금`])),
      selectedBookContext: {
        ...lockedContext,
        recommendationReason: sectionType === "direct"
          ? `${book.title}은(는) ${axisLabel}에서 러시아 문학의 인물 갈등, 사회 변화, 실존적 질문을 분석하는 직접 연결 도서입니다.`
          : `${book.title}은(는) ${axisLabel}에서 러시아 문학·사상·문화와 비교문학 관점으로 확장하는 참고 도서입니다.`,
        matchReasons: uniq(arr(lockedContext.matchReasons).concat([`노어노문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서`]))
      },
      bookA44RussianLiteratureLock: true,
      bookA44RussianLiteratureRank: rank,
      bookA44RussianLiteratureAxisLock: axisId
    };
  }

  function applyBookA44RussianLiteratureHardLock(result, ctx){
    if (!result || !isBookA44RussianLiteratureContext(ctx)) return result;
    const axisId = inferBookA44RussianLiteratureAxis(ctx);
    const directMap = {
      narrative_structure: ["카라마조프 가의 형제들", "안나 카레니나", "시학"],
      storytelling_media: ["우리 시대의 영웅", "검찰관", "갈매기"],
      character_conflict: ["카라마조프 가의 형제들", "안나 카레니나", "갈매기"],
      lyric_appreciation: ["갈매기", "우리 시대의 영웅", "문학과 예술의 사회사"],
      creative_expression: ["검찰관", "우리 시대의 영웅", "시학"],
      emotion_media: ["미디어의 이해", "검찰관", "갈매기"],
      reflective_writing: ["우리 시대의 영웅", "안나 카레니나", "카라마조프 가의 형제들"]
    };
    const expansionMap = {
      narrative_structure: ["우리 시대의 영웅", "검찰관", "갈매기", "문학과 예술의 사회사", "어둠의 속"],
      storytelling_media: ["카라마조프 가의 형제들", "안나 카레니나", "문학과 예술의 사회사", "미디어의 이해", "1984"],
      character_conflict: ["우리 시대의 영웅", "검찰관", "문학과 예술의 사회사", "어둠의 속", "이방인"],
      lyric_appreciation: ["시학", "젊은 예술가의 초상", "문학과 예술의 사회사", "안나 카레니나", "갈매기"],
      creative_expression: ["문학과 예술의 사회사", "갈매기", "카라마조프 가의 형제들", "미디어의 이해", "고도를 기다리며"],
      emotion_media: ["1984", "반지성주의", "문학과 예술의 사회사", "카라마조프 가의 형제들", "안나 카레니나"],
      reflective_writing: ["검찰관", "갈매기", "문학과 예술의 사회사", "이방인", "고도를 기다리며"]
    };
    const directTitles = arr(directMap[axisId] || directMap.narrative_structure);
    const expansionTitles = arr(expansionMap[axisId] || expansionMap.narrative_structure);
    const directBooks = directTitles.map((title, index) =>
      cloneBookForA44RussianLiteratureLock(findBookForLock(title, result), ctx, "direct", axisId, index + 1)
    ).filter(Boolean).slice(0, 3);
    if (!directBooks.length) return result;
    const directIds = new Set(directBooks.map(book => bookKey(book)));
    const expansionBooks = expansionTitles.map((title, index) =>
      cloneBookForA44RussianLiteratureLock(findBookForLock(title, result), ctx, "expansion", axisId, index + 1)
    ).filter(book => book && !directIds.has(bookKey(book))).slice(0, 5);
    return {
      ...result,
      directBooks,
      ...
      debug: {
        ...(result.debug || {}),
        bookA44RussianLiteratureHardLock: axisId,
        bookA44RussianLiteratureVersion: "v199",
        bookA44RussianLiteratureDirectTitles: directBooks.map(book => book.title),
        bookA44RussianLiteratureExpansionTitles: expansionBooks.map(book => book.title)
      }
    };
  }
  ...
  function getBookA47KoreanLanguageVisibleActiveTrackText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
  ...
  function isBookA47KoreanLanguageContext(ctx){
    ctx = ctx || {};
    const pageText = getBookA47KoreanLanguageVisiblePageText();
    const careerText = normalizeLockText([
      ctx.career, ctx.selectedMajor, ctx.department, ctx.major, ctx.majorName, ctx.majorSelectedName, pageText
    ].join(" "));
    const subjectConceptText = normalizeLockText([
    ...
    const isKoreanLanguageMajor = /한국어학과/i.test(careerText);
    const isActualFlow = /(공통국어1|공통국어Ⅰ|공통국어|국어|음운\s*변동과\s*국어\s*규범|문장\s*구성|사회적\s*쟁점\s*글쓰기|비판적\s*읽기와\s*토론|공동체\s*의사소통과\s*공감|언어\s*규범|정확한\s*표현|언어생활|주장\s*글쓰기|문장\s*점검|공공\s*[·ㆍ-]?\s*매체\s*표현|자료\s*검증|논증\s*[·ㆍ-]?\s*토론|화법\s*[·ㆍ-]?\s*공감|공동체\s*협업|갈등\s*조정)/i.test(subjectConceptText);
    return !!(isKoreanLanguageMajor && isActualFlow);
  }

  function inferBookA47KoreanLanguageAxis(ctx){
    ctx = ctx || {};
    const activeAxisText = normalizeLockText([
      getBookA47KoreanLanguageVisibleActiveTrackText(),
      ctx.axisLabel,
      ctx.followupAxisId,
      ctx.linkTrack,
      ctx.axisDomain,
    ...
    const conceptText = normalizeLockText([
      ...
      ctx.keyword,
      ctx.selectedKeyword,
      getBookA47KoreanLanguageVisiblePageText()
    ].join(" "));
    const fromText = function(text){
      text = normalizeLockText(text || "");
      if (!text) return "";
      // 실제 4번 카드 제목/선택값을 먼저 판별한다. 설명문 속 일반어로 오인하지 않도록 축명을 우선한다.
      if (/(language_norm_inquiry|언어\s*규범\s*탐구\s*축|언어\s*규범\s*탐구|국어\s*규범|음운\s*규범|발음\s*규칙)/i.test(text)) return "language_norm";
      if (/(accurate_expression|정확한\s*표현\s*축|정확한\s*표현|표준\s*발음|표현\s*정확성)/i.test(text)) return "accurate_expression";
      if (/(language_life_application|언어생활\s*적용\s*축|언어생활\s*적용|언어생활)/i.test(text)) return "language_life";
      if (/(argumentative_writing|주장\s*글쓰기\s*축|주장\s*글쓰기|논증\s*글쓰기)/i.test(text)) return "argumentative_writing";
      if (/(sentence_revision_editing|문장\s*점검\s*[·ㆍ-]?\s*수정\s*축|문장\s*점검\s*[·ㆍ-]?\s*수정|문장\s*점검|퇴고|문장\s*구성)/i.test(text)) return "sentence_revision";
      if (/(public_media_expression|공공\s*[·ㆍ-]?\s*매체\s*표현\s*축|공공\s*[·ㆍ-]?\s*매체\s*표현|매체\s*표현)/i.test(text)) return "public_media";
      if (/(evidence_verification_analysis|자료\s*검증\s*[·ㆍ-]?\s*쟁점\s*분석\s*축|자료\s*검증\s*[·ㆍ-]?\s*쟁점\s*분석|자료\s*검증|쟁점\s*분석)/i.test(text)) return "evidence_verification";
      if (/(argument_discussion|논증\s*[·ㆍ-]?\s*토론\s*축|논증\s*[·ㆍ-]?\s*토론|토론\s*축)/i.test(text)) return "argument_discussion";
      if (/(critical_interpretation_extension|비판\s*해석\s*확장\s*축|비판\s*해석\s*확장|비판\s*해석|비판\s*확장)/i.test(text)) return "critical_interpretation";
  ...
  function cloneBookForA47KoreanLanguageLock(book, ctx, sectionType, axisId, rank){
    ...
    const axisLabel = axisLabelMap[axisId] || "한국어학과 선택 축";
    return {
      ...book,
      matchType: sectionType,
      matchScore: 6010 - rank * 10,
      matchReasons: uniq(arr(book.matchReasons).concat([`A-47 한국어학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서 잠금`])),
      selectedBookContext: {
        ...lockedContext,
        recommendationReason: sectionType === "direct"
          ? `${book.title}은(는) ${axisLabel}에서 한국어의 음운·문법·의미·표현 구조를 설명하는 직접 연결 도서입니다.`
          : `${book.title}은(는) ${axisLabel}에서 언어 사용, 매체 소통, 사회적 의미로 확장하는 참고 도서입니다.`,
        matchReasons: uniq(arr(lockedContext.matchReasons).concat([`한국어학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서`]))
      },
      bookA47KoreanLanguageLock: true,
      bookA47KoreanLanguageRank: rank,
      bookA47KoreanLanguageAxisLock: axisId
    };
  }

  function applyBookA47KoreanLanguageHardLock(result, ctx){
    if (!result || !isBookA47KoreanLanguageContext(ctx)) return result;
    const axisId = inferBookA47KoreanLanguageAxis(ctx);
    const directMap = {
      language_norm: ["철학적 탐구", "시학", "같기도 하고 아니 같기도 하고"],
      accurate_expression: ["철학적 탐구", "의사소통 행위이론", "시학"],
      language_life: ["의사소통 행위이론", "미디어의 이해", "철학적 탐구"],
      argumentative_writing: ["의사소통 행위이론", "반지성주의", "같기도 하고 아니 같기도 하고"],
      sentence_revision: ["시학", "철학적 탐구", "정지용전집"],
      public_media: ["미디어의 이해", "의사소통 행위이론", "1984"],
      evidence_verification: ["같기도 하고 아니 같기도 하고", "반지성주의", "미디어의 이해"],
      argument_discussion: ["의사소통 행위이론", "철학적 탐구", "반지성주의"],
      critical_interpretation: ["철학적 탐구", "1984", "미디어의 이해"],
      communication_empathy: ["의사소통 행위이론", "사람, 장소, 환대", "철학적 탐구"],
      community_collaboration: ["사람, 장소, 환대", "의사소통 행위이론", "죽은 시인의 사회"],
      conflict_mediation: ["의사소통 행위이론", "사람, 장소, 환대", "갈매기"]
    };
    const expansionMap = {
      language_norm: ["미디어의 이해", "의사소통 행위이론", "정지용전집", "문학과 예술의 사회사", "반지성주의"],
      accurate_expression: ["미디어의 이해", "같기도 하고 아니 같기도 하고", "정지용전집", "문학과 예술의 사회사", "반지성주의"],
      language_life: ["반지성주의", "성의 역사 1", "감시와 처벌", "사람, 장소, 환대", "1984"],
      argumentative_writing: ["철학적 탐구", "미디어의 이해", "1984", "시학", "자유론"],
      sentence_revision: ["의사소통 행위이론", "미디어의 이해", "반지성주의", "문학과 예술의 사회사", "정지용전집"],
      public_media: ["철학적 탐구", "반지성주의", "성의 역사 1", "감시와 처벌", "시학"],
      evidence_verification: ["철학적 탐구", "1984", "성의 역사 1", "감시와 처벌", "시학"],
      argument_discussion: ["같기도 하고 아니 같기도 하고", "1984", "미디어의 이해", "자유론", "공정하다는 착각"],
      critical_interpretation: ["감시와 처벌", "성의 역사 1", "반지성주의", "시학", "의사소통 행위이론"],
      communication_empathy: ["미디어의 이해", "반지성주의", "갈매기", "마음", "데미안"],
      community_collaboration: ["미디어의 이해", "죽은 시인의 사회", "갈매기", "1984", "자유론"],
    ...
    const directTitles = arr(directMap[axisId] || directMap.language_norm);
    const expansionTitles = arr(expansionMap[axisId] || expansionMap.language_norm);
    const directBooks = directTitles.map((title, index) =>
      cloneBookForA47KoreanLanguageLock(findBookForLock(title, result), ctx, "direct", axisId, index + 1)
    ).filter(Boolean).slice(0, 3);
    if (!directBooks.length) return result;
    const directIds = new Set(directBooks.map(book => bookKey(book)));
    const expansionBooks = expansionTitles.map((title, index) =>
      cloneBookForA47KoreanLanguageLock(findBookForLock(title, result), ctx, "expansion", axisId, index + 1)
    ).filter(book => book && !directIds.has(bookKey(book))).slice(0, 5);
    return {
      ...result,
      directBooks,
  ...
  // v210 수정: 이전 인계문·v209에서 섞인 비판적 읽기·토론형 기준을 제거하고,
  // 업로드 ZIP과 현재 화면에 실제 존재하는 문학 갈래형 3개 개념과 4번 카드 제목/선택값만 기준으로 도서를 잠근다.
  ...
  function getBookA48ClassicalChineseVisibleActiveConceptText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
    ...
    try {
      const selectors = [
        ".engine-concept-card.is-active", ".engine-concept-card[aria-pressed='true']", ".engine-concept-card[aria-selected='true']",
        ".engine-concept-card.selected", ".engine-concept-card.active", ".engine-concept-card.is-selected",
        "[data-concept].is-active", "[data-concept].selected", "[data-concept].active", "[data-concept].is-selected",
        "[data-keyword].is-active", "[data-keyword].selected", "[data-keyword].active", "[data-keyword].is-selected"
      ];
      const seen = new Set();
      selectors.forEach(function(selector){
        document.querySelectorAll(selector).forEach(function(node){
          if (!node || seen.has(node)) return;
          seen.add(node);
          push(node.getAttribute("data-concept"));
          push(node.getAttribute("data-keyword"));
          push(node.getAttribute("data-concept-id"));
          push(node.getAttribute("data-keyword-id"));
          const titleNode = node.querySelector && node.querySelector(".engine-concept-title, .concept-title, .card-title, strong, h3, h4");
          push(titleNode ? titleNode.textContent : "");
          push(node.textContent || "");
        });
      });
  ...
  function getBookA48ClassicalChineseVisibleActiveTrackText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
  ...
  function isBookA48ClassicalChineseContext(ctx){
    ctx = ctx || {};
    const pageText = getBookA48ClassicalChineseVisiblePageText();
    const careerText = normalizeLockText([
      ctx.career, ctx.selectedMajor, ctx.department, ctx.major, ctx.majorName, ctx.majorSelectedName, pageText
    ].join(" "));
    const subjectConceptText = normalizeLockText([
  ...
  function inferBookA48ClassicalChineseAxis(ctx){
    ctx = ctx || {};
    const activeAxisText = normalizeLockText([
      getBookA48ClassicalChineseVisibleActiveTrackText(),
      ctx.axisLabel,
      ctx.followupAxisId,
      ctx.linkTrack,
      ctx.axisDomain,
    ...
    const selectedConceptText = normalizeLockText([
      getBookA48ClassicalChineseVisibleActiveConceptText(),
      ctx.concept,
      ctx.selectedConcept,
      ctx.keyword,
      ctx.selectedKeyword
    ...
    const fromText = function(text){
      ...
      if (/(narrative_structure_analysis|서사\s*구조\s*분석\s*축|서사\s*구조\s*분석|이야기\s*구성|구성\s*분석)/i.test(text)) return "narrative_structure";
      if (/(storytelling_media_extension|스토리텔링\s*[·ㆍ-]?\s*매체\s*축|스토리텔링\s*[·ㆍ-]?\s*매체|매체\s*서사|장면\s*구성|대본)/i.test(text)) return "storytelling_media";
      if (/(character_conflict_interpretation|인물\s*[·ㆍ-]?\s*갈등\s*해석\s*축|인물\s*[·ㆍ-]?\s*갈등\s*해석|갈등\s*해석|관계\s*해석)/i.test(text)) return "character_conflict";
      if (/(reflective_writing_extension|성찰\s*글쓰기\s*축|성찰\s*글쓰기|성찰적\s*표현|자기\s*성찰)/i.test(text)) return "reflective_writing";
      if (/(observation_interpretation_extension|관찰\s*[·ㆍ-]?\s*해석\s*확장\s*축|관찰\s*[·ㆍ-]?\s*해석\s*확장|관찰\s*[·ㆍ-]?\s*해석|대상\s*관찰)/i.test(text)) return "observation_interpretation";
      if (/(explanatory_recording_extension|설명\s*[·ㆍ-]?\s*기록\s*확장\s*축|설명\s*[·ㆍ-]?\s*기록\s*확장|설명\s*[·ㆍ-]?\s*기록|정보\s*기록)/i.test(text)) return "explanatory_recording";
      return "";
    };
    const axisId = fromText(activeAxisText);
    if (axisId) return axisId;
    ...
    // 화면에 3개 후보가 함께 보이는 대기 상태에서는 실제 첫 카드인 서정 갈래 기준으로만 기본값을 둔다.
    // 비판적 읽기와 토론형은 한문학과 실제 화면 기준이 아니므로 여기서 사용하지 않는다.
    if (/서정\s*갈래와\s*시적\s*표현/i.test(pageConceptText) && /서사\s*[·ㆍ-]?\s*극\s*갈래와\s*이야기\s*구성/i.test(pageConceptText) && /교술\s*갈래와\s*성찰적\s*표현/i.test(pageConceptText)) return "lyric_appreciation";
  ...
  function cloneBookForA48ClassicalChineseLock(book, ctx, sectionType, axisId, rank){
    ...
    const axisLabel = axisLabelMap[axisId] || "한문학과 선택 축";
    return {
      ...book,
      matchType: sectionType,
      matchScore: 6040 - rank * 10,
      ...
      selectedBookContext: {
        ...lockedContext,
        recommendationReason: sectionType === "direct"
          ? `${book.title}은(는) ${axisLabel}에서 한문 고전·문헌·사상 텍스트를 해석하는 직접 연결 도서입니다.`
          : `${book.title}은(는) ${axisLabel}에서 한자 문화권의 서사, 사상, 기록, 사회적 의미로 탐구를 넓히는 확장 참고 도서입니다.`,
        matchReasons: uniq(arr(lockedContext.matchReasons).concat([`한문학과 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서`]))
      },
      bookA48ClassicalChineseLock: true,
      bookA48ClassicalChineseRank: rank,
      bookA48ClassicalChineseAxisLock: axisId
    };
  }

  function applyBookA48ClassicalChineseHardLock(result, ctx){
    ...
    const directMap = {
      ...
      narrative_structure: ["사기열전", "삼국유사", "논어"],
      storytelling_media: ["삼국유사", "수호전", "사기열전"],
      character_conflict: ["사기열전", "수호전", "맹자"],
      reflective_writing: ["논어", "맹자", "성호사설"],
      observation_interpretation: ["성호사설", "사기열전", "삼국유사"],
    ...
    const expansionMap = {
      ...
      narrative_structure: ["수호전", "인물로 읽는 장자", "시학", "문학과 예술의 사회사", "열하일기"],
      storytelling_media: ["사기열전", "미디어의 이해", "문학과 예술의 사회사", "열하일기", "논어"],
      character_conflict: ["삼국유사", "인물로 읽는 장자", "국가", "시학", "성호사설"],
      reflective_writing: ["인물로 읽는 장자", "아함경", "국가", "시학", "정지용전집"],
    ...
    const directIds = new Set(directBooks.map(book => bookKey(book)));
    const expansionBooks = expansionTitles.map((title, index) =>
      cloneBookForA48ClassicalChineseLock(findBookForLock(title, result), ctx, "expansion", axisId, index + 1)
    ).filter(book => book && !directIds.has(bookKey(book))).slice(0, 5);
    return {
      ...result,
      directBooks,
      ...
      debug: {
        ...
        bookA48ClassicalChineseVersion: "v210",
  ...
  function getBookA50IntlTradeVisiblePageText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (text) parts.push(text);
  ...
  function getBookA50IntlTradeVisibleActiveTrackText(){
    const parts = [];
    const push = function(value){
      const text = normalizeLockText(value || "");
      if (!text) return;
  ...
  function isBookA50IntlTradeContext(ctx){
    ctx = ctx || {};
    const pageText = getBookA50IntlTradeVisiblePageText();
    const careerText = normalizeLockText([
      ctx.career, ctx.selectedMajor, ctx.department, ctx.major, ctx.majorName, ctx.majorSelectedName, pageText
    ].join(" "));
    const subjectConceptText = normalizeLockText([
    ...
    const isIntlTradeMajor = /(국제통상학과|무역학과)/i.test(careerText);
    const isActualBusinessSocietyFlow = /(통합\s*사회1|통합사회1|통합사회Ⅰ|통합\s*사회|통합사회|시장\s*경제와\s*금융\s*생활|미래와\s*지속\s*가능한\s*삶|문화\s*다양성과\s*세계화|합리적\s*선택|기회비용|시장\s*구조|가격\s*변동|금융\s*생활|위험관리|지속가능\s*경영|ESG|미래\s*산업|소비\s*변화|세계화|국제무역|소비문화|글로벌\s*마케팅)/i.test(subjectConceptText);
    return !!(isIntlTradeMajor && isActualBusinessSocietyFlow);
  }

  function inferBookA50IntlTradeAxis(ctx){
    ctx = ctx || {};
    const activeAxisText = normalizeLockText([
      getBookA50IntlTradeVisibleActiveTrackText(),
      ctx.axisLabel,
      ctx.followupAxisId,
      ctx.linkTrack,
      ctx.axisDomain,
    ...
    const conceptText = normalizeLockText([
      ...
      ctx.keyword,
      ctx.selectedKeyword,
      getBookA50IntlTradeVisiblePageText()
    ].join(" "));
    const fromText = function(text){
      text = normalizeLockText(text || "");
      if (!text) return "";
      ...
      if (/(business_finance_risk|금융\s*생활\s*[·ㆍ-]?\s*위험관리\s*축|금융\s*생활|위험관리)/i.test(text)) return "finance_risk";
      ...
      if (/(business_data_decision|데이터\s*기반\s*경영\s*의사결정\s*축|데이터\s*기반|경영\s*의사결정)/i.test(text)) return "business_data";
    ...
    const axisId = fromText(activeAxisText);
    if (axisId) return axisId;
    if (/문화\s*다양성과\s*세계화/i.test(conceptText)) return "global_trade";
    if (/미래와\s*지속\s*가능한\s*삶/i.test(conceptText)) return "esg_sustainability";
    if (/시장\s*경제와\s*금융\s*생활/i.test(conceptText)) return "market_price";
    return "global_trade";
  }

  function cloneBookForA50IntlTradeLock(book, ctx, sectionType, axisId, rank){
    if (!book) return null;
    const lockedContext = buildLockedBookContextA26Humanities(book, ctx, sectionType, axisId, rank);
    const axisLabelMap = {
      business_choice: "합리적 선택·기회비용 분석 축",
    ...
    const axisLabel = axisLabelMap[axisId] || "국제통상·무역 선택 축";
    return {
      ...book,
      matchType: sectionType,
      matchScore: 6040 - rank * 10,
      matchReasons: uniq(arr(book.matchReasons).concat([`A-50 국제통상·무역 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서 잠금`])),
      selectedBookContext: {
        ...lockedContext,
        recommendationReason: sectionType === "direct"
          ? `${book.title}은(는) ${axisLabel}에서 국제 거래, 세계화, 시장 구조, 공급망과 소비 변화를 해석하는 직접 연결 도서입니다.`
          : `${book.title}은(는) ${axisLabel}에서 문화 차이, 윤리적 소비, 불평등, 정책 쟁점으로 탐구를 확장하는 참고 도서입니다.`,
        matchReasons: uniq(arr(lockedContext.matchReasons).concat([`국제통상·무역 ${sectionType === "direct" ? "직접 일치" : "확장 참고"} 도서`]))
      },
      bookA50IntlTradeLock: true,
      bookA50IntlTradeRank: rank,
      bookA50IntlTradeAxisLock: axisId
    };
  }

  function applyBookA50IntlTradeHardLock(result, ctx){
    if (!result || !isBookA50IntlTradeContext(ctx)) return result;
    const axisId = inferBookA50IntlTradeAxis(ctx);
    const directMap = {
      ...
      consumer_marketing: ["국화와 칼", "오리엔탈리즘", "문명의 충돌"]
    };
    const expansionMap = {
      business_choice: ["공정하다는 착각", "성호사설", "프로테스탄트 윤리와 자본주의 정신", "자본론", "정의란 무엇인가"],
      market_price: ["프로테스탄트 윤리와 자본주의 정신", "자본론", "성호사설", "공정하다는 착각", "왜 세계의 절반은 굶주리는가"],
      finance_risk: ["자본론", "공정하다는 착각", "성호사설", "프로테스탄트 윤리와 자본주의 정신", "왜 세계의 절반은 굶주리는가"],
      esg_sustainability: ["공정하다는 착각", "프로테스탄트 윤리와 자본주의 정신", "자본론", "성호사설", "정의란 무엇인가"],
      future_industry: ["멋진 신세계", "1984", "미디어의 이해", "감시와 처벌", "공정하다는 착각"],
      business_data: ["미디어의 이해", "1984", "감시와 처벌", "공정하다는 착각", "물질문명과 자본주의"],
      global_trade: ["국부론", "문명의 충돌", "오리엔탈리즘", "공정하다는 착각", "돈으로 살 수 없는 것들"],
      consumer_marketing: ["사피엔스", "서유견문", "돈으로 살 수 없는 것들", "미디어의 이해", "제3의 물결"]
    };
    const directTitles = arr(directMap[axisId] || directMap.global_trade);
    const expansionTitles = arr(expansionMap[axisId] || expansionMap.global_trade);
    const directBooks = directTitles.map((title, index) =>
      cloneBookForA50IntlTradeLock(findBookForLock(title, result), ctx, "direct", axisId, index + 1)
    ).filter(Boolean).slice(0, 3);
    if (!directBooks.length) return result;
    const directIds = new Set(directBooks.map(book => bookKey(book)));
    const expansionBooks = expansionTitles.map((title, index) =>
      cloneBookForA50IntlTradeLock(findBookForLock(title, result), ctx, "expansion", axisId, index + 1)
    ).filter(book => book && !directIds.has(bookKey(book))).slice(0, 5);
    return {
      ...result,
      directBooks,
  ...
  function getGlobalBookSearchResults(rawQuery, excludedIds, limit){
    ...
    return master.books
      ...
      .slice(0, limit || 24)
      .map(item => ({ ...item.book, __globalSearchHit: true }));
  }
  ...
  global.renderBookSelectionHTML = function(ctx){
    ...
    result = applyBookA41ChineseLiteratureHardLock(result, ctx);
  ...
  function installBridgeBookClickHandler(){
    ...
    document.addEventListener("click", function(event){
      ...
      try {
        const state = global.__TEXTBOOK_HELPER_STATE__;
        if (state) state.selectedBookSource = source;
      } catch(e){}
