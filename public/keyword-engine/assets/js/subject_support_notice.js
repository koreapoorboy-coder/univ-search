(function(global){
  ...
  const VERSION = "subject-support-notice-v1.4.0";
  ...
  const THIN_SEED = new Set([
    "공통수학1", "공통수학2", "지구과학",
    "통합사회1", "통합사회2", "지구시스템과학",
    // 한국사는 2026-09-20 에 단원 사전과 논문 묶음을 넣어 보고서가 만들어진다. 다만 이 과목 전용
    // 런타임 후보(assessment_seed_cross_axis)는 아직 0건이라 「확충 중」 안내를 보여 준다 — 「준비 중」이
    // 아니다. 되는 과목을 안 된다고 적어 두면 학생이 그냥 나간다.
    // **영어는 목록에서 뺐다**(2026-09-25) — 고를 수 없으므로 안내할 것도 없다.
    "한국사",
    // 국어는 2026-09-22 에 다시 열었다. 단원 사전은 이미 다른 과목만큼 있었고(공통국어1 8개),
    // 서평 틀도 있었다. 막혀 있던 것은 사이트가 서평 과제에 실험 뼈대를 보내던 것이고,
    // 그것을 막았다(report_stages_v1 의 실험 절 검사). 다만 국어 전용 런타임 사례는 아직 0건이라
    // 「확충 중」 안내를 보여 준다 — 한국사와 같은 자리다.
    "공통국어1", "공통국어2"
  ]);
