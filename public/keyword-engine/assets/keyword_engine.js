window.__KEYWORD_ENGINE_VERSION = "admissions-v17-student-writable";
...
async function callGenerateAPI(payload){
  const response=await fetch(`${WORKER_BASE_URL}/generate`,{
    ...
    body:JSON.stringify({
      keyword:payload.keyword, grade:payload.grade, track:payload.track, major:payload.major,
      style:payload.style, activityLevel:payload.activityLevel, subject:payload.subject,
      taskType:payload.taskType, taskDescription:payload.taskDescription, career:payload.career
    })
  });
...
function scoreExtensionTemplate(template,context){
  ...
  toArray(template.fit_conditions?.required_any_keywords).forEach(keyword=>{
    const k=normalizeText(keyword); if(haystack.some(item=>item.includes(k)||k.includes(item))) score+=8;
  });
  toArray(template.fit_conditions?.preferred_subjects).forEach(subject=>{
    const s=normalizeText(subject); if([context.subject,...(context.subjectLinks||[])].map(normalizeText).some(item=>item.includes(s)||s.includes(item))) score+=7;
  });
  toArray(template.fit_conditions?.preferred_methods).forEach(method=>{
    const m=normalizeText(method); if([context.taskType,context.taskDescription].map(normalizeText).some(item=>item.includes(m)||m.includes(item))) score+=5;
  });
  toArray(template.theme_tags).forEach(tag=>{
    const t=normalizeText(tag); if(haystack.some(item=>item.includes(t)||t.includes(item))) score+=3;
  });
...
function defaultFinalDecision(payload,assessmentReference){
  ...
  const modeMap={research:"자료조사형 비교 보고서",analysis:"자료분석형 비교 보고서",design:"설계형 아이디어 보고서",experiment:"실험형 비교 보고서"};
  return {
    mode:modeMap[topMode]||"자료조사형 비교 보고서",
    topic:`${payload.keyword}를 ${payload.subject} 개념과 연결해 비교·분석하기`,
    action:["비교 대상 2개 정하기","비교 기준 3개 정하기","보고서 틀에 맞춰 정리하기"]
  };
}
function buildStudentReport(payload, apiData, assessmentReference){
  const result=apiData.result||{};
  const finalDecision=result.finalDecision||defaultFinalDecision(payload,assessmentReference);
  const frame=toArray(result.reportFrame).length?toArray(result.reportFrame):["탐구 동기","교과 개념 정리","비교 또는 자료 분석","결론"];
  const steps=toArray(result.steps).slice(0,3);
  const links=toArray(result.subjectLinks).slice(0,2);
  const warnings=toArray(result.warnings).slice(0,2);

  const topic=finalDecision.topic||`${payload.keyword} 관련 탐구`;
  const compareA = payload.keyword.includes("이차전지") ? "리튬이온전지" : "사례 1";
  const compareB = payload.keyword.includes("이차전지") ? "전고체배터리" : "사례 2";
  const criteria = payload.subject.includes("과학") ? ["구조","성능","안전성"] : ["특징","장점","한계"];
  ...
  return `
    ...
    <h4>2. 주제 선정 동기</h4>
    <p>${escapeHtml(payload.subject)} 과목에서 배우는 내용과 ${escapeHtml(payload.career)} 진로를 연결해 보고자 했다. 특히 ${escapeHtml(payload.keyword)}는 실생활과 연결성이 높아 수행평가 주제로 적합하다고 판단했다. 이번 탐구에서는 단순 정리가 아니라 비교와 분석을 통해 핵심 차이를 정리하고자 한다.</p>

    <h4>3. 탐구 방법</h4>
    <ul>
      ${toArray(finalDecision.action).map(v=>`<li>${escapeHtml(v)}</li>`).join("")}
    </ul>
    ...
    <table>
      ...
      <tbody>
        <tr><td>비교 대상</td><td>${escapeHtml(compareA)} / ${escapeHtml(compareB)}</td></tr>
        <tr><td>비교 기준</td><td>${escapeHtml(criteria.join(" / "))}</td></tr>
        <tr><td>연결 교과</td><td>${escapeHtml(links.join(" / ") || payload.subject)}</td></tr>
      </tbody>
    </table>

    <h4>5. 탐구 내용 작성 틀</h4>
    <ul>
      ${frame.map(v=>`<li>${escapeHtml(v)}</li>`).join("")}
    </ul>

    <h4>6. 예상 결론</h4>
    <p>${escapeHtml(result.reason || `${topic}를 비교하면 각 대상의 특징과 차이를 분명하게 정리할 수 있고, 이를 ${payload.career} 진로와 연결해 의미를 해석할 수 있다.`)}</p>

    <h4>7. 작성할 때 주의할 점</h4>
    <ul>
      ${warnings.map(v=>`<li>${escapeHtml(v)}</li>`).join("")}
    </ul>
  `;
}
...
function renderStudentView(payload, apiData, textbookMatches, extensionMatches, assessmentReference){
  ...
  $("actionSteps").innerHTML=actions.map((step,idx)=>`<div class="step-item"><div class="step-no">${idx+1}</div><div>${escapeHtml(step)}</div></div>`).join("");
  ...
  $("reportFrameCard").innerHTML=`<h3>보고서 틀</h3><div class="frame-list">${reportFrame.slice(0,4).map((item,idx)=>`<div class="frame-item"><b>${idx+1}</b><div>${escapeHtml(item)}</div></div>`).join("")}</div>`;
  ...
  $("inputSummaryCard").innerHTML=`<h3>이번 입력</h3><div class="mini-block-list"><div class="mini-item"><b>과목</b><div>${escapeHtml(payload.subject)}</div></div><div class="mini-item"><b>수행 형태</b><div>${escapeHtml(payload.taskType)}</div></div><div class="mini-item"><b>진로</b><div>${escapeHtml(payload.career)}</div></div><div class="mini-item"><b>키워드 / 학년</b><div>${escapeHtml(payload.keyword)} / ${escapeHtml(payload.grade)}</div></div></div>`;

  $("studentReport").innerHTML=buildStudentReport(payload, apiData, assessmentReference);
...
function handleReset(){
  ["subject","taskType","taskDescription","career","keyword","grade"].forEach(id=>{const el=$(id); if(el) el.value="";});
  clearError(); clearResults();
}
