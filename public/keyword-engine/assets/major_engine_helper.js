window.__MAJOR_SEARCH_FAST_GUARD_VERSION = 'v78-major-search-direct-rollback';
...
(function(){
  ...
  const CANDIDATE_PROFILES = {
    ...
    '에너지공학과': { track:'에너지·시스템', keywords:['에너지','전환','저장','효율','배터리','신재생'], aliases:['에너지','에너지공','에너지공학','에너지공학과','신재생','신재생에너지','에너지시스템'] },
  ...
  const QUERY_GROUPS = [
    ...
    { test: /(에너지공학|에너지공|에너지|신재생|신재생에너지|에너지시스템)/, names:['에너지공학과','전기공학과','화학공학과','신소재공학과'] },
    { test: /(신소재|소재|재료|배터리|이차전지)/, names:['신소재공학과','화학공학과','반도체공학과','에너지공학과'] },
  ...
  const STRICT_QUERY_ORDER = [
    ...
    { test: /^(에너지|에너지공|에너지공학|에너지공학과|신재생|신재생에너지)$/, names:['에너지공학과','전기공학과','화학공학과','신소재공학과'] }
  ];
  ...
  function renderCandidates(raw){
    // v78: 후보 카드 방식은 사용성이 떨어져 직접 반영형으로 되돌린다.
    // 입력 중 후보 패널은 숨기고, 아래 autoConfirmFromInput()이 가장 가까운 학과를 자동 확정한다.
  ...
  function shouldAutoConfirm(raw, row){
    const nq = norm(raw);
    if (!nq || !row) return false;
    if (nq.length < 2 && !/^[A-Za-z]{2,}$/.test(String(raw || '').trim())) return false;
    if (row.score >= 85) return true;
    return false;
  }

  function scheduleAutoConfirm(raw){
    clearTimeout(state.renderTimer);
    state.renderTimer = setTimeout(() => {
      const input = getInput();
      if (!input) return;
      const current = String(input.value || '').trim();
      if (norm(current) !== norm(raw)) return;
      const rows = findCandidates(current);
      const top = rows[0];
      if (!shouldAutoConfirm(current, top)) return;
      confirmMajor(top.name, { source: 'auto-direct' });
    }, 120);
  }
  ...
  function confirmMajor(name, options = {}){
    const input = getInput();
    ...
    window.__MAJOR_ENGINE_SELECTED__ = {
      ...
      comparison: null,
      source: options.source || 'major_search_direct_rollback'
    };
    input.value = name;
    const panel = ensurePanel();
    if (panel) {
