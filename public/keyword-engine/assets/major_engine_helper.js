...
(function(){
  ...
  const DIRECT_QUERY_MAJOR_MAP = {
    // v79: major_search_fast_guard.js 없이 native major_engine_helper 내부에서 직접 확정할 약칭만 관리한다.
    // broad 후보 카드가 필요한 검색어는 DIRECT_QUERY_BROAD_MAP에 남겨둔다.
    '컴공': ['컴퓨터공학과'],
    '컴퓨터': ['컴퓨터공학과'],
    '컴퓨터공': ['컴퓨터공학과'],
    '컴퓨터공학': ['컴퓨터공학과'],
...
Object.assign(MAJOR_COPY_OVERRIDES, {
  ...
  // v82 E-line bio/food/pharma/cosmetic major data connection lock
  // - UI/report flow is not changed.
  // - Keeps v81 D-line rehab fixes intact.
  // - Locks representative routing for broad E-line queries before generic fuzzy scoring.
  Object.assign(DIRECT_QUERY_MAJOR_MAP, {
    '생명': ['생명과학과'],
    '생명과': ['생명과학과'],
    '생명과학': ['생명과학과'],
  ...
  Object.assign(DIRECT_QUERY_BROAD_MAP, {
    '생명계열': ['생명과학과','생명공학과','생물학과','미생물학과','식품생명공학과'],
    '바이오계열': ['생명공학과','생명과학과','미생물학과','식품생명공학과','제약공학과'],
    '식품계열': ['식품공학과','식품생명공학과','식품영양학과','식품자원경제학과'],
    '제약약학': ['제약공학과','약학과','한약학과','생명공학과','화학과'],
    '약학계열': ['약학과','제약공학과','한약학과','화학과','생명공학과']
  });

  ['생명계열','바이오계열','식품계열','제약약학','약학계열'].forEach(keyword => BROAD_QUERY_KEYWORDS.add(keyword));

  QUERY_BOOST_RULES.unshift(
    { queries:['생명'], test: /(생명과학과|생명공학과|생물학과|미생물학과|식품생명공학과)/, boost: 86 },
    { queries:['생명과학','생명과'], test: /(생명과학과|생물학과|미생물학과|생명공학과)/, boost: 92 },
    { queries:['생물','생물학'], test: /(생물학과|생명과학과|미생물학과|생명공학과)/, boost: 90 },
    { queries:['미생물','미생물학'], test: /(미생물학과|생명과학과|생물학과|생명공학과)/, boost: 92 },
    { queries:['생명공학','생명공','바이오','바이오공학','바이오공'], test: /(생명공학과|생명과학과|미생물학과|식품생명공학과|제약공학과|화공생명공학과)/, boost: 92 },
    { queries:['식품'], test: /(식품공학과|식품영양학과|식품생명공학과|식품자원경제학과)/, boost: 92 },
    { queries:['식품공학','식품공'], test: /(식품공학과|식품생명공학과|식품영양학과)/, boost: 94 },
    { queries:['식품생명','식품생명공학','식품생명공'], test: /(식품생명공학과|식품공학과|생명공학과|미생물학과)/, boost: 94 },
    { queries:['식품영양','영양'], test: /(식품영양학과|식품공학과|보건관리학과)/, boost: 94 },
    { queries:['식품자원','식품자원경제','식품경제'], test: /(식품자원경제학과|농업경제학과|식품공학과)/, boost: 94 },
    { queries:['제약','제약공학','제약공','바이오제약'], test: /(제약공학과|약학과|생명공학과|화학공학과|화공생명공학과)/, boost: 94 },
    { queries:['약','약학'], test: /(약학과|제약공학과|한약학과|화학과|생명공학과)/, boost: 94 },
    { queries:['한약','한약학'], test: /(한약학과|약학과|제약공학과|화학과)/, boost: 94 },
    { queries:['화장품','화장품공학','화장품공'], test: /(화장품공학과|화학공학과|화학과|제약공학과)/, boost: 94 },
    { queries:['화공생명','화공생명공학','화공생명공'], test: /(화공생명공학과|화학공학과|생명공학과|식품생명공학과)/, boost: 94 }
  );

  Object.assign(MAJOR_COPY_OVERRIDES, {
    '생명과학과': {
      card: '세포, 유전, 진화, 생태 같은 생명 현상의 원리를 실험과 데이터로 탐구하는 기초 생명과학 학과입니다.',
      fit: '응용 기술보다 생명 현상 자체의 원리와 실험 자료 해석에 관심 있는 학생에게 잘 맞습니다.',
      intro: '생명과학과는 세포, 유전, 진화, 생태 같은 생명 현상의 원리를 이론과 실험으로 탐구하는 기초과학 중심 학과입니다.',
      subjects: ['생명과학','화학','공통수학1','정보','통합과학1'],
      topics: ['유전 정보 발현 과정이 생명체 기능에 미치는 영향 탐구', '효소 활성과 세포 대사 조건을 실험 설계로 비교', '생태계 변화와 생물다양성 감소 원인 분석'],
      group_label: '바이오·생명과학',
      track_category: '세포/유전/생명탐구',
      search_aliases: ['생명','생명과학','의생명과학'],
      compare_profiles: [
        { display_name: '생명공학과', track_category: '유전자/세포/바이오기술', focus: '세포·유전자·미생물을 활용해 의약, 식품, 환경 기술로 연결하는 응용 중심 학과입니다.', hint: '기초 원리를 산업 기술과 실험 응용으로 확장하고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '생물학과', track_category: '생물/분류/생태탐구', focus: '생물의 구조, 분류, 생태, 진화와 다양성을 폭넓게 탐구하는 학과입니다.', hint: '세포·유전뿐 아니라 생물 다양성과 생태를 넓게 보고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '미생물학과', track_category: '미생물/세균/바이러스', focus: '세균, 곰팡이, 바이러스 같은 미생물의 구조와 생장, 반응 원리를 실험으로 배우는 학과입니다.', hint: '생명 현상 중 미생물과 감염·발효·환경 미생물에 더 관심 있는 학생에게 잘 맞습니다.' }
      ]
    },
    '생물학과': {
      card: '생물의 구조, 분류, 생태, 진화와 다양성을 폭넓게 탐구하는 학과입니다.',
      fit: '생물 개체와 생태, 진화와 다양성까지 넓게 보고 싶은 학생에게 잘 맞습니다.',
      intro: '생물학과는 생물의 구조와 기능, 분류, 생태, 진화, 다양성을 실험과 관찰 자료로 이해하는 기초 자연과학 학과입니다.',
      subjects: ['생명과학','화학','통합과학1','공통수학1','정보'],
      topics: ['생물 분류 기준이 생태 이해에 미치는 영향 분석', '환경 변화가 생물 다양성에 미치는 영향 탐구', '개체군 변화 자료를 활용한 생태계 해석'],
      group_label: '바이오·생명과학',
      track_category: '생물/분류/생태탐구',
      search_aliases: ['생물','생물학'],
      compare_profiles: [
        { display_name: '생명과학과', track_category: '세포/유전/생명탐구', focus: '세포, 유전, 진화, 생태 같은 생명 현상의 원리를 실험과 데이터로 탐구하는 학과입니다.', hint: '생물 다양성보다 세포·유전 원리를 더 깊게 보고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '미생물학과', track_category: '미생물/세균/바이러스', focus: '미생물의 구조와 생장, 반응 원리를 실험으로 배우는 학과입니다.', hint: '생물 중 미생물과 감염·발효 현상에 집중하고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '생명공학과', track_category: '유전자/세포/바이오기술', focus: '생명 현상을 의약, 식품, 환경 기술로 연결하는 응용 중심 학과입니다.', hint: '기초 생물 지식을 기술 응용으로 확장하고 싶은 학생에게 잘 맞습니다.' }
      ]
    },
    '미생물학과': {
      card: '세균, 곰팡이, 바이러스 같은 미생물의 구조와 생장, 반응 원리를 실험으로 배우는 학과입니다.',
      fit: '감염, 발효, 환경 미생물처럼 작은 생명체가 실제 문제에 미치는 영향을 탐구하고 싶은 학생에게 잘 맞습니다.',
      intro: '미생물학과는 세균, 곰팡이, 바이러스 등 미생물의 구조와 생장, 반응, 활용 가능성을 실험 중심으로 배우는 학과입니다.',
      subjects: ['생명과학','화학','정보','통합과학1','공통수학1'],
      topics: ['배양 조건 변화가 미생물 생장에 미치는 영향 분석', '발효 미생물이 식품 품질에 미치는 효과 비교', '감염 확산과 미생물 관리 기준 탐구'],
      group_label: '바이오·생명공학',
      track_category: '미생물/발효/감염관리',
      search_aliases: ['미생물','미생물학'],
      compare_profiles: [
        { display_name: '생명과학과', track_category: '세포/유전/생명탐구', focus: '생명 현상의 원리를 실험과 데이터로 탐구하는 기초과학 중심 학과입니다.', hint: '미생물보다 생명 현상 전체를 넓게 보고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '생물학과', track_category: '생물/분류/생태탐구', focus: '생물의 구조, 분류, 생태, 진화와 다양성을 폭넓게 탐구하는 학과입니다.', hint: '미생물보다 생물 다양성과 생태를 넓게 보고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '생명공학과', track_category: '유전자/세포/바이오기술', focus: '미생물과 세포 기술을 의약·식품·환경 산업으로 연결하는 응용 중심 학과입니다.', hint: '미생물 원리를 산업 기술로 확장하고 싶은 학생에게 잘 맞습니다.' }
      ]
    },
    '생명공학과': {
      card: '세포·유전자·미생물을 활용해 의약, 식품, 환경 기술로 연결하는 응용 중심 학과입니다.',
      fit: '생명 현상을 실험으로 확인하고 기술과 산업으로 확장해 보고 싶은 학생에게 잘 맞습니다.',
      intro: '생명공학과는 세포, 유전자, 단백질, 미생물 같은 생명 시스템을 이해하고 이를 의약, 식품, 환경, 산업 기술로 연결하는 응용 중심 학과입니다.',
      subjects: ['생명과학','화학','정보','공통수학1','통합과학1'],
      topics: ['유전자 편집 기술의 가능성과 윤리 문제를 함께 탐구', '세포 배양 기술이 의료 연구와 바이오산업에 활용되는 방식 비교', '미생물 활용 기술이 식품·환경 산업에 미치는 영향 분석'],
      group_label: '바이오·생명공학',
      track_category: '유전자/세포/바이오기술',
      search_aliases: ['생명공','생명공학','바이오','바이오공학','의생명공학'],
      compare_profiles: [
        { display_name: '생명과학과', track_category: '세포/유전/생명탐구', focus: '생명 현상의 원리를 실험과 데이터로 탐구하는 기초과학 중심 학과입니다.', hint: '기술 응용보다 생명 현상 자체의 원리를 깊게 파고들고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '미생물학과', track_category: '미생물/발효/감염관리', focus: '미생물의 구조와 생장, 반응 원리를 실험으로 배우는 학과입니다.', hint: '세포·유전자 기술 중 미생물 활용에 특히 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '식품생명공학과', track_category: '식품/발효/기능성소재', focus: '식품과 생명과학을 바탕으로 안전성, 발효, 기능성 소재를 연구하는 학과입니다.', hint: '바이오 기술을 식품 산업과 건강 문제로 연결하고 싶은 학생에게 잘 맞습니다.' }
      ]
    },
    '화공생명공학과': {
      card: '화학공정과 생명기술을 결합해 의약품, 식품, 에너지, 소재 생산 과정을 설계하는 융합 학과입니다.',
      fit: '생명기술을 실험실 수준에서 끝내지 않고 대규모 생산 공정과 산업 설계로 연결하고 싶은 학생에게 잘 맞습니다.',
      intro: '화공생명공학과는 화학공정과 생명공학을 결합해 의약품, 식품, 에너지, 소재 생산 과정을 설계하는 융합 학과입니다.',
      subjects: ['화학','생명과학','공통수학1','정보','통합과학1'],
      topics: ['바이오 공정과 화학 공정의 차이를 생산 관점에서 비교', '의약품 생산 공정에서 중요한 설계 요소 탐구', '생명공학 기술이 화학 산업과 소재 개발에 미치는 영향 분석'],
      group_label: '바이오·생명공학',
      track_category: '화학공정/바이오생산/소재',
      search_aliases: ['화공생명','화공생명공','화공생명공학','바이오화공','바이오화학공학'],
      compare_profiles: [
        { display_name: '화학공학과', track_category: '화학공정/반응/생산설계', focus: '화학 반응과 물질전달, 공정 설계를 바탕으로 소재·에너지·생산 시스템을 다루는 학과입니다.', hint: '생명기술보다 화학 반응과 공정 설계 자체에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '생명공학과', track_category: '유전자/세포/바이오기술', focus: '세포·유전자·미생물을 활용해 의약, 식품, 환경 기술로 연결하는 응용 중심 학과입니다.', hint: '대규모 공정보다 생명기술 실험과 응용에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '식품생명공학과', track_category: '식품/발효/기능성소재', focus: '식품과 생명과학을 바탕으로 안전성, 발효, 기능성 소재를 연구하는 학과입니다.', hint: '바이오 공정을 식품과 기능성 소재로 연결하고 싶은 학생에게 잘 맞습니다.' }
      ]
    },
    '식품공학과': {
      card: '식품의 가공, 저장, 발효, 품질관리, 생산 시스템을 공학적으로 배우는 학과입니다.',
      fit: '먹거리를 조리나 영양만이 아니라 공정, 안전성, 품질관리 관점에서 보고 싶은 학생에게 잘 맞습니다.',
      intro: '식품공학과는 식품의 가공, 저장, 발효, 품질관리, 생산 시스템을 배우며 먹거리를 공학적으로 다루는 학과입니다.',
      subjects: ['화학','생명과학','정보','공통수학1','통합과학1'],
      topics: ['식품 저장 조건이 품질 유지에 미치는 영향 비교', '발효 공정이 식품 안전성과 맛에 미치는 효과 탐구', '식품 가공 방식에 따른 영양 성분 변화 분석'],
      group_label: '식품·농업·환경생명',
      track_category: '식품/가공/품질관리',
      search_aliases: ['식품','식품공','식품공학'],
      compare_profiles: [
        { display_name: '식품생명공학과', track_category: '식품/발효/기능성소재', focus: '식품과 생명과학을 바탕으로 안전성, 발효, 기능성 소재를 연구하는 학과입니다.', hint: '식품 공정을 생명과학과 기능성 소재로 확장하고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '식품영양학과', track_category: '영양/식품/건강관리', focus: '식품의 성분과 조리, 영양 설계, 건강 증진과 식생활 관리의 관계를 함께 배우는 학과입니다.', hint: '생산 공정보다 사람의 식생활과 영양 관리에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '식품자원경제학과', track_category: '식품자원/경제/유통정책', focus: '식품과 자원, 유통과 시장 구조를 경제 관점에서 배우는 학과입니다.', hint: '식품 자체보다 자원·유통·시장 구조에 더 관심 있는 학생에게 잘 맞습니다.' }
      ]
    },
    '식품생명공학과': {
      card: '식품과 생명과학을 바탕으로 안전성, 발효, 기능성 소재를 연구하는 학과입니다.',
      ...
      intro: '식품생명공학과는 식품의 제조, 가공, 안전성, 영양, 발효, 기능성 소재를 생명과학과 공학 관점에서 함께 다루는 학과입니다.',
      subjects: ['생명과학','화학','정보','공통수학1','통합과학1'],
      topics: ['기능성 식품의 원리와 건강 효과 비교', '발효 공정이 식품 품질과 안전성에 미치는 효과 탐구', '푸드테크가 식품 산업과 소비 방식에 미치는 영향 분석'],
      group_label: '식품·농업·환경생명',
      track_category: '식품/발효/기능성소재',
      search_aliases: ['식품생명','식품생명공','식품생명공학','바이오식품'],
      compare_profiles: [
        { display_name: '식품공학과', track_category: '식품/가공/품질관리', focus: '식품 원료를 가공하고 보존하며 품질과 안전성을 관리하는 공정 중심 학과입니다.', hint: '생명과학보다 식품 생산 공정과 품질관리에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '생명공학과', track_category: '유전자/세포/바이오기술', focus: '세포·유전자·미생물을 활용해 의약, 식품, 환경 기술로 연결하는 응용 중심 학과입니다.', hint: '식품보다 생명기술 전반으로 확장하고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '미생물학과', track_category: '미생물/발효/감염관리', focus: '세균, 곰팡이, 바이러스 같은 미생물의 구조와 생장, 반응 원리를 실험으로 배우는 학과입니다.', hint: '식품 발효와 미생물 활용 원리를 더 깊게 보고 싶은 학생에게 잘 맞습니다.' }
      ]
    },
    '식품영양학과': {
      card: '식품의 성분과 조리, 영양 설계, 건강 증진과 식생활 관리의 관계를 함께 배우는 학과입니다.',
      fit: '식품 생산 공정보다 사람의 건강, 식생활, 영양 관리에 관심 있는 학생에게 잘 맞습니다.',
      intro: '식품영양학과는 식품의 성분과 조리, 영양 설계, 건강 증진과 식생활 관리의 관계를 함께 배우는 학과입니다.',
      subjects: ['생명과학','화학','보건','가정','통합과학1'],
      topics: ['식단 구성 차이가 건강 지표에 미치는 영향 분석', '영양 성분 표시를 활용한 식품 선택 기준 탐구', '생활 습관과 영양 관리의 관계 비교'],
      group_label: '식품·농업·환경생명',
      track_category: '영양/식품/건강관리',
      search_aliases: ['식품영양','영양'],
      compare_profiles: [
        { display_name: '식품공학과', track_category: '식품/가공/품질관리', focus: '식품 원료를 가공하고 보존하며 품질과 안전성을 관리하는 공정 중심 학과입니다.', hint: '사람의 식생활보다 식품 생산 공정과 품질관리에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '보건관리학과', track_category: '보건/예방/건강관리', focus: '질병 예방, 건강 증진, 보건 정책, 의료행정과 건강 통계를 함께 배우는 학과입니다.', hint: '개인 영양 관리보다 지역사회 건강 관리와 보건 정책에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '식품생명공학과', track_category: '식품/발효/기능성소재', focus: '식품과 생명과학을 바탕으로 안전성, 발효, 기능성 소재를 연구하는 학과입니다.', hint: '영양 설계보다 식품 기능성 소재와 발효 연구에 더 관심 있는 학생에게 잘 맞습니다.' }
      ]
    },
    '식품자원경제학과': {
      card: '식품과 자원, 유통과 시장 구조를 경제 관점에서 배우는 학과입니다.',
      fit: '식품 생산 자체보다 식량 자원, 가격, 유통, 정책과 시장 구조를 함께 보고 싶은 학생에게 잘 맞습니다.',
      intro: '식품자원경제학과는 식품과 자원의 생산·유통·가격 구조를 경제와 정책 관점에서 배우는 학과입니다.',
      subjects: ['경제','통합사회','공통수학1','정보','사회와 문화'],
      topics: ['식품 가격 변동이 생산자와 소비자에 주는 영향 분석', '유통 구조 차이가 지역 농산물 경쟁력에 미치는 효과 비교', '식량 정책 변화가 자원 관리에 주는 영향 탐구'],
      group_label: '식품·농업·환경생명',
      track_category: '식품자원/경제/유통정책',
      search_aliases: ['식품자원','식품자원경제','식품경제'],
      compare_profiles: [
        { display_name: '농업경제학과', track_category: '농업/경제/정책', focus: '농업 생산과 유통, 가격, 농촌 경제와 정책을 배우는 학과입니다.', hint: '식품 자원보다 농업 전반의 경제와 정책에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '식품공학과', track_category: '식품/가공/품질관리', focus: '식품 원료를 가공하고 보존하며 품질과 안전성을 관리하는 공정 중심 학과입니다.', hint: '시장 구조보다 식품 생산 공정과 품질관리에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '경제학과', track_category: '경제/시장/정책분석', focus: '시장 원리와 자원 배분, 경제 지표를 데이터와 이론으로 해석하는 학과입니다.', hint: '식품 분야를 넘어 시장과 경제 지표를 넓게 보고 싶은 학생에게 잘 맞습니다.' }
      ]
    },
    '제약공학과': {
      card: '의약품의 제형 설계, 약물전달, 생산 공정, 품질 관리를 공학적으로 다루는 학과입니다.',
      fit: '신약이 실제 약으로 생산되고 관리되는 과정, 품질관리와 공정 설계에 관심 있는 학생에게 잘 맞습니다.',
      intro: '제약공학과는 신약 후보 물질이 실제 의약품으로 만들어지는 과정에서 필요한 제형 설계, 약물전달, 생산 공정, 품질관리를 공학적으로 배우는 학과입니다.',
      subjects: ['화학','생명과학','공통수학1','정보','통합과학1'],
      topics: ['약물전달 시스템이 약효와 부작용에 미치는 영향 비교', '바이오의약품 생산 공정에서 품질관리 기준이 중요한 이유 탐구', '친환경 제약 생산 공정 설계 아이디어 분석'],
      group_label: '바이오·생명공학',
      track_category: '제약/약물전달/생산공정',
      search_aliases: ['제약','제약공','제약공학','바이오제약'],
      compare_profiles: [
        { display_name: '약학과', track_category: '약학/약물/조제', focus: '약물의 구조와 성질, 조제와 복약, 약효와 안전성을 배우는 학과입니다.', hint: '생산 공정보다 약물 작용과 복약, 약효와 안전성에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '생명공학과', track_category: '유전자/세포/바이오기술', focus: '세포·유전자·미생물을 활용해 의료·식품·환경 기술로 연결하는 학과입니다.', hint: '의약품 생산보다 생명기술 전반의 실험 응용에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '화학공학과', track_category: '화학공정/반응/생산설계', focus: '화학 반응과 물질전달, 공정 설계를 바탕으로 생산 시스템을 다루는 학과입니다.', hint: '약물보다 화학 반응과 산업 공정 설계 자체에 더 관심 있는 학생에게 잘 맞습니다.' }
      ]
    },
    '약학과': {
      card: '약물의 구조와 성질, 조제와 복약, 약효와 안전성을 배우는 약학 계열 학과입니다.',
      fit: '약이 몸에서 어떻게 작용하고 안전하게 사용되는지 화학과 생명과학 관점에서 보고 싶은 학생에게 잘 맞습니다.',
      intro: '약학과는 약물의 구조와 성질, 조제와 복약, 약효와 안전성을 배우며 의약품의 사용과 관리, 약물 작용 원리를 이해하는 학과입니다.',
      subjects: ['화학','생명과학','보건','공통수학1','정보'],
      topics: ['약물 구조 차이가 약효에 미치는 영향 분석', '복약 방식 변화가 안전성에 주는 효과 비교', '약물 전달 과정이 치료 효율에 미치는 영향 탐구'],
      group_label: '보건·임상',
      track_category: '약학/약물/조제',
      search_aliases: ['약','약학'],
      compare_profiles: [
        { display_name: '제약공학과', track_category: '제약/약물전달/생산공정', focus: '의약품의 개발·생산·품질 관리를 공정 관점에서 다루는 학과입니다.', hint: '약물 작용보다 의약품 생산 공정과 품질관리에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '한약학과', track_category: '한약/생약/약재이해', focus: '생약과 약재, 처방과 효능, 안전성을 배우는 약학 계열 학과입니다.', hint: '일반 의약품보다 자연 유래 약재와 생약에 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '화학과', track_category: '화학/분자/반응탐구', focus: '원자·분자 수준의 구조와 반응을 실험과 분석으로 탐구하는 기초과학 학과입니다.', hint: '약물 사용보다 물질 구조와 반응 원리를 더 깊게 보고 싶은 학생에게 잘 맞습니다.' }
      ]
    },
    '한약학과': {
      card: '생약과 약재, 처방과 효능, 안전성을 배우는 약학 계열 학과입니다.',
      fit: '자연 유래 약재와 약효를 전통 지식이 아니라 성분과 안전성 관점에서 이해하고 싶은 학생에게 잘 맞습니다.',
      intro: '한약학과는 생약과 약재의 성질, 처방과 효능, 안전성과 복용 구조를 배우는 약학 계열 학과입니다.',
      subjects: ['화학','생명과학','보건','공통수학1','정보'],
      topics: ['약재 조합 차이가 효능에 미치는 영향 분석', '생약 성분 변화가 안전성에 주는 효과 비교', '처방 기준 변화가 복용 방식에 미치는 영향 탐구'],
      group_label: '보건·임상',
      track_category: '한약/생약/약재이해',
      search_aliases: ['한약','한약학'],
      compare_profiles: [
        { display_name: '약학과', track_category: '약학/약물/조제', focus: '약물의 구조와 성질, 조제와 복약, 약효와 안전성을 배우는 학과입니다.', hint: '생약보다 일반 의약품과 약물 작용에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '제약공학과', track_category: '제약/약물전달/생산공정', focus: '의약품의 개발·생산·품질 관리를 공정 관점에서 다루는 학과입니다.', hint: '약재 이해보다 의약품 생산 공정과 품질관리로 확장하고 싶은 학생에게 잘 맞습니다.' },
        { display_name: '화학과', track_category: '화학/분자/반응탐구', focus: '원자·분자 수준의 구조와 반응을 실험과 분석으로 탐구하는 기초과학 학과입니다.', hint: '생약 성분의 화학적 원리를 더 깊게 보고 싶은 학생에게 잘 맞습니다.' }
      ]
    },
    '화장품공학과': {
      card: '화장품 소재와 제형, 피부 과학과 품질관리, 제품 개발 구조를 배우는 학과입니다.',
      fit: '뷰티 제품을 감각이나 미용이 아니라 소재, 제형, 피부 과학, 안정성 관점에서 보고 싶은 학생에게 잘 맞습니다.',
      intro: '화장품공학과는 화장품 소재와 제형, 피부 과학과 품질관리, 제품 개발 구조를 배우는 학과입니다.',
      subjects: ['화학','생명과학','정보','가정','공통수학1'],
      topics: ['제형 차이가 사용감과 안정성에 미치는 영향 분석', '소재 구조 변화가 흡수와 보존성에 주는 효과 비교', '품질관리 기준이 제품 신뢰도에 미치는 영향 탐구'],
      group_label: '화학·에너지·소재',
      track_category: '화장품/소재/제품개발',
      search_aliases: ['화장품','화장품공','화장품공학'],
      compare_profiles: [
        { display_name: '화학공학과', track_category: '화학공정/반응/생산설계', focus: '화학 반응과 물질전달, 공정 설계를 바탕으로 소재·생산 시스템을 다루는 학과입니다.', hint: '화장품 제품보다 화학 공정과 생산 설계 전반에 더 관심 있는 학생에게 잘 맞습니다.' },
        { display_name: '화학과', track_category: '화학/분자/반응탐구', focus: '원자·분자 수준의 구조와 반응을 실험과 분석으로 탐구하는 기초과학 학과입니다.', hint: '제품 개발보다 성분 구조와 반응 원리를 더 깊게 보고 싶은 학생에게 잘 맞습니다.' },
