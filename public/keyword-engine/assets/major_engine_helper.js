...
(function () {
  ...
  function injectStyles() {
    ...
    style.textContent = `
      ...
      .engine-inline-preview { margin-top: 10px; display: grid; gap: 8px; }
      .engine-preview-card { border: 1px solid #dbe5f4; border-radius: 14px; background: #fbfdff; padding: 12px; }
      .engine-preview-head { display:flex; justify-content:space-between; gap:10px; align-items:flex-start; }
      .engine-preview-title { font-size: 13px; font-weight: 800; color:#172033; }
      .engine-preview-track { display:inline-flex; padding: 4px 8px; border-radius: 999px; background:#eef4ff; color:#245ee8; font-size:11px; font-weight:700; white-space:nowrap; }
      .engine-preview-copy { margin-top: 6px; color:#55647e; font-size:12px; line-height:1.55; }
      .engine-preview-chip-wrap { display:flex; flex-wrap:wrap; gap:8px; margin-top: 8px; }
      .engine-preview-chip { display:inline-flex; padding:6px 10px; border-radius:999px; background:#f1f5fd; color:#314767; font-size:12px; font-weight:700; }
      .engine-preview-note { color:#61708c; font-size:12px; line-height:1.5; }
      .engine-preview-empty { color:#6f7d95; font-size:12px; line-height:1.55; }
      .engine-status-row {
        display: grid;
  ...
  function injectUI() {
    ...
    if (keywordInput) {
      ...
      if (label && !$("engineMajorKeywordPreview")) {
        const preview = document.createElement("div");
        preview.id = "engineMajorKeywordPreview";
        preview.className = "engine-inline-preview";
  ...
  function bindEvents() {
    ...
    window.addEventListener("major-engine-selection-changed", function (event) {
      syncMajorSelectionFromEvent(event?.detail || null);
      syncCareerFromInput();
      renderAll();
    });
  ...
  function getAutoKeywordPreviewText() {
    return getAutoKeywordPreviewList().slice(0, 4).join(", ");
  }
  ...
  function renderSelectionSummary() {
    ...
    const chips = [
      state.subject,
      state.career,
      state.majorSelectedName && state.majorSelectedName !== state.career ? state.majorSelectedName : "",
    ...
    payloadEl.innerHTML = `
      ...
      전공 자동 키워드: ${escapeHtml((getAutoKeywordPreviewList() || []).join(", ") || "-")}<br>
  ...
  function buildMiniPayload() {
    ...
    return {
      ...
      concept_context: {
        concept: state.concept,
        keyword: state.keyword,
        auto_keyword_preview: getAutoKeywordPreviewList()
      },
      book_context: {
      ...
      major_compare_context: {
        ...
        track_category: state.majorTrackCategory || '',
        auto_keywords: getAutoKeywordPreviewList(),
