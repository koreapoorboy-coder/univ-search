...
window.__MAJOR_ENGINE_HELPER_VERSION__ = "v0.7.1-major-search-integrated-bundle";
...
(function(){
  ...
  const MAJOR_COPY_OVERRIDES = {
    '안경광학과': {
      ...
      fit: '시각 검사, 정밀 측정, 광학 원리와 의료 보조 기술에 관심 있는 학생에게 잘 맞습니다.',
      intro: '안경광학과는 시각 기능과 굴절 이상, 렌즈와 광학 원리를 바탕으로 시력 교정, 검안, 시기능 평가를 배우는 학과입니다.',
      subjects: ['물리학', '생명과학', '보건', '통합과학1', '공통수학1'],
      topics: ['굴절 이상 교정 원리와 렌즈 종류별 차이 분석', '시각 기능 검사 항목이 진로 적합성과 연결되는 방식 탐구', '광학 기초 개념이 검안·시력 교정 장비에 적용되는 구조 이해'],
      group_label: '시각·검사 기반 학과',
  ...
  function classifyCandidateGroup(row, rawInput){
    ...
    const rules = [
      ...
      { id:'clinical_health', label:'환자 진료·검사 쪽', desc:'환자 돌봄, 검사, 영상, 보건관리처럼 의료 현장과 가까운 학과입니다.', test: /(보건관리|간호|안경광학|방사선|임상병리|치위생|치기공|응급구조|의예|약학|한의|수의|보건)/ },
  ...
  function buildStudentDescription(profile, group){
    ...
    const rules = [
      ...
      [/안경광학/, '시각 기능 평가와 렌즈·광학 원리를 바탕으로 검안과 시력 교정을 배우는 학과입니다.'],
  ...
  function buildStudentFit(profile, group){
    ...
    const rules = [
      ...
      [/안경광학/, '시각 검사, 광학 기초, 정밀 측정 기반의 의료 보조 직무에 관심 있는 학생에게 잘 맞습니다.'],
